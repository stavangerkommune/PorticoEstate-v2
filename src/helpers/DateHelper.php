...
class DateHelper
{
	...
	public static function showDate($t = 0, $format = '', $preferences = []): string
	{
		...
		$preferences = isset(Settings::getInstance()->get('user')['preferences']) ? Settings::getInstance()->get('user')['preferences'] : $preferences;
	 ...
	 */
	public static function dateformatorder($yearstr, $monthstr, $daystr, $add_seperator = False)
	{
		$userSettings = Settings::getInstance()->get('user');

		$dateformat = strtolower($userSettings['preferences']['common']['dateformat']);
		$sep = substr($userSettings['preferences']['common']['dateformat'], 1, 1);
