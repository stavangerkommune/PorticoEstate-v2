...
class rental_uicontract extends rental_uicommon
{
	...
	private $account_id, $accounts_obj;
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('contracts')]);
	...
	private function _get_filters()
	{
		...
		if ($this->isAdministrator() || $this->isExecutiveOfficer())
		{
			...
			foreach ($types as $id => $label)
			{
				...
				if ($names['appname'] == $this->flags['currentapp'])
				{
...
*/
	...
	private function _get_tableDef_composite($mode, $contract_id)
	{
		...
		if (!empty($this->config->config_data['contract_future_info']))
		{
			$columns_def[] = array(
				"key" => "contracts",
				"label" => lang('contract_future_info'),
				"sortable" => false,
				"hidden" => false
			);
		}
	...
	private function _get_tableDef_price($mode, $contract_id)
	{
		if (empty($this->config->config_data['contract_furnished_status']))
		{
			$columns_def = array(
				...
				array(
					...
					'formatter' => $mode == 'edit' ? 'formatterDateStart_price_item' : "",
					'className' => 'center'
				),
				array(
					"key" => "date_end",
					"label" => lang('date_end'),
					'formatter' => $mode == 'edit' ? 'formatterDateEnd_price_item' : "",
					'className' => 'center'
				),
				array(
					"key" => "is_one_time",
					"label" => lang('is_one_time'),
					'formatter' => $mode == 'edit' ? 'formatterIs_one_time' : "",
					'className' => 'center'
				),
				array(
		...
		else
		{
			$columns_def = array(
				...
				array(
					'key' => 'custom_factor',
					'label' => lang('custom price factor'),
					'className' => '',
					'sortable' => true,
					'hidden' => false
				),
...
*/
	...
	private function _get_tableDef_invoice($mode, $contract_id)
	{
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uiinvoice_price_item.query',
				'type' => 'invoice_price_items',
				'editable' => true,
				'invoice_id' => '-1',
				'phpgw_return_as' => 'json'
	...
	private function _get_tableDef_document($mode, $contract_id, $permission = false)
	{
		...
		$datatable_def[] = array(
			'container' => $table_name,
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uidocument.query',
				'type' => 'documents_for_contract',
				'editable' => true,
				'contract_id' => $contract_id,
				'phpgw_return_as' => 'json'
			))),
	...
	private function _get_tableDef_notification($mode, $contract_id)
	{
		...
		$datatable_def[] = array(
			'container' => $table_name,
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uinotification.query',
				'type' => 'notifications',
				'editable' => true,
				'contract_id' => $contract_id,
				'phpgw_return_as' => 'json'
			...
			'ColumnDefs' => array(
				...
				array(
					'key' => 'name',
					'label' => lang('user_or_group'),
					'sortable' => false,
					'hidden' => false
				),
				array(
					'key' => 'field_of_responsibility',
					'label' => lang('field_of_responsibility'),
					'sortable' => false,
					'hidden' => false
				)
			),
	...
	public function query()
	{
		...
		switch ($type)
		{
			case 'contracts_for_adjustment':
				...
				$filters = array(
					'contract_type' => $adjustment->get_responsibility_id(),
					...
					'extra_adjustment' => $adjustment->is_extra_adjustment()
				);
				break;
			case 'contracts_part':	// Contracts for this party
				$filters = array(
					'party_id' => Sanitizer::get_var('party_id'),
					'contract_status' => Sanitizer::get_var('contract_status'),
					'contract_type' => Sanitizer::get_var('contract_type'),
					'status_date_hidden' => Sanitizer::get_var('status_date')
				);
				break;
	...
	public function index()
	{
		...
		$jqcal2->add_listener('filter_start_date_report');
		$jqcal2->add_listener('filter_end_date_report');
...
JS;
	...
	public function save()
	{
		...
		if (isset($contract_id) && $contract_id > 0)
		{
			...
			{
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_edit_contract'));
			}
	...
	public function get($id = 0)
	{
		...
		if (!empty($contract_id))
		{
			...
			{
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_view_contract'));
			}
			...
			$ret['executive_officer'] = $this->accounts_obj->id2name($contract->get_executive_officer_id());
	...
	public function view()
	{
		...
		{
			$account = $this->accounts_obj->get($executive_officer);
			if (!empty($account))
			{
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uicontract.get_total_price',
				'contract_id' => $contract_id,
				'phpgw_return_as' => 'json'
			))),
			'ColumnDefs' => array(
		...
		$link_included_composites = json_encode(self::link(array(
		...
		$link_included_parties = json_encode(self::link(array(
			'menuaction' => 'rental.uiparty.query',
			'type' => 'included_parties',
			'editable' => true,
			'contract_id' => $contract_id,
			'phpgw_return_as' => 'json'
		)));
		$link_included_price_items = json_encode(self::link(array(
			'menuaction' => 'rental.uiprice_item.query',
			'type' => 'included_price_items',
			'editable' => true,
			'contract_id' => $contract_id,
			'phpgw_return_as' => 'json'
		)));
...
JS;
		...
		$data = array(
			...
			'security_amount_simbol' => $this->userSettings['preferences']['common']['currency'],
	...
	public function get_executive_officer_options()
	{
		...
		$accounts = $this->acl->get_user_list_right(ACL_ADD, $location_info['location'], 'rental');
		$executive_officer_options[] = array('id' => '', 'name' => lang('nobody'), 'selected' => 0);
		foreach ($accounts as $account)
		{
			$executive_officer_options[] = array(
				'id' => $account['account_id'],
				'name' => $this->accounts_obj->get($account['account_id'])->__toString(),
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		else
		{
			if ($this->isAdministrator() || $this->isExecutiveOfficer())
			{
				...
				$created_by = $this->accounts_obj->id2name($this->account_id);
			...
			{
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_new_contract'));
			}
		}
		...
		$jqcal->add_listener('date_start');
		$jqcal->add_listener('date_end');
		$jqcal->add_listener('due_date');
		$jqcal->add_listener('billing_start_date');
		$jqcal->add_listener('billing_end_date');
		...
		$accounts = $this->acl->get_user_list_right(ACL_ADD, $location_name, 'rental');
		$executive_officer_options[] = array('id' => '', 'name' => lang('nobody'), 'selected' => 0);
		foreach ($accounts as $account)
		{
			$executive_officer_options[] = array(
		...
		if ($contract_id)
		{
			...
			$link_not_included_composites = json_encode(self::link(array(
				'menuaction' => 'rental.uicomposite.query',
				'type' => 'not_included_composites',
				'editable' => true,
				'contract_id' => $contract_id,
			...
			$link_not_included_parties = json_encode(self::link(array(
				'menuaction' => 'rental.uiparty.query',
				'type' => 'not_included_parties',
				'editable' => true,
				'contract_id' => $contract_id,
			...
			$link_not_included_price_items = json_encode(self::link(array(
				'menuaction' => 'rental.uiprice_item.query',
				'type' => 'not_included_price_items',
				'editable' => true,
				'contract_id' => $contract_id,
			...
			$link_upload_document = json_encode(self::link(array(
				'menuaction' => 'rental.uidocument.add',
				'contract_id' => $contract_id,
				'phpgw_return_as' => 'json'
			)));
			...
			/*				 * ***************************** notification form */
			$jqcal = createObject('phpgwapi.jqcal');
			$jqcal->add_listener('date_notification');

			$notification_recurrence_options[] = array(
				'id' => rental_notification::RECURRENCE_NEVER,
				'name' => lang('never'),
				'selected' => 1
			);
			$notification_recurrence_options[] = array(
				'id' => rental_notification::RECURRENCE_ANNUALLY,
				'name' => lang('annually'),
				'selected' => 0
			);
			$notification_recurrence_options[] = array(
				'id' => rental_notification::RECURRENCE_MONTHLY,
				'name' => lang('monthly'),
				'selected' => 0
			);
			$notification_recurrence_options[] = array(
				'id' => rental_notification::RECURRENCE_WEEKLY,
				'name' => lang('weekly'),
				'selected' => 0
			);

			$accounts_users = $this->acl->get_user_list_right(ACL_READ, 'run', 'rental');
			$users[] = array('id' => $this->account_id, 'name' => lang('target_me'));
			foreach ($accounts_users as $account)
			{
				if ($account['account_id'] != $this->account_id)
				{
					$users[] = array('id' => $account['account_id'], 'name' => $this->accounts_obj->get($account['account_id'])->__toString());
				}
			}
			$notification_user_group_options[] = array(
				'label' => lang('notification_optgroup_users'),
				'options' => $users
			);

			$accounts_groups = $this->accounts_obj->get_list('groups');
...
JS;
		...
		$data = array(
			...
			'value_publish_comment' => $contract->get_publish_comment(),
			'value_cancelled' => $contract->get_cancelled(),
			'value_cancelled_by_name' => $contract->get_cancelled_by() ? $this->accounts_obj->get($contract->get_cancelled_by())->__toString() : '',
			'location_id' => $contract->get_location_id(),
			'contract_id' => $contract_id,
			'mode' => $mode,
			'link_included_composites' => $link_included_composites,
			...
			'tabs' => phpgwapi_jquery::tabview_generate($tabs, $active_tab),
			'img_cal' => json_encode($this->phpgwapi_common->image('phpgwapi', 'cal')),
			'dateformat' => str_ireplace(array('d', 'm', 'y'), array('dd', 'mm', 'yy'), $this->userSettings['preferences']['common']['dateformat']),
			'moveout' => $moveout,
			'movein' => $movein
		);
	...
	public function notify_on_expire()
	{
		...
		foreach ($candidates as $contract_id => $contract)
		{
			...
			if ($do_notify)
			{
				...
				try
				{
					$rcpt = $send->msg('email', $notify_on_expire_email, $subject, stripslashes($message), '', $cc='', $bcc='', $from_email, $from_email, 'html');
				}
				catch (Exception $exc)
				{
