...
class rental_bofellesdata
{
	...
	public function get_result_unit_with_leader($org_unit_id, $org_level = 4)
	{
		...
		$columns = "V_ORG_ENHET.ORG_ENHET_ID, V_ORG_ENHET.ORG_NAVN, V_ORG_ENHET.EPOST,"
			. " V_ORG_PERSON.FORNAVN, V_ORG_PERSON.ETTERNAVN, V_ORG_PERSON.BRUKERNAVN, V_ORG_PERSON.EPOST_ARBEID AS EPOST_LEDER";
		$tables = "V_ORG_ENHET";
	...
	protected function log($class, $function)
	{
		...
		if (isset($serverSettings['log_levels']['module']['rental']) && $serverSettings['log_levels']['module']['rental'])
		{
			$bt = debug_backtrace();
			$log = new Log();
			$log->debug(array(
