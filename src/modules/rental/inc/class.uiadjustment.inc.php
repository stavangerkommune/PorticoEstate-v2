...
class rental_uiadjustment extends rental_uicommon
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang($this->flags['currentapp']) . '::' . lang('adjustment')]);
	}
...
JS;
	...
	public function save()
	{
		...
		if (isset($adjustment_id) && $adjustment_id > 0)
		{
			...
			{
				unset($adjustment);
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_edit_adjustment'));
			}
		}
		else
		{
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		else
		{
			...
			{
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_new_adjustment'));
			}
		}

		$jqcal = createObject('phpgwapi.jqcal');
		$jqcal->add_listener('adjustment_date');

		$adjustment_date = ($adjustment->get_adjustment_date()) ? $this->phpgwapi_common->show_date($adjustment->get_adjustment_date(), $this->dateFormat) : '';

		$current_adjustment_type = $adjustment->get_adjustment_type();
		$adjustment_type_options[] = array(
			'id' => 'adjustment_type_KPI',
			'name' => 'adjustment_type_KPI',
			'selected' => (($current_adjustment_type == 'adjustment_type_KPI') ? 1 : 0)
		);
		$adjustment_type_options[] = array(
			'id' => 'adjustment_type_deflator',
			'name' => 'adjustment_type_deflator',
	...
	{
		phpgw::no_access($this->flags['currentapp'], lang('permission_denied_view_adjustment'));
	}
		...
		if ($adjustment_id)
		{
			...
			$datatable_def[] = array(
				...
				'ColumnDefs' => array(
					...
					array(
						'key' => 'adjustment_interval',
						'label' => lang('adjustment_interval'),
						'sortable' => false,
						'className' => 'center'
					),
					array(
						'key' => 'adjustment_share',
						'label' => lang('adjustment_share'),
						'sortable' => false,
						'className' => 'center'
					),
					array(
						'key' => 'adjustment_year',
						'label' => lang('adjustment_year'),
						'sortable' => false,
						'className' => 'center'
					)
				),
