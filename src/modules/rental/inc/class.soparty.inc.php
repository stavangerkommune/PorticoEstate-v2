...
class rental_soparty extends rental_socommon
{
	...
	function update($party)
	{
		...
		if ($location_id)
		{
			...
			$loc = $location_obj->get_name($location_id);
			$name = $loc['location'];
	...
	{
		$acl = Acl::getInstance();
		return $acl->check(self::LOCATION_ROOT, ACL_PRIVATE, 'rental');
	}
	...
	protected function isExecutiveOfficer()
	{
		$acl = Acl::getInstance();
		return (
			$acl->check(self::LOCATION_IN, ACL_ADD, 'rental') ||
			$acl->check(self::LOCATION_OUT, ACL_ADD, 'rental') ||
			$acl->check(self::LOCATION_INTERNAL, ACL_ADD, 'rental')
		);
	}
