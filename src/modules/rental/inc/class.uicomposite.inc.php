...
class rental_uicomposite extends rental_uicommon
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('rc')]);
...
*/
		...
		switch ($query_type)
		{
			...
			case 'all_composites': // ... all composites, filters (active and vacant)
				...
				$filters = array(
					...
					'has_contract' => Sanitizer::get_var('has_contract'),
					'availability_date_from' => Sanitizer::get_var('filter_availability_date_from'),
					'availability_date_to' => Sanitizer::get_var('filter_availability_date_to'),
	...
	public function index()
	{
		...
		$jqcal->add_listener('filter_availability_date_from');
		$jqcal->add_listener('filter_availability_date_to');
	...
	public function view()
	{
		...
		{
			phpgw::no_access($this->flags['currentapp'], lang('invalid_request'));
		}
		...
		{
			phpgw::no_access($this->flags['currentapp'], lang('permission_denied_view_composite'));
		}
	}
	...
	{
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang($mode)]);
		...
		if ($mode == 'edit')
		{
			...
			{
				phpgw::no_access($this->flags['currentapp'], lang('permission_denied_edit'));
			}
		}
		...
		if ($composite_id)
		{
			...
			$datatable_def[] = array(
				...
				'requestUrl' => json_encode(self::link(array(
					'menuaction' => 'rental.uiunit.query',
			...
			if ($mode == 'edit')
			{
				...
				$datatable_def[] = array(
					...
					'requestUrl' => json_encode(self::link(array(
						'menuaction' => 'rental.uiproperty_location.query',
						'composite_id' => $composite_id,
						'phpgw_return_as' => 'json',
			...
			$datatable_def[] = array(
				...
				'requestUrl' => json_encode(self::link(array(
					'menuaction' => 'rental.uicontract.query',
					'type' => 'contracts_for_composite',
					'composite_id' => $composite_id,
					'editable' => 0,
					'phpgw_return_as' => 'json'
			...
			if ($mode == 'edit')
			{
				$search_options[] = array(
					'id' => 'objno_name_address',
					'name' => lang('objno_name_address'),
					'selected' => 1
				);
			...
			$contracts_search_options[] = array(
				'id' => 'id',
				'name' => lang('contract_id'),
				'selected' => 0
			);
			$contracts_search_options[] = array(
				'id' => 'party_name',
				'name' => lang('party_name'),
				'selected' => 0
			);
			$contracts_search_options[] = array(
				'id' => 'composite',
				'name' => lang('composite_name'),
				'selected' => 0
			);
			$contracts_search_options[] = array(
				'id' => 'composite_address',
				'name' => lang('composite_address'),
				'selected' => 0
			);
			$contracts_search_options[] = array(
				'id' => 'location_code',
				'name' => lang('object_number'),
				'selected' => 0
			);
		...
		$jqcal = createObject('phpgwapi.jqcal');
		$jqcal->add_listener('status_date');
		...
		foreach ($composite_standard_arr as $composite_standard)
		{
			...
			$composite_standard_options[] = array(
				'id' => $composite_standard['id'],
				'name' => $composite_standard['name'],
				'selected' => $selected
			);
		}
		$composite_type_name = '';
...
JS;
		...
		$data = array(
			...
			'list_part_of_town' => array('options' => execMethod('property.bogeneric.get_list', array(
				'type' => 'part_of_town',
				'selected' => $composite->get_part_of_town_id(),
				'add_empty' => true
			))),
	...
	function add_unit()
	{
		if (!($this->isExecutiveOfficer() || $this->isAdministrator()))
		{
			phpgw::no_access($this->flags['currentapp'], lang('permission_denied'));
		}
	...
	function columns()
	{
		...
		{
			$preferences = CreateObject('phpgwapi.preferences', $this->userSettings['account_id']);
			$preferences->add('rental', 'rental_columns_composite', $values['columns'], 'user');
	...
	public function schedule()
	{
		...
		$schedule['picker_img'] = $this->phpgwapi_common->image('phpgwapi', 'cal');
	...
	public function handle_multi_upload_file()
	{
		...
		$options['upload_dir'] = $this->serverSettings['files_dir'] . '/rental/' . $options['base_dir'] . '/';
