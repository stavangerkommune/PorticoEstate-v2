...
class rental_uiapplication extends rental_uicommon
{
	...
	{
		parent::__construct();
		$app_header = $this->flags['app_header'];
		Settings::getInstance()->update('flags', ['app_header' => $app_header . '::' . lang('application')]);
...
*/
	...
	public function view()
	{
		...
		$app_header = $flags['app_header'];
		Settings::getInstance()->update('flags', ['app_header' => $app_header . '::' . lang('view')]);
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		$flags = Settings::getInstance()->get('flags');
		$app_header = $flags['app_header'];
		Settings::getInstance()->update('flags', ['app_header' => $app_header . '::' . lang('edit')]);
		...
		$jqcal2->add_listener('date_start', 'datetime');
		$jqcal2->add_listener('date_end', 'datetime');
		$jqcal2->add_listener('assign_date_start', 'datetime');
		$jqcal2->add_listener('assign_date_end', 'datetime');

		$accounts = $this->acl->get_user_list_right(ACL_EDIT, $this->acl_location, 'rental');
		...
		foreach ($accounts as $account)
		{
			$executive_officer_options[] = array(
				...
				'name' => $accounts_obj->get($account['account_id'])->__toString(),
		...
		$data = array(
			...
			'step'		=> $step,
			'value_ecodimb_descr' => ExecMethod(
				'property.bogeneric.get_single_attrib_value',
				array(
					'type' => 'dimb',
	...
	public function query()
	{
		...
		foreach ($applications['results'] as $key => &$application)
		{
			...
			$application['entry_date'] = $this->phpgwapi_common->show_date($application['entry_date']);
			$application['assign_date_start'] = $this->phpgwapi_common->show_date($application['assign_date_start'], $dateformat);
			$application['assign_date_end'] = $this->phpgwapi_common->show_date($application['assign_date_end'], $dateformat);
			$application['executive_officer'] = $application['executive_officer'] ? $accounts_obj->get($application['executive_officer'])->__toString() : '';
