			...
			switch (strlen('' . $identifier))
			{
				case 4: // Intern organisasjonstilknytning
					...
					)
					{
						...
						// Get location ID
						$subs = $this->locations->get_subs_from_pattern('rental', '.ORG.BK.__.' . $identifier); //cPersonForetaknr
						if (count($subs) > 0)
						{
		...
		foreach ($datalines as $data)
		{
			...
			if ($type_id == '3')
			{
				if (isset($date) && is_numeric($date) && $contract_id > 0)
				{
					...
					{
						$old_end_date = date($this->userSettings['preferences']['common']['dateformat'], $con->get_contract_date()->get_end_date());
