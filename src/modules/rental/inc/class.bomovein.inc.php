<?php
	...
	class rental_bomovein extends phpgwapi_bocommon
	{
		...
		public function read_single($id, $return_object = true)
		{
			if ($id)
			{
				$values = rental_somovein::get_instance()->read_single($id, $return_object);
			}
			else
			{
			...
			if($custom_fields)
			{
				$custom_fields = rental_somovein::get_instance()->read_custom_field_values($id, $custom_fields);
				$_values = createObject('property.custom_fields')->prepare(array('attributes' => $custom_fields), 'rental', rental_movein::acl_location, $view = false);
