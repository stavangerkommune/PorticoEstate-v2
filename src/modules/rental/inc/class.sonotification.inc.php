	...
	public function populate_workbench_notifications($day = null)
	{
		...
		while ($this->db2->next_record())
		{
			...
			if ($is_today_notification_date)
			{
				...
				if ($account_id)
				{
					if ($this->accounts_obj->get_type($account_id) == phpgwapi_account::TYPE_GROUP)
					{
						$accounts = $this->accounts_obj->get_members($account_id);
				...
				if ($location_id)
				{
					$location_names = $this->location_obj->get_name($location_id);
					if ($location_names['appname'] == $this->flags['currentapp'])
					{
						$acl = Acl::getInstance();
						$responsible_accounts = $acl->get_user_list_right(ACL_EDIT, $location_names['location']);
