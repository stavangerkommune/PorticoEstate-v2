...
class rental_uibilling extends rental_uicommon
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('invoice_menu')]);
	...
	public function add()
	{
		...
		if (Sanitizer::get_var('step') == 2  && Sanitizer::get_var('next') != null) // User clicked next on step 2
		{
			...
			if (($contract_ids != null && is_array($contract_ids) && count($contract_ids) > 0) || (is_array($contract_bill_only_one_time) && count($contract_bill_only_one_time) > 0)) // User submitted contracts to bill
			{
				...
				{
					$_decimal_place = isset($this->userSettings['preferences']['rental']['currency_decimal_places']) ? isset($this->userSettings['preferences']['rental']['currency_decimal_places']) : 2;
...
JS;
			...
			if (($contract_ids != null && is_array($contract_ids) && count($contract_ids) > 0) || (is_array($contract_bill_only_one_time) && count($contract_bill_only_one_time) > 0)) // User submitted contracts to bill
			{
				...
				if ($missing_billing_info == null || count($missing_billing_info) == 0)
				{
					$billing_job = rental_sobilling::get_instance()->create_billing(
						isset($this->userSettings['preferences']['rental']['currency_decimal_places']) ? isset($this->userSettings['preferences']['rental']['currency_decimal_places']) : 2,
						Sanitizer::get_var('contract_type'),
						Sanitizer::get_var('billing_term'),
						Sanitizer::get_var('year'),
						Sanitizer::get_var('month'),
		...
		if ($step == 2 || (Sanitizer::get_var('step') == '1' && Sanitizer::get_var('next') != null) || Sanitizer::get_var('step') == 'simulation' && Sanitizer::get_var('previous') != null) // User clicked next on step 1 or previous on step simulation
		{
				...
				foreach ($contract_price_items as $contract_price_item)
				{
					if (!array_key_exists($contract_price_item->get_contract_id(), $contracts))
					{
						$aditional_contracts = rental_socontract::get_instance()->get(0, 0, '', false, '', '', array(
							'contract_id' => $contract_price_item->get_contract_id(),
							'contract_type' => $contract_type
						));
			...
			$uicols_irregular_contracts[] = array(
				'key' => 'override',
				'label' => lang('override'),
				'className' => 'center',
				'formatter' => 'formatCheckOverride'
			);
			$uicols_irregular_contracts[] = array(
				'key' => 'bill2',
				'label' => lang('bill2'),
				'className' => 'center',
				'formatter' => 'formatCheckBill2'
			);
			...
			$uicols_contracts_with_one_time[5] = array(
				'key' => 'bill_only_one_time',
				'label' => lang('only_one_time'),
				'className' => 'center'
			);
			$uicols_contracts_with_one_time[6] = array(
				'key' => 'total_price',
				'label' => lang('total_price'),
				'className' => 'right',
				'hidden' => false,
...
JS;
	...
	public function view()
	{
		...
		{
			phpgw::no_access($this->flags['currentapp']);
		}

		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('invoice_run')]);
		...
		$last_updated = $this->phpgwapi_common->show_date($billing_job->get_timestamp_stop(), $this->dateFormat . ' H:i:s');
		...
		{
			$timestamp_commit = $this->phpgwapi_common->show_date($timestamp_commit, $this->dateFormat . ' H:i:s');
		}
		...
		$tabletools[] = array(
			...
			'download' => self::link(array(
				'menuaction' => 'rental.uibilling.download',
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uibilling.query',
				'type' => 'invoices',
				'editable' => true,
				'billing_id' => $billing_job->get_id(),
				'phpgw_return_as' => 'json'
...
JS;
	...
	private function transfer($id, $export_format)
	{
		...
		if ($config->config_data['common']['method'] == 'ftp' || $config->config_data['common']['method'] == 'ssh')
		{
			...
			{
				case 'ftp';
					$tmpfname = tempnam(sys_get_temp_dir(), 'rental_');
	...
	public function add_actions(&$value, $key, $params)
	{
		...
		switch ($query_type)
		{
			case 'all_billings':
				...
				{
					$url_delete = html_entity_decode(self::link(array(
						'menuaction' => 'rental.uibilling.delete',
						'id' => $value['id'],
						'phpgw_return_as' => 'json'
					)));
					$actions[] = '<a href="#" onclick="onDelete(\'' . $url_delete . '\')">' . lang('delete') . '</a>';

					$url_commit = html_entity_decode(self::link(array(
						'menuaction' => 'rental.uibilling.commit',
						'id' => $value['id'],
						'phpgw_return_as' => 'json'
					)));
