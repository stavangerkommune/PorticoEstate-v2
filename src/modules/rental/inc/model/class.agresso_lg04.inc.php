...
class rental_agresso_lg04 implements rental_exportable
{
	...
	protected function run()
	{
		...
		if (
			isset($serial_config_start) && is_numeric($serial_config_start) &&
	...
	protected function run_excel_export($excel_export_type)
	{
		...
		foreach ($invoices as $invoice) // Runs through all invoices
		{
			...
			$start_date = $phpgwapi_common->show_date($contract->get_contract_date()->get_start_date(), $this->dateformat);
			$end_date = $phpgwapi_common->show_date($contract->get_contract_date()->get_end_date(), $this->dateformat);
			$billing_start_date = $phpgwapi_common->show_date($contract->get_billing_start_date(), $this->dateformat);
			$billing_end_date = $phpgwapi_common->show_date($contract->get_billing_end_date(), $this->dateformat);
