...
class rental_agresso_gl07 implements rental_exportable
{
	...
	protected $billing_job,
		...
		$service_id_strlen,
		$project_id_strlen,$userSettings;
	...
	{
		$this->lines = array();
		$decimal_separator = isset($this->userSettings['preferences']['rental']['decimal_separator']) ? $this->userSettings['preferences']['rental']['decimal_separator'] : ',';
		$thousands_separator = isset($this->userSettings['preferences']['rental']['thousands_separator']) ? $this->userSettings['preferences']['rental']['thousands_separator'] : '.';
		...
		foreach ($invoices as $invoice) // Runs through all invoices
		{
			...
			$responsibility_in = isset($this->userSettings['preferences']['rental']['responsibility']) ? $this->userSettings['preferences']['rental']['responsibility'] : '028120';
			$project_id_in = isset($this->userSettings['preferences']['rental']['project_id']) ? $this->userSettings['preferences']['rental']['project_id'] : '9';
	...
	protected function get_order_excel_nlsh($invoice, $responsibility, $building, $project, $price_item, $description)
	{
		...
		$start_date = $phpgwapi_common->show_date($contract->get_contract_date()->get_start_date(), $dateformat);
		$end_date = $phpgwapi_common->show_date($contract->get_contract_date()->get_end_date(), $dateformat);
		$billing_start_date = $phpgwapi_common->show_date($contract->get_billing_start_date(), $dateformat);
		$billing_end_date = $phpgwapi_common->show_date($contract->get_billing_end_date(), $dateformat);
	...
	protected function get_formatted_amount_excel($amount)
	{
		return $amount;
		//			$amount = round($amount, 2) * 100;
		//			$belop = (float)(substr($amount, 0, strlen($amount) - 2) . '.' . substr($amount, -2));
