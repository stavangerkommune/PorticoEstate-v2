...
class rental_uiparty extends rental_uicommon
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('parties')]);
	}
		...
		switch ($type)
		{
			case 'included_parties':
				if ($editable)
				{
					...
					{
						$url = html_entity_decode(self::link(array(
							'menuaction' => 'rental.uicontract.set_payer',
							'party_id' => $value['id'],
							'contract_id' => $params[0],
			...
			default:
				...
				if ($user_is[ADMINISTRATOR] || $user_is[EXECUTIVE_OFFICER])
				{
					...
					{
						$url1 = html_entity_decode(self::link(array(
							'menuaction' => 'rental.uiparty.delete_party',
							'id' => $value['id'],
							'phpgw_return_as' => 'json'
						)));
					...
					{
						$url2 = html_entity_decode(self::link(array(
							'menuaction' => 'frontend.uihelpdesk.index',
							'org_enhet_id' => $value['org_enhet_id']
						)));
						$actions[] = '<a href="' . $url2 . '">' . lang('frontend_access') . '</a>';

						$url3 = html_entity_decode(self::link(array(
							'menuaction' => 'rental.uiparty.syncronize_party',
							'org_enhet_id' => $value['org_enhet_id'],
							'party_id' => $value['id'],
							'phpgw_return_as' => 'json'
						)));
...
JS;
	...
	public function view()
	{
		...
		{
			phpgw::no_access($this->flags['currentapp'], lang('permission_denied_view_party'));
		}
	}
	...
	{
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('parties') . '::' . lang($mode)]);
		...
		else
		{
			...
			if ($email_validator->check_email_address($email) && !$this->accounts_obj->exists($email))
			{
				$valid_email = 1;
	...
	public function sync()
	{
		...
		switch ($sync_job)
		{
			case 'resp_and_service':
				...
				$extra_cols = array(
					array(
						...
						"label" => lang('responsibility_id'),
						"sortable" => false,
						"hidden" => false
					),
					array(
						"key" => "sync_message",
						"label" => lang('sync_message'),
						"sortable" => false,
						"hidden" => false
					),
					array(
						"key" => "org_unit_name",
						"label" => lang('org_unit_name'),
						"sortable" => false,
						"hidden" => false
					)
				);
			...
			case 'res_unit_number':
				...
				$extra_cols = array(
					array(
						"key" => "result_unit_number",
						"label" => lang('result_unit_number'),
						"sortable" => false,
						"hidden" => false
					),
					array(
			...
			case 'org_unit':
				...
				$extra_cols = array(
					array(
						"key" => "org_unit_name",
						"label" => lang('sync_org_name_fellesdata'),
						"sortable" => false,
						"hidden" => false
					),
					array(
						"key" => "dep_org_name",
						"label" => lang('sync_org_department_fellesdata'),
						"sortable" => false,
						"hidden" => false
					),
					array(
						"key" => "unit_leader",
						"label" => lang('sync_org_unit_leader_fellesdata'),
						"sortable" => false,
						"hidden" => false
					),
					array(
						"key" => "org_email",
						"label" => lang('sync_org_email_fellesdata'),
						"sortable" => false,
						"hidden" => false
					)
				);
		...
		if (($this->isExecutiveOfficer() || $this->isAdministrator()))
		{
			...
			if (isset($org_unit_id) && $org_unit_id > 0)
			{
				...
				$jsonArr = array(
					...
					"unit_leader_fullname" => trim($unit_leader_fullname),
					"department" => trim($dep_org_name)
				);
	...
	public function create_user_based_on_email()
	{
		...
		if (($this->isExecutiveOfficer() || $this->isAdministrator()))
		{
			if (isset($party_id) && $party_id > 0)
			{
				...
				if ($this->accounts_obj->exists($email))
				{
				...
				$passwd = $this->phpgwapi_common->randomstring(6) . "ABab1!";
				...
				try
				{
					...
					$account->expires = -1;
					$frontend_account = $this->accounts_obj->create($account, array($renter_group), array(), array(
						'frontend'
					));
				...
				if (isset($this->serverSettings['smtp_server']) && $this->serverSettings['smtp_server'])
				{
					...
					//Send email
					$rcpt = $send->msg('email', $email, $title, stripslashes(nl2br($message)), '', '', '', $from, 'System message', 'html', '', array(), false);
