...
class rental_uiresultunit extends rental_uicommon
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('delegates')]);
	}
	...
	{
		Settings::getInstance()->update('flags', ['app_header' => lang('rental') . '::' . lang('delegates') . '::' . lang('edit')]);
		...
		if (($this->isExecutiveOfficer() || $this->isAdministrator()) && $use_fellesdata)
		{
			$tabletools[] = array(
				...
				'custom_code' => "
					...
					'name' => 'account_id',
					'source' => 'account_id'
				), array('name' => 'owner_id', 'source' => 'owner_id')))) . ";
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'rental.uidelegate.query',
				'unit_id' => $unit_id,
				'type' => 'included_delegates',
				'phpgw_return_as' => 'json'
			))),
			'ColumnDefs' => array(
	...
	public function add_delegate(int $account_id, $org_unit_id, $org_name)
	{
		...
		if ($success)
		{
			...
			$user_account = $accounts_obj->get($account_id);
			$owner_account = $accounts_obj->get($this->userSettings['account_id']);
	...
	public function remove_delegate()
	{
		...
		{
			$result = frontend_bofrontend::remove_delegate($account_id, 0, $unit_id);
			if ($result)
			{
