...
class uiifc
{
	...
	private $serverSettings, $userSettings, $phpgwapi_common;

	function __construct()
	{
		Settings::getInstance()->update('flags', ['xslt_app' => true, 'menu_selection' => 'bim::ifc']);
	...
	function index()
	{
		...
		$data = array(
			...
			'cat_filter' => $this->cats->formatted_xslt_list(array(
				'select_name' => 'cat_id',
				'selected' => $this->cat_id, 'globals' => true, 'link_data' => $link_data
			)),
			'filter_data' => $this->nextmatchs->xslt_filter(array(
				'filter' => $this->filter,
				'link_data' => $link_data
			)),
			'allow_allrows'							=> true,
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('bim') . ': ' . $function_msg]);
	...
	function import()
	{
		Settings::getInstance()->update('flags', ['menu_selection' => 'bim::ifc::import']);
		...
		if (isset($values) && is_array($values))
		{
			if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
			{
				...
				if (!isset($receipt['error']) || !$receipt['error'])
				{
					...
					{
						Cache::session_set('ifc_receipt', 'session_data', $receipt);
		...
		$msgbox_data = isset($msgbox_data) ? $this->phpgwapi_common->msgbox_data($receipt) : '';
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('ifc') . ': ' . lang('import')]);
		//_debug_array($data);
	...
	function edit()
	{
		...
		$insert_record_values = Cache::session_get('ifc', 'insert_record_values' . $acl_location);
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('bim') . ' - ' . $appname . ': ' . $function_msg]);
		if ($output == 'wml')
		{
			phpgwapi_xslttemplates::getInstance()->set_output('wml');
		}
	...
	function no_access()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('bim') . ' - ' . $appname]);
