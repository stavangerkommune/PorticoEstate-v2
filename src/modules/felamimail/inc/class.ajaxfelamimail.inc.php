	...
	class ajaxfelamimail {
		...
		*/
		function compressFolder() 
		{
			...
			if($messageCounter > $GLOBALS['phpgw_info']["user"]["preferences"]["common"]["maxmatchs"]) {
				$lastPage = $messageCounter - ($messageCounter % $GLOBALS['phpgw_info']["user"]["preferences"]["common"]["maxmatchs"]) + 1;
				if($lastPage > $messageCounter)
					$lastPage -= $GLOBALS['phpgw_info']["user"]["preferences"]["common"]["maxmatchs"];
		...
		*/
		...
		function changeActiveAccount($accountData)
		{
			...
			require_once(PHPGW_INCLUDE_ROOT.'/felamimail/inc/class.bopreferences.inc.php');
		...
		*/
		function emptyTrash() 
		{
			...
			if(!empty($GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder'])) {
				$this->bofelamimail->compressFolder($GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder']);
			}
		...
		*/
		...
		function generateMessageList($_folderName) 
		{
			...
			$headerTable = $this->uiwidgets->messageTable(
				...
				$listMode,
				$_folderName,
				$GLOBALS['phpgw_info']['user']['preferences']['felamimail']['message_newwindow'],
				$GLOBALS['phpgw_info']['user']['preferences']['felamimail']['rowOrderStyle']
			);
			...
			if(!empty($GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder'])) {
				$folderStatus = $this->bofelamimail->getFolderStatus($GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder']);
				if($folderStatus['unseen'] > 0) {
					$response->addScript("tree.setItemText('". $GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder'] ."', '<b>". $folderStatus['shortDisplayName'] ." (". $folderStatus['unseen'] .")</b>');");
				} else {
					$response->addScript("tree.setItemText('". $GLOBALS['phpgw_info']['user']['preferences']['felamimail']['trashFolder'] ."', '". $folderStatus['shortDisplayName'] ."');");
				}
			}
		...
		*/
		...
		{
			$GLOBALS['phpgw']->session->appsession('ajax_session_data','',$this->sessionDataAjax);
