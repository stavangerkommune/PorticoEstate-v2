...
class sms_sosms
{
	...
	function read_inbox($data)
	{
		...
		while ($this->db->next_record())
		{
			$inbox[] = array(
				...
				'user' => $accounts_obj->id2name($this->db->f('in_uid')),
			);
		}
	...
	function read_outbox($data)
	{
		...
		if ($acl_location)
		{
			...
			$grants = $this->acl->get_grants2('sms', $acl_location);
		...
		while ($this->db->next_record())
		{
			$outbox[] = array(
				...
				'user' => $accounts_obj->id2name($this->db->f('uid')),
				'dst_group' => $accounts_obj->id2name($this->db->f('p_gpid')),
