...
class sms_socommand
{
	...
	function read_single_command($id)
	{
		...
		$this->db->query($sql, __LINE__, __FILE__);
		$bin_path = PHPGW_SERVER_ROOT . "sms/bin/{$this->userSettings['domain']}";
		$values = array();
	...
	function add_command($values)
	{
		...
		$values['exec'] = PHPGW_SERVER_ROOT . "/sms/bin/{$this->userSettings['domain']}/{$values['exec']}";
