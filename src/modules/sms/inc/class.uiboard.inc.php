...
class sms_uiboard
{
	...
	function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['menu_selection' => 'sms::board']);
	}
	...
	function index()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('SMS') . ' - ' . lang('List/Edit/Delete SMS boards')]);
		...
		while ($this->db->next_record())
		{
			...
			$content .= "[<a href=" . phpgw::link('/index.php', array(
				'menuaction' => 'sms.uiboard.view',
				'board_id' => $this->db->f('board_id')
			)) . ">v</a>] ";
			$content .= "[<a href=" . phpgw::link('/index.php', array(
				'menuaction' => 'sms.uiboard.edit',
				'board_id' => $this->db->f('board_id')
			)) . ">e</a>] ";
			$content .= "[<a href=" . phpgw::link('/index.php', array(
				'menuaction' => 'sms.uiboard.delete',
				'board_id' => $this->db->f('board_id')
			)) . ">x</a>] ";
	...
	function view()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('SMS') . ' - ' . lang('View SMS board')]);
		$this->phpgwapi_common->phpgw_header();
	...
	function add()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('SMS') . ' - ' . lang('Add SMS board')]);
		$this->phpgwapi_common->phpgw_header();
	...
	function edit()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('SMS') . ' - ' . lang('Edit SMS board')]);
		$this->phpgwapi_common->phpgw_header();
