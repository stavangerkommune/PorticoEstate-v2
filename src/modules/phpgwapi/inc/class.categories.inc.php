...
class phpgwapi_categories
{
	...
	function __construct($accountid = '', $app_name = '', $location = '')
	{
		...
		$account_id = (int)$this->userSettings['account_id'];

		if (!$app_name)
		{
			$app_name = $this->flags['currentapp'];
		}
	...
	function total($for = 'app')
	{
		...
		{
			case 'app':
				$w = " WHERE cat_appname='{$this->app_name}' {$filter_location}";
				break;
			case 'appandmains':
				$w = " WHERE cat_appname='{$this->app_name}' {$filter_location} AND cat_parent =0";
				break;
			case 'appandsubs':
				$w = " WHERE cat_appname='{$this->app_name}' {$filter_location} AND cat_parent !=0";
				break;
			case 'subs':
				$w = ' WHERE cat_parent != 0';
				break;
			case 'mains':
				$w = ' WHERE cat_parent = 0';
				break;
	...
	function return_array($type, $start, $limit = True, $query = '', $sort = '', $order = '', $globals = False, $parent_id = '', $lastmod = -1, $column = '', $use_acl = false)
	{
		...
		if ($column)
		{
			switch ($column)
			{
				case 'id':
					...
					break;
				case 'owner':
					$table_column = ' cat_owner ';
					break;
				case 'access':
					$table_column = ' cat_access ';
					break;
				case 'app_name':
					$table_column = ' cat_appname ';
					break;
				case 'main':
					$table_column = ' cat_main ';
					break;
				case 'parent':
					$table_column = ' cat_parent ';
					break;
				case 'name':
					$table_column = ' cat_name ';
					break;
				case 'description':
					$table_column = ' cat_description ';
					break;
				case 'data':
					$table_column = ' cat_data ';
					break;
				case 'last_mod':
					$table_column = ' last_mod ';
					break;
				default:
					$table_column = ' cat_id ';
					break;
		...
		if ($use_acl)
		{
			foreach ($_cats as $cat)
			{
				...
				if ($this->acl->check($location, ACL_READ, $this->app_name))
				{
					$cats[] = $cat;
				}
			}
	...
	function return_sorted_array($start, $limit = True, $query = '', $sort = '', $order = '', $globals = False, $parent_id = 0, $use_acl = false)
	{
		...
		{
			$scats = array();
			$max = $this->userSettings['preferences']['common']['maxmatchs'];
			$max = $max + $start;
...
*/
				...
				$s .= '>' . str_repeat('.&nbsp;', (int)$cat['level']) . phpgw::strip_html($cat['name']);

				//					$s .= phpgw::strip_html($cat['name']);
		...
		if ($format == 'list')
		{
			...
			if ($this->total_records > 0)
			{
				for ($i = 0; $i < count($cats); ++$i)
				{
					...
					$s .= '<td>' . $space_set . '<a href="' . phpgw::link($site_link, 'cat_id=' . $cats[$i]['id']) . '">'
	...
	function formatted_xslt_list($data = 0)
	{
		...
		phpgwapi_xslttemplates::getInstance()->add_file($this->phpgwapi_common->get_tpl_dir('phpgwapi', 'base') . '/categories');
		...
		foreach ($cats as $key => $cat)
		{
			...
			$name = str_repeat(' . ', (int)$cat['level']) . phpgw::strip_html($cat['name']);
	...
	function add($values)
	{
		...
		$this->hooks->single($args, $this->app_name);
	...
	{
		$this->app_name		= $this->db->db_addslashes($appname);
		$this->location		= $this->db->db_addslashes($location);
		$this->location_id	= $this->locations->get_id($appname, $location);
		$this->grants		= $this->acl->get_grants($appname, $location);
	}
