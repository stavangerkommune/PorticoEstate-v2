<?php
	...
	*/
	...
	if ( !isset($serverSettings['db_type'])
		|| empty($serverSettings['db_type']) )
	{
		$serverSettings['db_type'] = 'mysql';
	}
	...
	*/
	abstract class phpgwapi_sql_
	{
		...
		*/
		...
		{
			$str = Db::getInstance()->db_addslashes($str);
			return "'$str'";
		}
		...
		* Data types conversion                                      *
		...
		public static function int_date2str($int, $format=False)
		{
			...
			$format = $format ? $format : $userSettings['preferences']['common']['dateformat'];
		...
		{
			$userSettings = Settings::getInstance()->get('user');
			$format = $format ? $format : $userSettings['preferences']['common']['timeformat'];
			return date($format, intval($int));
		}
