<?php
	...
	class contacts_ extends sql_builder
	{
		 ...
		 */
		var $trans = False;
		var $userSettings, $serverSettings, $accounts, $acl;
...
//			$this->db = Db::getInstance();
			...
			$this->account_id = isset($this->userSettings['account_id']) ? $this->userSettings['account_id'] : 0;
			...
			if (isset($this->serverSettings['contact_repository']) && $this->serverSettings['contact_repository'] == 'ldap')
			{
				$phpgwapi_common = new \phpgwapi_common();
				$this->LDAPSyncEnabled = true;
				$this->LDAPResource = $phpgwapi_common->ldapConnect($this->serverSettings['ldap_contact_host'], $this->serverSettings['ldap_contact_dn'], $this->serverSettings['ldap_contact_pw']);
			}
		...
		{
			$data = Cache::session_get('phpgwapi', 'session_data');
		...
		function save_sessiondata()
		{
			...
			Cache::session_set('phpgwapi', 'session_data', $data);
		}
		...
		function add( $owner, $fields, $access = '', $cat_id = '', $tid = 'n' )
		{
			...
			{
				$criteria = $this->criteria_for_index($this->userSettings['account_id'], PHPGW_CONTACTS_CATEGORIES_ALL, array(
		...
		function get_system_contacts( $data, $action = PHPGW_SQL_RUN_SQL )
		{
			...
			$accounts = $this->accounts->get_list();
		...
		function edit_contact( $cid, $principal, $action = PHPGW_SQL_RETURN_SQL )
		{
			...
			{
				$principal['owner'] = $this->userSettings['account_id'];
			}
		...
		function add_contact( $type, $principal = array(), $comms = array(), $locations = array(), $categories = array(), $others = array(), $contact_relations = array(), $notes = array() )
		{
			...
			else if (isset($this->userSettings['account_id']))
			{
				$owner = (int)$this->userSettings['account_id'];
			}
...
//					$this->lock_table($this->location->table, '', true);
		...
		function delete( $cid, $contact_type = '', $transaction = True )
		{
			...
			$hooks = new Hooks();
			$hook_response = $hooks->process(array(
		...
		{
			$account_id = $this->accounts->search_person($contact_id);
		...
		function are_users( $person_id, $data = '' )
		{
			...
			while ($this->db->next_record())
			{
				...
				$contact['account_id'] = $this->accounts->search_person($contact['contact_id']);
				$contacts[] = $contact;
			}
			return $contacts;
		}
		...
		function criteria_for_index( $owner, $access = PHPGW_CONTACTS_ALL, $categories = PHPGW_CONTACTS_CATEGORIES_ALL, $search_fields = array(), $pattern = '', $show_fields = array() )
		{
			...
			switch ($access)
			{
				...
				default:
					$criteria = phpgwapi_sql_criteria::token_or(phpgwapi_sql_criteria::token_and(phpgwapi_sql_criteria::_equal('access', 'public'), phpgwapi_sql_criteria::_in('owner', $this->get_contacts_shared($owner, ACL_READ))), phpgwapi_sql_criteria::_equal('owner', $owner));
			}
		...
		function get_contacts_shared( $owner_id, $acl_type = ACL_READ )
		{
			$required_grants = array();
			$this->grants = $this->acl->get_grants('addressbook');
			if (isset($this->serverSettings['addressmaster']) && $this->acl->check('addressmaster', 7, 'addressbook'))
			{
				$required_grants[] = $this->serverSettings['addressmaster'];
			}
		...
		{
			$grants = $this->acl->get_grants('addressbook');
			if ($owner_id == '')
			{
			...
			return ($this->check_perms($grants[$owner_id], $needed) || $this->acl->check('addressmaster', 7, 'addressbook'));
		}
		...
		{
			return $this->check_acl($contact_id, ACL_ADD, $owner_id);
		}
		...
		{
			return $this->check_acl($contact_id, ACL_EDIT, $owner_id);
		}
		...
		{
			return $this->check_acl($contact_id, ACL_READ, $owner_id);
		}
		...
		{
			return $this->check_acl($contact_id, ACL_DELETE, $owner_id);
		}
		...
		{
			$type = ($type) ? $type : $this->get_type_contact($contact_id);
		...
		{
			if (isset($this->userSettings['account_id']))
			{
				return $this->userSettings['account_id'];
			}
			return 0;
		}
		...
		{
			$result = ldap_search($this->LDAPResource, $this->serverSettings['ldap_contact_context'], 'employeeNumber=' . $id, array(
				'dn'));
		...
		function LDAPSync( $id )
		{
			...
			$dn = 'uid=' . mb_convert_encoding($uid, 'UTF-8', 'ISO-8859-1') . ',' . $this->serverSettings['ldap_contact_context'];
		...
		function contact_import( $fields, $type = '', $update = false )
		{
			...
			if ((isset($fields['first_name']) && !isset($fields['org_name'])) || $type)
			{
				...
				$contact['owner'] = isset($fields['owner']) ? $fields['owner'] : $this->userSettings['account_id'];
			...
			if (count($fields) > 0)
			{
				foreach ($fields as $field_name => $field_value)
				{
					...
					$other['other_owner'] = $this->userSettings['account_id'];
					$others[] = $other;
				}
			}
