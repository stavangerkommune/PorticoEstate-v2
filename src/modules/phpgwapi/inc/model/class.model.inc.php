...
abstract class phpgwapi_model
{
	...
	public function __construct(int $id)
	{
		...
		$this->appname = $this->flags['currentapp'];
	...
	{
		return $this->acl->check($this->get_field_of_responsibility_name(), $permission, $this->appname);
	}
	...
	public function get_permission_array()
	{
		...
		return array(
			ACL_READ => $this->acl->check($location_name, ACL_READ, $this->appname),
			ACL_ADD => $this->acl->check($location_name, ACL_ADD, $this->appname),
			ACL_EDIT => $this->acl->check($location_name, ACL_EDIT, $this->appname),
			ACL_DELETE => $this->acl->check($location_name, ACL_DELETE, $this->appname),
			ACL_PRIVATE => $this->acl->check($location_name, ACL_PRIVATE, $this->appname)
		);
	}
				...
				if (!empty($value) && is_array($value))
				{
					foreach ($value as $key => $sub_entity)
					{
						$this->_validate(
							...
							$entity,
							array($field => $params['manytomany']['column']),
							$errors
						);
						$sub_entity_count++;
					}
