...
class phpgwapi_custom_fields
{
	...
	public function __construct($appname = null)
	{
		...
		{
			$this->_appname = $flags['currentapp'];
		}
		...
		$this->_datetimeformat 	= Db::datetime_format();
	...
	public function add($attrib, $attrib_table = null, $doubled = false)
	{
		...
		{
			$attrib_table = $this->locations->get_attrib_table($appname, $location);
		}
		...
		$values = array(
			...
			'lookup_form'	=> false,
			'disabled'		=> false,
			'helpmsg'		=> isset($attrib['helpmsg']) ? $this->_db->db_addslashes($attrib['helpmsg']) : '',
			'group_id'		=> isset($attrib['group_id']) ? (int) $attrib['group_id'] : 0,
			'attrib_sort'	=> 0,
			'datatype'		=> $this->_db->db_addslashes($attrib['column_info']['type']),
			'precision_'	=> isset($attrib['column_info']['precision']) ? (int) $attrib['column_info']['precision'] : 0,
			'scale'			=> isset($attrib['column_info']['scale']) ? (int) $attrib['column_info']['scale'] : 0,
			'default_value'	=> '',
			'nullable'		=> $attrib['column_info']['nullable'],
			'get_list_function'	=> isset($attrib['get_list_function']) ? $attrib['get_list_function'] : null,
			'get_list_function_input' => isset($attrib['get_list_function_input']) ? $this->_db->db_addslashes(serialize($attrib['get_list_function_input'])) : '',
			'get_single_function'	=> isset($attrib['get_single_function']) ? $attrib['get_single_function'] : null,
			'get_single_function_input' => isset($attrib['get_single_function_input']) ? $this->_db->db_addslashes(serialize($attrib['get_single_function_input'])) : '',
	...
	{
		$loc_id		= $this->locations->get_id($appname, $location);
	...
	public function delete($appname, $location, $attrib_id, $table = '', $doubled = false)
	{
		...
		if (!$table)
		{
			$table	= $this->locations->get_attrib_table($appname, $location);
		}
	...
	function edit_group($group)
	{
		...
		$location_id	= $this->locations->get_id($group['appname'], $group['location']);
	...
	function edit($attrib, $attrib_table = '', $doubled = false)
	{
		...
		if (!$attrib_table)
		{
			$attrib_table = $this->locations->get_attrib_table($attrib['appname'], $attrib['location']);
		}

		$location_id	= $this->locations->get_id($attrib['appname'], $attrib['location']);
	...
	public function delete_choice($location_id, $attrib_id, $choice_id, $replacement_id = null)
	{
		...
		$sql = "SELECT column_name FROM phpgw_cust_attribute WHERE location_id = " . (int) $location_id
			. " AND id= " . (int) $attrib_id;
		$this->_db->query($sql, __LINE__, __FILE__);
		$this->_db->next_record();
		$column_name = $this->_db->f('column_name');

		$category = CreateObject('property.soadmin_entity')->get_single_category($location_id, true);
		if ($category['is_eav'])
		{
			$sql = "SELECT location_id FROM fm_bim_item WHERE location_id = " . (int) $location_id
	...
	)
	{
		...
		if (!$allrows)
		{
			$num_rows = isset($this->userSettings['preferences']['common']['maxmatchs']) ? (int) $this->userSettings['preferences']['common']['maxmatchs'] : 15;
	...
	public function get_table_def($table = '', $table_def = array())
	{
		...
		$setup = new \App\modules\phpgwapi\services\setup\Process();
		$setup->oProc = $this->_oProc;
		$setup->oProc->m_odb->fetchmode = 'BOTH';
