...
abstract class phpgwapi_socommon
{
	...
	protected function add(&$object)
	{
		...
		{
			if (($field_info['action'] & ACL_ADD) && empty($field_info['join']) && empty($field_info['related']) && empty($field_info['manytomany']))
			{
		...
		$attrib_table = $this->location_obj->get_attrib_table($appname, $object::acl_location);
	...
	protected function update($object)
	{
		...
		{
			if (($field_info['action'] & ACL_EDIT) && empty($field_info['join']) && empty($field_info['related']) && empty($field_info['manytomany']))
			{
				if ($field_info['type'] == 'json')
				{
					$value_set[$field] = json_encode($object->$field);
