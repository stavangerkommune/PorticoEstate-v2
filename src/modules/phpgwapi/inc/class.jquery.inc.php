...
class phpgwapi_jquery
{
	...
	public static function load_widget($widget)
	{
		...
		switch ($widget)
		{
			...
			case 'bootstrap-multiselect':
				...
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/bootstrap-multiselect/css/bootstrap-multiselect.min.css");

				break;
			case 'select2':
				$load = array(
					//				"js/{$_jquery_core}{$_type}",
					'select2' => array("js/select2{$_type}", "js/i18n/{$userSettings['preferences']['common']['lang']}")
				);

				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/select2/css/select2{$_type}.css");

				if (in_array($userSettings['preferences']['common']['template_set'], array('bootstrap', 'bootstrap2', 'bookingfrontend', 'mobilefrontend')))
				{
					//						phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/select2/css/select2-bootstrap4{$_type}.css");
					phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/select2/css/select2-bootstrap-5-theme{$_type}.css");
				}
			...
			case 'glider':
				...
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/glider/glider{$_type}.css");
				break;
			case 'intl-tel-input':
				$load = array(
				...
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/intl-tel-input/css/intlTelInput.min.css");
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/intl-tel-input/css/isValidNumber.css");
			...
			case 'daterangepicker':
				...
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/daterangepicker/css/daterangepicker.css");
		...
		foreach ($load as $key => $scripts)
		{
			...
			{
				$test = phpgwapi_js::getInstance()->validate_file($package, $script);
				if (!$test)
				{
	...
	public static function formvalidator_generate($modules = array(), $form_id = 'form', $errorMessagePosition_id = '')
	{
		...
		switch ($userSettings['preferences']['common']['lang'])
		{
			case 'no':
			case 'nn':
				$lang	 = 'no';
			...
			case 'en':
			case 'pt':
				$lang	 = $userSettings['preferences']['common']['lang'];
				break;
			default:
				$lang	 = 'en';
				break;
...
JS;
	...
	public static function init_ckeditor($target)
	{
		...
		phpgwapi_js::getInstance()->validate_file('ckeditor', 'ckeditor');
		phpgwapi_js::getInstance()->validate_file('ckeditor', 'adapters/jquery');
		$userlang = isset($serverSettings['default_lang']) && $serverSettings['default_lang'] ? $serverSettings['default_lang'] : 'en';
	...
	public static function init_summernote($target)
	{
		...
		switch ($userSettings['preferences']['common']['template_set'])
		{
			...
			default:
				...
				phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/summernote/dist/summernote-lite.css");
				break;
		}
		...
		phpgwapi_js::getInstance()->validate_file('summernote', "dist/lang/summernote-{$lang}");
...
JS;
	...
	public static function init_quill($target)
	{
		 ...
		 */
		self::load_widget('core');
		phpgwapi_js::getInstance()->validate_file('quill', 'quill.min');
		phpgwapi_js::getInstance()->validate_file('quill', 'quill-textarea');
		phpgwapi_css::getInstance()->add_external_file("phpgwapi/js/quill/quill.snow.css");
...
JS;
	...
	public static function init_intl_tel_input($target)
	{
		...
		{
			$cache_refresh_token = $serverSettings['cache_refresh_token'];
		}
