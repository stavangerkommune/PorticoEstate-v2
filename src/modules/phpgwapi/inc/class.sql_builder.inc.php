	...
	class sql_builder
	{
		...
		function get_entity( $field )
		{
			...
			$this->ldebug('get_entity', array('Field' => $field,
				'Entity Name' => $ent));
			if ((!isset($this->entities[$ent]) || !is_object($this->entities[$ent]) ) && !empty($ent))
			{
		...
		{
			if (isset($this->entities[$entity_name]) && is_object($this->entities[$entity_name]))
			{
				$this->ldebug('instance_entity', "Already an object $entity_name", 'msg');
			}
			else
			{
				$this->entities[$entity_name] = createObject($entity_name);
				$this->ldebug('instance_entity', "Creating object $entity_name", 'msg');
			}
		...
		function sort_by_distances()
		{
			...
			$this->ldebug('sort_by_distances', array('Distances' => $this->distance,
				'Path' => $this->path,
			...
			while (!($this->last_distance()))
			{
				...
				if ($entity_distance != 0)
				{
					...
					$this->add_path($entity_far_away, TRUE);
					$this->entity_to_center($entity_far_away, $entity_distance, TRUE);
					$this->ldebug('sort_by_distances', array('Path' => array_keys($this->path)), 'dump');
					$this->merge_paths();
				}
			}
		}
		...
		function entity_to_center( $entity_name, $entity_distance, $reverse = FALSE )
		{
			...
			$links = array_merge($this->entities[$entity_name]->get_ilinks(), $this->entities[$entity_name]->get_elinks());
			...
			foreach ($links as $entity => $link)
			{
				...
				$this->ldebug('entity_to_center', array('field' => $field,
				...
				{
					//We found a good one
					$this->ldebug('entity_to_center', "Actual Distance $dist < Min Distance $min_distance", 'msg');
					$min_distance = $dist;
		...
		function get_sql()
		{
			...
			$this->ldebug('get_sql', array('Entities' => array_keys($this->entities),
		...
		function get_join()
		{
			...
			// from +
			// table as alias (1)
			$this->ldebug('get_join', array('Path' => $path), 'dump');
			...
			foreach($path as $entity => $data)
			{
				...
				$this->ldebug('get_join', array('entity' => $entity,
				...
				$from .= ' = ' . $link['alias'] . '.' .
					$this->real_field($link['prev_entity'], $link['ffield']);
		...
		function ent_name( $field )
		{
			...
			else
			{
				...
				$this->raise_error();
				$this->ldebug('ent_name', array('Class Map' => $this->map), 'dump', 'error');
				$this->ldebug('ent_name', array('Field' => $field), 'string', 'error');
				return '';
			}
		}
		...
		function insert( $data, $value = '' )
		{
			...
			{
				$this->ldebug('insert', array('Data' => $field_));
		...
		function _delete( $criteria, $action = PHPGW_SQL_RETURN_SQL )
		{
			if (is_string($criteria))
			{
				...
				if (empty($field) || empty($value))
				{
					...
					$log->error(array('text' => 'E-ArgError, err: Wrong argument $criteria' . $criteria,
			...
			$criteria = $entity->index_criteria(array('real_field' => $real_name, 'value' => $value,
				'field' => $field));
		...
		function _request( $data )
		{
			 ...
			 * 1-1.
			 */
			if (isset($this->map[$data][PHPGW_SQL_CHILD_FIELDS]) && is_array($this->map[$data][PHPGW_SQL_CHILD_FIELDS]) && count($this->map[$data][PHPGW_SQL_CHILD_FIELDS]) > 0)
			{
			...
			if (isset($this->map[$data][PHPGW_SQL_CHANGE_DISTANCE]) && (int)$this->map[$data][PHPGW_SQL_CHANGE_DISTANCE] != 0)
			{
