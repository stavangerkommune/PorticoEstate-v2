...
// 		}
			...
			for ($i = 0; $i < count($ldap_fields); $i++)
			{
				...
				if ($ldap_fields[$i]['uidnumber'][0])
				{
					...
					if ($yes)
					{
						if ($DEBUG)
						{
							echo $ldap_fields[$i]['uidnumber'][0] . ' matched all!' . '<br>';
						}
					...
					else
					{
						if ($DEBUG)
						{
							echo $ldap_fields[$i]['uidnumber'][0] . ' did not match all.' . '<br>';
						}
					}
				}
			}
		...
		function formatted_address( $id, $business = True, $afont = '', $asize = '2' )
		{
			...
			$t = createObject('phpgwapi.template', $phpgwapi_common->get_tpl_dir('addressbook'));
			...
			{
				$address[$k] = phpgw::strip_html($val);
			}
			...
			if (!$country)
			{
				$country = $this->userSettings['preferences']['common']['country'];
			}

			if (file_exists(PHPGW_SERVER_ROOT . '/addressbook/templates/default/format_' . strtolower($country) . '.tpl'))
			{
			...
			if ($country != $this->userSettings['preferences']['common']['country'])
			{
		...
		function formatted_address_full( $id, $business = True, $afont = '', $asize = '2' )
		{
			...
			if (file_exists(PHPGW_SERVER_ROOT . '/addressbook/templates/default/full_format_' . strtolower($country) . '.tpl'))
			{
		...
		function formatted_address_line( $id, $business = True, $afont = '', $asize = '2' )
		{
			...
			if (file_exists(PHPGW_SERVER_ROOT . '/addressbook/templates/default/line_format_' . strtolower($country) . '.tpl'))
			{
