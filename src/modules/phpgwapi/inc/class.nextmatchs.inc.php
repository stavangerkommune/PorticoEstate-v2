	...
	class phpgwapi_nextmatchs extends phpgwapi_nextmatchs_xslt
	{
		...
		*/
		public function __construct($website = false)
		{
			...
			$this->maxmatches = 15;
			if ( isset($this->userSettings['preferences']['common']['maxmatchs']) &&
		...
		protected function _get_var()
		{
			return array
			(
				Sanitizer::get_var('filter'),
				Sanitizer::get_var('qfield'),
				Sanitizer::get_var('start'),
				Sanitizer::get_var('order'),
				Sanitizer::get_var('sort'),
				Sanitizer::get_var('query')
			);
		}
		...
		function page($extravars = array())
		{
			...
			return phpgw::link('/index.php', $extravars);
		}
		...
		*
		...
		public static function filterobj($filtertable, $idxfieldname, $strfieldname)
		{
			...
			{
				$index++;
				$filter_obj[$index][0] = $row[$idxfieldname];
				$filter_obj[$index][1] = $row[$strfieldname];
			}
		...
		*
		...
		function show_sort_order($sort, $var = '', $order = '', $program = '', $text = '', $extra='', $build_an_href = True)
		{
			...
			$link = strlen($this->action) ? $this->page($extra) : phpgw::link($program, $extra);
		...
		function show_sort_order_imap($old_sort, $new_sort, $default_order, $order, $program, $text, $extra = array() )
		{
			...
			$link = strlen($this->action) ? $this->page($extravar) : phpgw::link($program,$extravar);
			return '<a href="' .$link .'">' .$text .'</a>';
		}
		...
		protected function _set_link($align, $img, $link, $alt, $extravars)
		{
			...
			{
				$action = phpgw::link($link, $extravars);
			}
			...
			(
				...
				'form_name' => $img,
				'hidden'    => $hidden,
				'img'       => $phpgwapi_common->image('phpgwapi', $img),
				'label'     => $alt,
		...
		{
			$phpgwapi_common = new phpgwapi_common();
			$img_full = $phpgwapi_common->image('phpgwapi', $img);
