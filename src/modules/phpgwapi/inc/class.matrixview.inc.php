...
class phpgwapi_matrixview
{
	...
	function __construct($month_int = 0, $year_int = 0)
	{
		...
		$this->set1PixelGif($phpgwapi_common->get_image_path('todo') . '/pix.gif');
	}
	...
	function setPeriod($item, $fromdate, $todate, $color = '#990033')
	{
		...
		while ($go == 1)
		{
			...
			if (
				mktime(0, 0, 0, $fmonth, $fday + $i, $fyear) >= mktime(0, 0, 0, $this->month + 1, 0, $this->year) ||
