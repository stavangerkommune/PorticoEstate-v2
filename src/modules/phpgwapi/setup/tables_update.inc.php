...
function phpgwapi_upgrade0_9_14_508()
{
	...
	while ($GLOBALS['phpgw_setup']->oProc->next_record())
	{
		$records[$GLOBALS['phpgw_setup']->oProc->f('id')] = array(
	...
	if (is_array($records))
	{
		foreach ($records as $key => $data)
		{
			...
			$GLOBALS['phpgw_setup']->db->query(
				"INSERT INTO phpgw_contact_person (person_id,first_name,last_name,
...
function phpgwapi_upgrade0_9_17_515()
{
	...
	$GLOBALS['phpgw_setup']->oProc->query(
		'SELECT phpgw_acl_location.*, phpgw_applications.app_id '
	...
	$locations = array();
	$GLOBALS['phpgw_setup']->oProc->query(
		'SELECT DISTINCT phpgw_acl.acl_location, phpgw_locations.location_id, phpgw_applications.app_name'
			. ' FROM phpgw_acl'
	...
	$apps = array();
	$GLOBALS['phpgw_setup']->oProc->query(
		'SELECT phpgw_history_log.history_appname, phpgw_applications.app_id'
...
function phpgwapi_upgrade0_9_17_555()
{
	...
	{
		//from file
		if ($bookmarks = \App\modules\phpgwapi\services\Cache::_user_get('controller', "location_bookmark", $user_id))
		{
			//to database
			\App\modules\phpgwapi\services\Cache::user_set('controller', "location_bookmark", $bookmarks, $user_id);
		}

		//from file
		if ($bookmarks = \App\modules\phpgwapi\services\Cache::_user_get('phpgwapi', "bookmark_menu", $user_id))
		{
			//to database
			\App\modules\phpgwapi\services\Cache::user_set('phpgwapi', "bookmark_menu", $bookmarks, $user_id);
		}
	}
