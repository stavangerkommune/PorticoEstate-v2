...
switch ($acct_type)
{
	...
	case 'ldap':
		include_once SRC_ROOT_PATH . "/modules/phpgwapi/controllers/Accounts/AccountsLdap.php";
		break;

	default:
		$acct_type = 'Sql';
}
include_once SRC_ROOT_PATH . "/modules/phpgwapi/controllers/Accounts/Accounts{$acct_type}.php";
...
abstract class Accounts_
{
	...
	public function update_user($user, $groups, $acls = array(), $modules = null)
	{
		...
		(new \App\modules\phpgwapi\services\Hooks())->process('editaccount');
	...
	{
		\App\modules\phpgwapi\services\Cache::system_set('phpgwapi', "account_{$account->id}", $account);
	}
