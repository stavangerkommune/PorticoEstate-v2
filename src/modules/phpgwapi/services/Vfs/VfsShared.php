...
class VfsShared
{
	...
	function __construct()
	{
		...
		$this->working_lid = $this->userSettings['account_lid'];
		...
		$this->mime_magic = new \App\helpers\MimeMagic();
	}
	...
	function locks_save_session()
	{
		...
		$this->session = Cache::session_set($app, 'vfs_shared', base64_encode(serialize($this->override_locks)));
	}
	 ...
	 *			of the ACL_* defines, for example:
	 *			ACL_READ
	 *			ACL_READ|ACL_ADD
	 *  * owner_id	phpGW ID to check access for.
	 *  * 			Default: $this->userSettings['account_id']
	...
	function view($data)
	{
		...
		$this->flags['noappheader'] = true;
		$this->flags['noappfooter'] = true;
		$ls_array = $this->ls(
			array(
				'string'	=>  $data['string'],
		...
		elseif (isset($this->userSettings['preferences']['filemanager']['viewtextplain']))
		{
	...
	function correct_attributes($data)
	{
		...
		elseif (preg_match("+^$this->fakebase\/(.*)$+U", $p->fake_full_path, $matches))
		{
			...
			$set_attributes_array = array(
				'owner_id' => $accounts_obj->name2id($matches[1])
			);
		}
		else
		{
...
*/
	...
	function getabsolutepath($data = array())
	{
		...
		{
			$basedir = $basedir . $this->userSettings['account_lid'] . $sep;
		}

		if ($data['mask'][0] & RELATIVE_USER_APP)
		{
			$basedir = $basedir . "." . $this->flags['currentapp'] . $sep;
		}
	...
	function cd($data = '')
	{
		...
		if ($data['relative'] == 'relative' || $data['relative'] == True)
		{
			...
			{
				$currentdir = Cache::session_get('vfs', 'currentdir');
				$basedir = $this->getabsolutepath(
					array(
		...
		Cache::session_set('vfs', 'currentdir', $basedir);
