...
class Crypto extends Crypto_
{
	...
	function init($vars)
	{
		...
		if (($serverSettings['mcrypt_enabled'] || $serverSettings['enable_crypto'] == 'mcrypt') && extension_loaded('mcrypt') && !$this->enabled) {
			$this->algo = MCRYPT_TRIPLEDES;
			$this->mode = MCRYPT_MODE_CBC;

			if (isset($serverSettings['mcrypt_algo'])) {
				$this->algo = $serverSettings['mcrypt_algo'];
			}
			if (isset($serverSettings['mcrypt_mode'])) {
				$this->mode = $serverSettings['mcrypt_mode'];
			}
