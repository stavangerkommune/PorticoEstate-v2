...
class LoginHelper
{
	...
	public function __construct($msg_only = false)
	{
		...
		if (
			isset($this->serverSettings['deny_all_logins'])
			&& $this->serverSettings['deny_all_logins']
		)
		{
			$tmpl->set_file(
				array(
	...
	public function	processLogin(Request $request, Response $response, array $args)
	{
		...
		if (empty($_POST))
		{
			$this->phpgw_display_login($variables, Sanitizer::get_var('cd', 'int', 'GET', 0));
		}
		else
		{
			$Login = new Login();
			...
			{
				$this->phpgw_display_login($variables, $Login->get_cd());
			}
		}
	...
	function check_langs()
	{
		...
		if (
			isset($this->serverSettings['lang_ctimes'])
			&& !is_array($this->serverSettings['lang_ctimes'])
		)
		{
	...
	function phpgw_display_login($variables, $cd = 0)
	{
		...
		if (
			$this->serverSettings['domain_from_host']
			&& !$this->serverSettings['show_domain_selectbox']
		)
		{
			$this->tmpl->set_var(
				array(
		...
		if ((!isset($this->serverSettings['usecookies']) || !$this->serverSettings['usecookies'])
...
JS;
		...
		$this->tmpl->set_var(
			'website_title',
			isset($this->serverSettings['site_title'])
				? $this->serverSettings['site_title']
				: 'PorticoEstate'
		);
...
HTML;
		...
		$autocomplete = '';
		if (
			isset($this->serverSettings['autocomplete_login'])
			&& $this->serverSettings['autocomplete_login']
		)
		{
