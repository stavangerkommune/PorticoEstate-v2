...
class helpdesk_sotts
{
	...
	$userSettings, $serverSettings, $location_obj, $acl,$account_obj;
	...
	function read($data)
	{
		...
		$this->grants	= $this->acl->get_grants2('helpdesk', '.ticket');
		...
		$categories = $this->location_obj->get_subs('helpdesk', '.ticket.category');

		$grant_category = array();
		foreach ($categories as $location)
		{
			if ($this->acl->check($location, ACL_READ, 'helpdesk'))
			{
		...
		if (!$dry_run)
		{
			...
			while ($this->db2->next_record())
			{
				...
				foreach ($custom_cols as $custom_col)
				{
					...
					{
						$custom_value = $this->custom->get_translated_value(
							array(
								'value' => $custom_value,
	...
	function read_single($id, $values = array())
	{
		...
		if ($this->db->next_record())
		{
			...
			{
				$ticket['reverse_name']	= $this->account_obj->get($ticket['reverse_id'])->__toString();
			}

			$ticket['user_name']	= $this->account_obj->get($ticket['user_id'])->__toString();
	...
	function update_view($id = '')
	{
		...
		$this->db->query("SELECT count(*) as cnt FROM phpgw_helpdesk_views where id={$id}"
			. " AND account_id='" . $this->userSettings['account_id'] . "'", __LINE__, __FILE__);
		$this->db->next_record();

		if (! $this->db->f('cnt'))
		{
			$this->db->query("INSERT INTO phpgw_helpdesk_views (id,account_id,time) values ({$id},'"
				. $this->userSettings['account_id'] . "','" . time() . "')", __LINE__, __FILE__);
		}
	}
	...
	function add(&$ticket, $values_attribute = array())
	{
		...
		{
			$ticket['assignedto'] = $this->userSettings['account_id'];
		}
		$ticket['user_id']	= !empty($ticket['set_user_id']) ? $ticket['set_user_id'] : $this->userSettings['account_id'];

		$value_set['priority'] = isset($ticket['priority']) ? $ticket['priority'] : 0;
		$value_set['user_id'] =  $ticket['user_id'];
		$value_set['reverse_id'] = $this->userSettings['account_id']; //The originator for the reversed ticket
		...
		{
			$set_notify_id = $this->account_obj->name2id($ticket['set_notify_lid']);
			$person_id = $this->account_obj->get($set_notify_id)->person_id;
	...
	function update_ticket(&$ticket, $id = 0, $receipt = array(), $values_attribute = array(), $simple = false)
	{
		...
		{
			$ticket['set_user_id'] = $this->account_obj->name2id($ticket['set_user_lid']);
		}
		...
		if (isset($ticket['status']) && ($old_status != $ticket['status']))
		{
			...
			if (($this->db->f('closed') || $ticket['status'] == 'X') && ($old_status != 'X' && !$old_closed))
			{
				...
				// at request
				execMethod(
					'property.sorequest.update_status_from_related',
					array(
						'location_id' => $location_id,
						'id' => $id,
						'status' => 'closed'
