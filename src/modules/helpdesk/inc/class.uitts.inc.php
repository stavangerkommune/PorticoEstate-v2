...
class helpdesk_uitts extends phpgwapi_uicommon_jquery
{
	...
	var $group_id, $account, $bo, $bocommon, $cats, $acl_location, $acl_read, $acl_add, $acl_edit, $acl_delete,
		...
		$location_code, $p_num, $type_id, $reported_by, $accounts_obj;

	public function __construct()
	{
		parent::__construct();

		self::set_active_menu('helpdesk::helpdesk');
		if ($this->tenant_id	= Cache::session_get('helpdesk', 'tenant_id'))
		{
			Settings::getInstance()->update('flags', ['noheader' => true, 'nofooter' => true]);
		}
		...
		{
			self::set_active_menu("helpdesk::helpdesk_{$this->parent_cat_id}");
		}
	...
	public function get_on_behalf_of()
	{
		...
		$account_list = $this->accounts_obj->get_list('accounts', -1, 'ASC', 'account_lastname',  $query, false, $filter);
...
HTML;
	...
	function download()
	{
		...
		$custom_cols = isset($this->userSettings['preferences']['helpdesk']['ticket_columns']) ? $this->userSettings['preferences']['helpdesk']['ticket_columns'] : array();
	...
	function edit_priority()
	{
		...
		if ((isset($this->bo->config->config_data['mailnotification']) && $this->bo->config->config_data['mailnotification']) || (isset($this->userSettings['preferences']['helpdesk']['tts_notify_me']) && $this->userSettings['preferences']['helpdesk']['tts_notify_me'] == 1 && $this->bo->fields_updated
			)
		)
		{
	...
	public function handle_multi_upload_file()
	{
		...
		$options['base_dir'] = $id;
		$options['upload_dir'] = $this->serverSettings['files_dir'] . '/helpdesk/' . $options['base_dir'] . '/';
	...
	function columns()
	{
		...
		{
			$preferences->add('helpdesk', 'ticket_columns', (array)$values['columns'], 'user');
	...
	private function _get_fields()
	{
		...
		$custom_cols = !empty($this->userSettings['preferences']['helpdesk']['ticket_columns']) ? $this->userSettings['preferences']['helpdesk']['ticket_columns'] : array();
	...
	private function _get_filters()
	{
		...
		$values_combo_box[3] = $this->bo->filter(array(
		...
		if (!$this->_simple)
		{
			...
			$filter_tts_assigned_to_me = $this->userSettings['preferences']['helpdesk']['tts_assigned_to_me'];
	...
	function index()
	{
		...
		{
			Settings::getInstance()->update('flags', ['app_header' => $this->lang_app_name . ': ' . lang('choose a section from the menu')]);
			$sub_menu = array();
		...
		$jqcal->add_listener('filter_start_date', 'date', '', array('no_button' => true));
		$jqcal->add_listener('filter_end_date', 'date', '', array('no_button' => true));
		...
		if (!empty($this->userSettings['preferences']['helpdesk']['refreshinterval']) && (int)$this->userSettings['preferences']['helpdesk']['refreshinterval'] > 0)
		{
			$refreshinterval = (int) $this->userSettings['preferences']['helpdesk']['refreshinterval'] * 1000;
...
JS;
		...
		if (!$this->_simple)
		{
			$jasper = execMethod('property.sojasper.read', array('location_id' => $this->locations->get_id('helpdesk', $this->acl_location)));
			...
			if (isset($this->userSettings['preferences']['helpdesk']['tts_status_link']) && $this->userSettings['preferences']['helpdesk']['tts_status_link'] == 'yes' && $this->acl_edit)
			{
				$status['X'] = array(
					'status' => lang('closed'),
				);
	...
	function add()
	{
		...
		Settings::getInstance()->update('flags', ['breadcrumb_selection' => $flags['menu_selection'] . '::add']);
		...
		if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
		{
			...
			$insert_record = Cache::session_get('helpdesk', 'insert_record');

			$insert_record_entity = Cache::session_get('helpdesk', 'insert_record_values' . $this->acl_location);
			...
			{
				$values['assignedto'] = isset($this->userSettings['preferences']['helpdesk']['assigntodefault']) ? $this->userSettings['preferences']['helpdesk']['assigntodefault'] : '';
			}
			...
			if (!isset($receipt['error']))
			{
				...
				//--------------end files
				Cache::session_set('helpdesk', 'receipt', $receipt);
		...
		if (isset($this->userSettings['preferences']['helpdesk']['tts_me_as_contact']) && $this->userSettings['preferences']['helpdesk']['tts_me_as_contact'] == 1)
		{
		...
		{
			$values['group_id'] = (isset($this->userSettings['preferences']['helpdesk']['groupdefault']) ? $this->userSettings['preferences']['helpdesk']['groupdefault'] : '');
		}
		...
		if (!$this->cat_id)
		{
			...
			{
				$this->cat_id = (isset($this->userSettings['preferences']['helpdesk']['tts_category']) ? $this->userSettings['preferences']['helpdesk']['tts_category'] : '');
			}
			else
			{
		...
		{
			$jqcal = createObject('phpgwapi.jqcal');
			$jqcal->add_listener('values_finnish_date');
		}
		...
		$data = array(
			...
			'pref_send_mail' => (isset($this->userSettings['preferences']['helpdesk']['tts_user_mailnotification']) ? $this->userSettings['preferences']['helpdesk']['tts_user_mailnotification'] : ''),
	...
	function view()
	{
		...
		$flags = Settings::getInstance()->get('flags');
		Settings::getInstance()->update('flags', ['breadcrumb_selection' => $flags['menu_selection'] . '::view::' . $id]);
		...
		{
			self::redirect(array(
				'menuaction' => 'helpdesk.uiexternal_communication.edit',
				'ticket_id' => $id,
		...
		$receipt = Cache::session_get('helpdesk', 'receipt');
		Cache::session_clear('helpdesk', 'receipt');
		if (!$receipt)
		{
			$receipt = array();
		}
		...
		if (isset($values['save']))
		{
			...
			$location_id = $this->locations->get_id('helpdesk', '.ticket');
			...
			if (
				!empty($values['send_mail'])
				|| Sanitizer::get_var('set_notify_lid', 'bool')
				|| (!empty($this->bo->config->config_data['mailnotification']) && $this->bo->fields_updated)
				|| (isset($this->userSettings['preferences']['helpdesk']['tts_notify_me']) && $this->userSettings['preferences']['helpdesk']['tts_notify_me'] == 1 && $this->bo->fields_updated)
			)
			{
		...
		$location_id = $this->locations->get_id('helpdesk', $this->acl_location);
		...
		foreach ($external_messages as &$external_message)
		{
			...
			$external_message['subject_link'] = "<a href=\"" . self::link(array(
				'menuaction' => $menuaction_external_message,
		...
		$data = array(
			...
			'textareacols' => isset($this->userSettings['preferences']['helpdesk']['textareacols']) && $this->userSettings['preferences']['helpdesk']['textareacols'] ? $this->userSettings['preferences']['helpdesk']['textareacols'] : 60,
			'textarearows' => isset($this->userSettings['preferences']['helpdesk']['textarearows']) && $this->userSettings['preferences']['helpdesk']['textarearows'] ? $this->userSettings['preferences']['helpdesk']['textarearows'] : 6,
		...
		Settings::getInstance()->update('flags', ['app_header' => $function_msg . "#{$id}"]);
	...
	public function custom_ajax()
	{
		...
		if (!$custom_functions = createObject('phpgwapi.custom_functions')->find($criteria))
		{
			return false;
		}
		...
		foreach ($custom_functions as $entry)
		{
			...
			$file = PHPGW_SERVER_ROOT . "/helpdesk/inc/custom/{$this->userSettings['domain']}/{$entry['file_name']}";
	...
	private function _insert_custom_js()
	{
		...
		foreach ($custom_functions as $entry)
		{
			...
			{
				phpgwapi_js::getInstance()->add_external_file("helpdesk/inc/custom/{$this->userSettings['domain']}/{$entry['file_name']}", true);
				$js_found = true;
			}
		}
	...
	private function _get_user_list($selected, $group_id = 0, $acl_location = '')
	{
		...
		$users = $this->acl->get_user_list_right(ACL_EDIT, $acl_location, 'helpdesk', $_group_candidates);
		$user_list = array();
		...
		if ($_selected && !$selected_found)
		{
			foreach ($_selected as $__selected)
			{
				...
				$user_list[] = array(
					'id' => $selected,
					'name' => $this->accounts_obj->get($__selected)->__toString(),
					'selected' => 1
				);
			}
		}
