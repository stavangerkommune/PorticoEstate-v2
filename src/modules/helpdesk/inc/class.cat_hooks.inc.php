...
class helpdesk_cat_hooks
{
	...
	function cat_add($data)
	{
		...
		$location_obj->add("{$location}.category.{$data['cat_id']}", $data['cat_name'], 'helpdesk');
	}
	...
	function cat_delete($data)
	{
		...
		if ($data['location_id'])
		{
			...
			$location_obj->delete('helpdesk', $location, false);
		}
	}
	...
	function cat_edit($data)
	{
		...
		if ($data['location_id'])
		{
			$location_info = $location_obj->get_name($data['location_id']);
			$location = "{$location_info['location']}.category.{$data['cat_id']}";
			$location_obj->update_description($location, $data['cat_name'], 'helpdesk');
		}
	}
}
