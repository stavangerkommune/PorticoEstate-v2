...
class helpdesk_botts
{
	...
	function get_priority_list($selected = '')
	{
		...
		if (!$selected)
		{
			$selected = isset($this->userSettings['preferences']['helpdesk']['prioritydefault']) ? $this->userSettings['preferences']['helpdesk']['prioritydefault'] : $prioritylevels;
		}
	...
	function read($data = array())
	{
		...
		$selected_columns = !empty($this->userSettings['preferences']['helpdesk']['ticket_columns']) ? $this->userSettings['preferences']['helpdesk']['ticket_columns'] : array();
		...
		foreach ($tickets as &$ticket)
		{
			...
			$ticket['entry_time'] = $this->phpgwapi_common->show_date($ticket['entry_date'], 'H:i:s');
			$ticket['entry_date'] = $this->phpgwapi_common->show_date($ticket['entry_date'], $this->dateformat);
			$ticket['modified_time'] = $this->phpgwapi_common->show_date($ticket['modified_date'], 'H:i:s');
			...
			$ticket['finnish_date'] = (isset($ticket['finnish_date']) && $ticket['finnish_date'] ? $this->phpgwapi_common->show_date($ticket['finnish_date'], $this->dateformat) : '');
	...
	function read_single($id, $values = array(), $view = false)
	{
		...
		$ticket['entry_date'] = $this->phpgwapi_common->show_date($history_values[0]['datetime'], $this->dateformat);
	...
	function read_record_history($id)
	{
		...
		foreach ($history_array as $value)
		{
			...
			{
				$record_history[$i]['value_new_value']	= $this->accounts_obj->get($value['new_value'])->__toString();
				$record_history[$i]['value_old_value']	= $this->accounts_obj->get($value['old_value'])->__toString();
			}
	...
	function add($data, $values_attribute = array())
	{
		...
		{
			$data['set_user_id'] = $this->accounts_obj->name2id($data['set_user_alternative_lid']);
		}
	...
	function mail_ticket($id, $fields_updated, $receipt = array(), $get_message = false, $send_mail = false)
	{
		...
		if (!$current_prefs_user['email'])
		{
			...
			$current_prefs_user['email'] = "{$this->userSettings['account_lid']}@{$email_domain}";
		}
		...
		//message when closed;
		$link_to_ticket =  phpgw::link('/index.php', array(
		...
		if ($ticket['assignedto'])
		{
			$_assignedto = $this->accounts_obj->id2name($ticket['assignedto']);
		}
		...
		$notify_list = execMethod(
			...
			array(
				'location_id' => $location_obj->get_id('helpdesk', $this->acl_location),
				'location_item_id' => $id
			)
		);
		...
		if (!empty($ticket['attributes']))
		{
			...
			$location_id = $location_obj->get_id('helpdesk', '.ticket');
...
HTML;
		...
		$preferences = createObject('phpgwapi.preferences', $ticket['user_id']);
		if ((isset($preferences->data['helpdesk']['tts_notify_me'])
				&& ($preferences->data['helpdesk']['tts_notify_me'] == 1)
			)
		...
		$preferences->setAccountId($ticket['assignedto']);
		if ((isset($preferences->data['helpdesk']['tts_notify_me'])
				&& ($preferences->data['helpdesk']['tts_notify_me'] == 1)
			)
		...
		foreach ($members as $account_id => $account_name)
		{
			...
			$log_recipients[] = $this->accounts_obj->get($account_id)->__toString();
