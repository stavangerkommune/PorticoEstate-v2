...
class controller_uidocument extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->read = $this->acl->check('.procedure', ACL_READ, 'controller'); //1
		$this->add = $this->acl->check('.procedure', ACL_ADD, 'controller'); //2
		$this->edit = $this->acl->check('.procedure', ACL_EDIT, 'controller'); //4
		$this->delete = $this->acl->check('.procedure', ACL_DELETE, 'controller'); //8

		self::set_active_menu('admin::controller::controller_document_types');
		Settings::getInstance()->update('flags', ['app_header' => lang('controller') . ': '. lang('document types')]);
	}
	...
	public function query()
	{
		...
		array_walk($rows, array($this, 'add_actions'), array(
			$type,
	...
	public function add_actions(&$value, $key, $params)
	{
		...
		$value['actions'][] = html_entity_decode(self::link(array(
			'menuaction' => 'controller.uidocument.view',
			'id' => $value['id']
		)));
		...
		switch ($type)
		{
			case 'documents_for_procedure':
				if ($edit_permission && $editable)
				{
					$value['ajax'][] = true;
					$value['actions'][] = html_entity_decode(self::link(array(
						'menuaction' => 'controller.uidocument.delete',
						'id' => $value['id']
					)));
			...
			case 'admin':
				$value['ajax'][] = true;
				$value['actions'][] = html_entity_decode(self::link(array(
					'menuaction' => 'controller.uidocument.edit_document_type',
					'id' => $value['id']
				)));
	...
	{
		Settings::getInstance()->update('flags', ['app_header' => lang('documents') . ': ' . lang('view')]);
		...
		else
		{
			...
			foreach ($documents as $document)
			{
				...
				$doc_array['link'] = self::link(array(
					'menuaction' => 'controller.uidocument.view',
					'id' => $doc_array['id']
				));
				$doc_array['delete_link'] = self::link(array(
					'menuaction' => 'controller.uidocument.delete',
			...
			Settings::getInstance()->update('flags', ['app_header' => lang('controller') . ': ' . lang('Procedure')]);

			self::render_template_xsl(array(
				'procedure/procedure_tabs',
