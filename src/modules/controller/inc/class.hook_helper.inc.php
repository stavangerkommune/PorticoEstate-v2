...
class controller_hook_helper
{
	...
	var $_category_acl,$serverSettings,$userSettings;
	...
	public function status_componants()
	{
		...
		$applicatio_obj = new Applications();
		$app_id = $applicatio_obj->name2id('controller');
		...
		$config				 = CreateObject('phpgwapi.config', 'controller');
		$config->read();
		$limit_no_of_planned = null; //isset($this->userSettings['preferences']['controller']['no_of_planned_controls']) ? $this->userSettings['preferences']['controller']['no_of_planned_controls'] : (isset($config->config_data['no_of_planned_controls']) && $config->config_data['no_of_planned_controls'] > 0 ? $config->config_data['no_of_planned_controls'] : 5);
				...
				foreach ($controls as $my_control)
				{
					...
					foreach ($deadline_dates_for_control as $deadline_ts)
					{
						...
						if ($check_list == null)
						{
							if ($control_type == "location")
							{
								$my_assigned_controls[$deadline_ts][] = array(
									"add", $deadline_ts, $my_control,
...
JS;
	...
	public function undone_controls($front_end = false)
	{
		...
		$config					 = CreateObject('phpgwapi.config', 'controller');
		$config->read();
		$limit_no_of_planned	 = isset($this->userSettings['preferences']['controller']['no_of_planned_controls']) ? $this->userSettings['preferences']['controller']['no_of_planned_controls'] : (isset($config->config_data['no_of_planned_controls']) && $config->config_data['no_of_planned_controls'] > 0 ? $config->config_data['no_of_planned_controls'] : 5);
		$limit_no_of_assigned	 = isset($this->userSettings['preferences']['controller']['no_of_assigned_controls']) ? $this->userSettings['preferences']['controller']['no_of_assigned_controls'] : (isset($config->config_data['no_of_assigned_controls']) && $config->config_data['no_of_assigned_controls'] > 0 ? $config->config_data['no_of_assigned_controls'] : 10);
		...
		foreach ($my_controls as $container_arr)
		{
			...
			foreach ($controls as $my_control)
			{
				...
				foreach ($deadline_dates_for_control as $deadline_ts)
				{
					...
					{
						$my_undone_controls[$deadline_ts][] = array(
							"add", $deadline_ts, $my_control,
							"location", $location_code
						);
					}
		...
		foreach ($my_check_lists as $_key => $my_check_list)
		{
			$my_undone_controls[$my_check_list['deadline']][] = array(
				"edit", $my_check_list['deadline'],
...
HTML;
	...
	function get_my_assigned_check_list($from_date_ts, $to_date_ts, $repeat_type, $completed = null, $limit_no_of_planned = null)
	{
		...
		$user_id = array($this->userSettings['account_id']);

		if (!empty($this->userSettings['preferences']['property']['approval_from']))
		{
			$user_id[] = $this->userSettings['preferences']['property']['approval_from'];
		}
	...
	function get_my_controls($my_locations, $from_date_ts, $to_date_ts, $repeat_type)
	{
		...
		$role_ids = array();
		$user_ids = array($this->userSettings['account_id']);
	...
	function get_location_name($location_code)
	{
		...
		{
			$_location_info = execMethod(
				'property.bolocation.read_single',
				array(
					'location_code'	 => $location_code,
