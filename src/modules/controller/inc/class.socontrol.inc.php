...
class controller_socontrol extends controller_socommon
{
	...
	public function get_assigned_check_list_at_location($from_date, $to_date, $repeat_type, $user_id, $completed = null, $return_type = "return_object")
	{
		if (is_array($user_id))
		{
			$filter_user =	'WHERE controller_check_list.assigned_to IN ( ' . implode(',', array_map('intval', $user_id)) . ')';
		}
		else
		{
			$filter_user =	'WHERE controller_check_list.assigned_to = ' . (int)$user_id;
		}
	 ...
	 */
	public function get_assigned_check_list_by_component($from_date, $to_date, $repeat_type, $user_id, $completed = null, $return_type = "return_object", $limit_no_of_planned = null)
	{
	...
	public function get_controls_by_component($from_date, $to_date, $repeat_type, $return_type = "return_object", $role_id = 0, $filter = null)
	{
		...
		$sql = "SELECT DISTINCT c.id as control_id, c.*, ";
	...
	function get_controls_for_assigned($assigned_to)
	{
		...
		if ($assigned_to)
		{
			$assigned_to_name = $this->accounts_obj->get($assigned_to)->__toString();
		}
	...
	function get_controls_at_component($data)
	{
		...
		foreach ($controls as &$entry)
		{
			...
			{
				$users[$entry['assigned_to']] = $this->accounts_obj->get($entry['assigned_to'])->__toString();
			}
	...
	function get_controls_at_component2($data, $control_id = 0)
	{
		...
		$sql = "SELECT DISTINCT controller_control_component_list.* ,"
			...
			. " ORDER BY repeat_type DESC, repeat_interval";
		//			_debug_array($sql);
		$this->db->query($sql, __LINE__, __FILE__);
	...
	function get_checklist_at_time_and_place($part_of_town_id, $control_id, $timestamp_start, $timestamp_end)
	{
		...
		$sql = " SELECT DISTINCT controller_check_list.location_id,controller_check_list.component_id"
			...
			. " AND ( completed_date IS NULL AND planned_date >= $timestamp_start AND planned_date <= $timestamp_end "
			. " OR completed_date >= $timestamp_start AND completed_date <= $timestamp_end)";
	...
	public function get_assigned_history($data)
	{
		...
		{
			$entry['assigned_to_name'] = $this->accounts_obj->get($entry['assigned_to'])->__toString();
		}
		return $history;
	}
