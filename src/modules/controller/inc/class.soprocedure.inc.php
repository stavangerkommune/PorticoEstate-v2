...
class controller_soprocedure extends controller_socommon
{
		...
		while ($this->db->next_record())
		{
			...
			if ($this->db->f('document_id'))
			{
				...
				if ($return_type == "return_array")
				{
					...
					$doc_as_array['document_link'] = controller_uidocument::link(array(
	...
	function get_other_revisions($id)
	{
		...
		while ($this->db->next_record())
		{
			...
			$procedure->set_reference($this->unmarshal($this->db->f('reference', true), 'string'));
			$procedure->set_attachment($this->unmarshal($this->db->f('attachment', true), 'string'));
			$procedure->set_start_date($this->phpgwapi_common->show_date($this->unmarshal($this->db->f('start_date'), 'int'), $dateformat));
			//	$procedure->set_start_date(date($this->userSettings['preferences']['common']['dateformat'], $this->unmarshal($this->db->f('start_date'), 'int')));
			//	$procedure->set_end_date(date($this->userSettings['preferences']['common']['dateformat'], $this->unmarshal($this->db->f('end_date'), 'int')));
			$procedure->set_end_date($this->phpgwapi_common->show_date($this->unmarshal($this->db->f('end_date'), 'int'), $dateformat));
			$procedure->set_procedure_id($this->unmarshal($this->db->f('procedure_id'), 'int'));
			$procedure->set_revision_no($this->unmarshal($this->db->f('revision_no'), 'int'));
			//	$procedure->set_revision_date(date($this->userSettings['preferences']['common']['dateformat'], $this->unmarshal($this->db->f('revision_date'), 'int')));
			$procedure->set_revision_date($this->phpgwapi_common->show_date($this->unmarshal($this->db->f('revision_date'), 'int'), $dateformat));
			$procedure->set_control_area_id($this->unmarshal($this->db->f('control_area_id'), 'int'));
			$category = execMethod('phpgwapi.categories.return_single', $this->unmarshal($this->db->f('control_area_id'), 'int'));
