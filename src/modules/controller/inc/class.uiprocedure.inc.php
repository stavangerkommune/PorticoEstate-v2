...
class controller_uiprocedure extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['menu_selection' => 'controller::procedure']);

		$this->read = $this->acl->check('.control', ACL_READ, 'controller'); //1
		$this->add = $this->acl->check('.control', ACL_ADD, 'controller'); //2
		$this->edit = $this->acl->check('.control', ACL_EDIT, 'controller'); //4
		$this->delete = $this->acl->check('.control', ACL_DELETE, 'controller'); //8
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('controller') . ': ' . $function_msg]);
	...
	public function index()
	{
		...
		$control_areas = $cats->formatted_xslt_list(array(
		...
		$data = array(
			...
			'datatable' => array(
				'source' => self::link(array(
					'menuaction' => 'controller.uiprocedure.index',
	...
	public function edit()
	{
		...
		if (isset($_POST['save_procedure'])) // The user has pressed the save button
		{
			...
			if (isset($procedure)) // Edit procedure
			{
				...
				$procedure->set_modified_by($this->userSettings['account_id']);
		...
		else if (isset($_POST['revisit_procedure'])) // The user has pressed the revisit button
		{
			...
			if (isset($procedure)) // Edit procedure
			{
				...
				$new_procedure->set_modified_by($this->userSettings['account_id']);
		...
		else
		{
			if (self::$flash_msgs)
			{
				$msgbox_data = $this->phpgwapi_common->msgbox_data(self::$flash_msgs);
			...
			$control_areas = $cats->formatted_xslt_list(array(
				'format' => 'filter', 'selected' => $procedure->get_control_area_id(),
				'globals' => true, 'use_acl' => $this->_category_acl
			));
			array_unshift($control_areas['cat_list'], array('cat_id' => '', 'name' => lang('select value')));
			$control_areas_array2 = array();
			...
			$jqcal->add_listener('revision_date');

			$end_date = $this->phpgwapi_common->show_date($procedure->get_end_date(), $this->dateformat);
			$revision_date = $this->phpgwapi_common->show_date($procedure->get_revision_date(), $this->dateformat);

			if (!empty($procedure_array['modified_by']))
			{
				$procedure_array['modified_by_name'] = $this->accounts_obj->id2name($procedure_array['modified_by']);
			}
			...
			$data = array(
				...
				'start_date' => $this->phpgwapi_common->show_date($procedure->get_start_date() ? $procedure->get_start_date() : time(), $this->dateformat),
			...
			{
				case 'edit_procedure':
					Settings::getInstance()->update('flags', ['app_header' => lang('controller') . '::' . lang('Procedure') . '::' . lang('edit')]);
					break;
				default:
					Settings::getInstance()->update('flags', ['app_header' => lang('controller') . '::' . lang('Procedure') . '::' . lang('new revision')]);
					break;
			}
	...
	public function view()
	{
		Settings::getInstance()->update(
			'flags', ['app_header' => lang('controller') . '::' . lang('Procedure') . '::' . lang('view')]);
		...
		else
		{
			...
			//				if (!$view_revision)
			{
				...
				foreach ($revised_procedures as &$rev)
				{
					if ($rev['modified_by'])
					{
						$rev['modified_by_name'] = $this->accounts_obj->id2name($rev['modified_by']);
					}
					$rev['modified_date'] = $this->phpgwapi_common->show_date($rev['modified_date'], $this->userSettings['preferences']['common']['dateformat']);
					$rev['link'] = self::link(array(
						'menuaction' => 'controller.uiprocedure.view',
			...
			$tabs = array(
				...
				'documents' => array(
					...
					'link' => phpgw::link('/index.php', array(
						'menuaction' => 'controller.uidocument.show',
	...
	public function query()
	{
		...
		foreach ($rows as &$row)
		{
			...
			{
				$row['modified_by_name'] = $this->accounts_obj->id2name($row['modified_by']);
			}
			$row['modified_date'] = $this->phpgwapi_common->show_date($row['modified_date'], $this->dateformat);
		}
	...
	public function add_actions(&$value, $key, $params)
	{
		...
		switch ($type)
		{
			default:
				$value['ajax'][] = false;
				$value['actions'][] = html_entity_decode(self::link(array(
					'menuaction' => 'controller.uiprocedure.view',
					'id' => $value['id']
				)));
