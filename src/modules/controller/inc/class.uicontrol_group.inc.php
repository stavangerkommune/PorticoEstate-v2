...
class controller_uicontrol_group extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', [
			'app_header' => lang('controller') . ': ' . $function_msg,
			'menu_selection' => "controller::control_group"
		]);
		...
		$data = array(
			...
			'datatable' => array(
				'source' => self::link(array(
					'menuaction' => 'controller.uicontrol_group.index',
	...
	public function edit()
	{
		...
		if (isset($_POST['save_control_group'])) // The user has pressed the save button
		{
			...
			$component_location_id = $this->locations->get_id('property', ".entity.{$entity_id}.{$category_id}");
			...
			if (isset($control_group)) // Add new values to the control item
			{
				...
				if ($new_control_group)
				{
					phpgw::redirect_link('/index.php', array(
						'menuaction' => 'controller.uicontrol_group.edit',
						'id' => $ctrl_group_id
					));
				}
				else
		...
		else if (isset($_POST['remove_control_group_items']))
		{
			...
			Settings::getInstance()->update('flags', [
				'app_header' => lang('controller') . '::' . lang('Control_group')
			]);
			...
			self::render_template_xsl(array(
				'control_group/control_group_tabs', 'control_group/control_group',
	...
	{
		Settings::getInstance()->update('flags', [
			'app_header' => lang('controller') . '::' . lang('view')
		]);
		...
		else
		{
			...
			$loc_arr = $this->locations->get_name($control_group->get_component_location_id());
			$entity_arr = explode('.', $loc_arr['location']);
			...
			Settings::getInstance()->update('flags', [
				'app_header' => lang('controller') . '::' . lang('Control group')
			]);
