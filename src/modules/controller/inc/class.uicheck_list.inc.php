...
class controller_uicheck_list extends phpgwapi_uicommon_jquery
{
	...
	function __construct()
	{
		...
		$this->read = $this->acl->check('.checklist', ACL_READ, 'controller'); //1
		$this->add = $this->acl->check('.checklist', ACL_ADD, 'controller'); //2
		$this->edit = $this->acl->check('.checklist', ACL_EDIT, 'controller'); //4
		$this->delete = $this->acl->check('.checklist', ACL_DELETE, 'controller'); //8
		...
		$data = array(
			...
			'datatable' => array(
				'source' => self::link(array(
					'menuaction' => 'controller.uicheck_list.index',
	...
	function add_check_list($check_list = null)
	{
		...
		if ($type == "component")
		{
			...
			if (substr($location_info['location'], 1, 8) == 'location')
			{
				...
				$level = $type_info[2];
				$item_arr = createObject('property.solocation')->read_single('', array(
		...
		{
			$responsible_user_id = execMethod(
				'property.soresponsible.get_responsible_user_id',
				array(
		...
		$users = $this->acl->get_user_list_right(ACL_ADD, $this->acl_location);
	...
	function edit_check_list($check_list = null)
	{
		...
		$last_completed_checklist_date = !empty($last_completed_checklist['completed_date']) ? $this->phpgwapi_common->show_date($last_completed_checklist['completed_date'], $this->dateFormat) : '';
		//			_debug_array($component_children);
		...
		$jqcal2 = createObject('phpgwapi.jqcal2');
		$jqcal2->add_listener('planned_date');
		$number_of_days_in_month = cal_days_in_month(CAL_GREGORIAN, date('m', $check_list->get_deadline()), date('Y', $check_list->get_deadline())) - 1;
	...
	public function handle_multi_upload_file()
	{
		...
		$options['upload_dir'] = $this->serverSettings['files_dir'] . '/controller/' . $options['base_dir'] . '/';
	...
	public function build_multi_upload_file()
	{
		...
		Settings::getInstance()->update('flags', ['noframework' => true, 'nofooter' => true, 'xslt_app' => false]);
	...
	private function _get_component_integration($location_id, $_component_arr = array())
	{
		...
		$attributes = $custom_fields->find2($location_id, 0, '', 'ASC', 'attrib_sort', $allrows = true);
	...
	function save_check_list()
	{
		...
		if ($planned_month)
		{
			...
			$planned_date = $_planned_date->format($this->userSettings['preferences']['common']['dateformat']);
		}
		...
		if ($submit_deviation)
		{
			...
			$assigned_to = $assigned_to ? $assigned_to : $this->userSettings['account_id'];
		}
		if ($completed_date != '')
		{
		...
		if ($submit_ok)
		{
			...
			$status = controller_check_list::STATUS_DONE;
			$assigned_to = $this->userSettings['account_id'];
		}
		...
		if (!$error && $check_list->validate())
		{
			...
			)
			{
				...
				$current_prefs_user = $bocommon->create_preferences('common', $this->userSettings['account_id']);
				$from_address = "{$this->userSettings['fullname']}<{$current_prefs_user['email']}>";
				$from_name = $this->userSettings['fullname'];

				$to_name = $this->accounts_obj->id2name($assigned_to);
				...
				if ($check_list->get_component_id() > 0)
				{
					$location_info = $this->locations->get_name($check_list->get_location_id());
...
*/
	...
	private function custom_functions($check_list)
	{
		...
		foreach ($custom_functions as $entry)
		{
			...
			$file = PHPGW_SERVER_ROOT . "/controller/inc/custom/{$this->userSettings['domain']}/{$entry['file_name']}";
	...
	private function notify_supervisor($check_list)
	{
		...
		if ($check_list->get_status() == controller_check_list::STATUS_DONE)
		{
			...
			{
				$account_lid = $this->accounts_obj->get($entry['id'])->lid;
				$person_id = $this->accounts_obj->get($entry['id'])->person_id;
			...
			if ($toarray)
			{
				...
				{
					$from_address = "NoReply@{$this->serverSettings['hostname']}";
				}

				$config_frontend = CreateObject('phpgwapi.config', 'mobilefrontend')->read();

				$enforce_ssl = $this->serverSettings['enforce_ssl'];
				$this->serverSettings['enforce_ssl'] = !!$config_frontend['backend_ssl'];
...
HTML;
	...
	function sendIcalEvent($from_name, $from_address, $to_name, $to_address, $startTime, $endTime, $subject, $html_description, $text_description, $location)
	{
		...
		$domain = $this->serverSettings['hostname'];
...
HTML;
	...
	function get_report_()
	{
		...
		$data = array(
			array(
				'col1' => "<b>Rapportnummer:</b>\n" . $check_list_id
					. "\n<b>Kontrolldato:</b>\n" . $this->phpgwapi_common->show_date($completed_date, $dateformat),
		...
		foreach ($_component_children as &$component_child)
		{
			...
			$system_location = $this->locations->get_name($component_child['location_id']);
			$system_location_arr = explode('.', $system_location['location']);
			$category_dir = "{$system_location_arr[1]}_{$system_location_arr[2]}_{$system_location_arr[3]}";
			...
			if (!empty($completed_items[$component_child['location_id']][$component_child['id']]))
			{
				$entry = array(
					...
					'col3' => 'kontrollert: ' . $this->phpgwapi_common->show_date($completed_items[$component_child['location_id']][$component_child['id']]['completed_ts'], $this->dateFormat)
				);
			}
			else
			{
	...
	function send_report()
	{
		...
		$subject	 = 'Avviksrapport::' . $this->serverSettings['site_title'];
		$body		 = 'Sjå vedlegg';
		$from_email	 = isset($config['email_sender']) && $config['email_sender'] ? $config['email_sender'] : "noreply<noreply@{$this->serverSettings['hostname']}>";
		$from_name	 = $this->userSettings['fullname'];
	...
	function get_report($check_list_id = null, $return_as_file = null)
	{
		...
		$report_data['completed_date'] = $this->phpgwapi_common->show_date($completed_date, $dateformat);
		...
		foreach ($report_info['open_check_items_and_cases'] as $check_item)
		{
			...
			foreach ($check_item->get_cases_array() as $case)
			{
				...
				$entry[] = array(
					'text' => $lang_date,
					'value' => $this->phpgwapi_common->show_date($case->get_entry_date(), $this->dateFormat)
				);
		...
		foreach ($_component_children as &$component_child)
		{
			...
			if (!empty($completed_items[$component_child['location_id']][$component_child['id']]))
			{
				...
				$controlled_text = 'kontrollert: ' . $this->phpgwapi_common->show_date($_completed_date, $this->dateFormat);
			}
		...
		foreach ($component_child_data as $data_set)
		{
			if ($section_location_id != $data_set['location_id'])
			{
				...
				$system_location = $this->locations->get_name($section_location_id);
				$section_id++;
			}
	...
	private function _get_report_info($check_list_id,  $case_location_code = '')
	{
		...
		foreach ($open_check_items_and_cases as $key => $check_item)
		{
			...
			foreach ($check_item->get_cases_array() as $case)
			{
				...
				if ($component_id)
				{
					$location_info = $this->locations->get_name($component_location_id);
