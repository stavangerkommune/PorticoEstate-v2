...
class controller_cat_hooks
{
	...
	function cat_add($data)
	{
		...
		$this->locations_obj->add("{$location}.category.{$data['cat_id']}", $data['cat_name'], 'controller');
	...
	function cat_delete($data)
	{
		...
		if ($data['location_id'])
		{
			...
			$this->locations_obj->delete('controller', $location, false);
	...
	function cat_edit($data)
	{
		...
		if ($data['location_id'])
		{
			$location_info = $this->locations_obj->get_name($data['location_id']);
			$location = "{$location_info['location']}.category.{$data['cat_id']}";
			$this->locations_obj->update_description($location, $data['cat_name'], 'controller');
