...
class controller_uicalendar_planner extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$manage = $this->acl->check('.control', 16, 'controller'); //16
	...
	public function index()
	{
		...
		for ($i = 1; $i <= 6; $i++)
		{
			$first_half_year[] = array(
				...
				'url'	 => self::link(array(
					'menuaction' => 'controller.uicalendar_planner.monthly',
					'year'		 => $current_year,
					'month'		 => $i
				))
			);
		...
		$entity_groups = createObject('property.bogeneric')->get_list(array(
			'type'		 => 'entity_group',
			'selected'	 => $entity_group_id, 'order'		 => 'name', 'sort'		 => 'asc'
		));

		$part_of_towns = createObject('property.bogeneric')->get_list(
			array(
				'type'		 => 'part_of_town',
		...
		Settings::getInstance()->update('flags', ['app_header' => 'controller::calendar_planner']);
			...
			if (!empty($items[$date_string]))
			{
				...
				foreach ($items[$date_string] as $item)
				{
					...
					$item_content = array(
						...
						'deadline_date'		 => $this->phpgwapi_common->show_date($item['schedule']['info']['deadline_date_ts'], $this->userSettings['preferences']['common']['dateformat']),
						'planned_date'		 => $this->phpgwapi_common->show_date($item['schedule']['info']['planned_date_ts'], $this->userSettings['preferences']['common']['dateformat']),
	...
	public function monthly()
	{
		...
		$data = array(
			...
			'current_year'	 => $year,
			'next_month_url' => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.monthly',
				'year' => $year,
				'month' => ($month + 1),
				'part_of_town_id' => $part_of_town_id,
			...
			'prev_month_url' => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.monthly',
				'year' => $year,
				'month' => ($month - 1),
				'part_of_town_id' => $part_of_town_id,
			...
			'start_url' => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.index',
				'current_year' => $year,
				//		'month' => $month,
				//		'part_of_town_id' => $part_of_town_id,
				'control_id' => $control_id,
			...
			'send_notification_url'  => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.send_notification',
				'current_year' => $year,
				'month' => $month,
				'part_of_town_id' => $part_of_town_id,
				'control_id' => $control_id,
		...
		Settings::getInstance()->update('flags', ['breadcrumb_selection' => 'controller::calendar_planner::monthly',
		 'app_header' => lang('calendar planner') . '::' . lang('monthly')]);
	...
	public function send_notification()
	{
		...
		foreach ($components as $component)
		{
			...
			$date = $this->phpgwapi_common->show_date($timestamp, $this->userSettings['preferences']['common']['dateformat']);
		...
		$data = array(
			...
			'form_action'  => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.save_send_notification',
				'current_year' => $year,
				'month' => $month,
				'part_of_town_id' => $part_of_town_id,
				'control_id' => $control_id,
		...
		Settings::getInstance()->update('flags', ['breadcrumb_selection' => 'controller::calendar_planner::send_notification',
		 'app_header' => lang('calendar planner') . '::' . lang('send notification')]);
...
HTML;
		...
		foreach ($send_email as $item => $value)
		{
			...
			if ($email[$item])
			{
				...
				$html = str_replace(array('__location__', '__week__', '__sender__'), array($location_name, $week, $this->userSettings['fullname']), $html);
	...
	public function inspection_history()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('calendar planner') . '::' . lang('inspection history')]);
...
HTML;
	...
	public function start_inspection()
	{
		...
		$jqcal2 = createObject('phpgwapi.jqcal2');
		$jqcal2->add_listener('current_day_str', 'date', $current_day->getTimestamp(), array('disabled_dates' => $disabled_dates));

		$data = array(
			'current_day_str' => $current_day->format('Y-m-d'),
			'current_day' => $current_day->getTimestamp(),
			'img_green_check' => $this->phpgwapi_common->image('phpgwapi', 'green-check'),
			'control_area_list'	 => array('options' => $control_area_list),
			'entity_group_list'	 => array('options' => $entity_groups),
			'part_of_town_list'	 => array('options' => $part_of_town_list),
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('calendar planner') . '::' . lang('start inspection'),
			'breadcrumb_selection' => 'controller::calendar_planner::start_inspection']);
	...
	public function ad_hoc()
	{
		...
		$data = array(
			...
			'current_year'	 => $year,
			'next_month_url' => self::link(array(
				'menuaction' => 'controller.uicalendar_planner.ad_hoc',
				'year' => $year,
				'month' => ($month + 1),
			)),
			'prev_month_url' => self::link(array(
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('calendar planner') . '::' . lang('ad hoc'),
				'breadcrumb_selection' => 'controller::calendar_planner::ad_hoc']
		);
