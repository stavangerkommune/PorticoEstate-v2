...
class controller_uicase extends phpgwapi_uicommon_jquery
{
	...
	function edit_component_child($edit_parent = false)
	{
		...
		if ($get_form || $get_edit_form || $get_info)
		{
			...
			if ($get_info)
			{
				...
				{
					$open_case['modified_date_text'] = $this->phpgwapi_common->show_date($open_case['modified_date'], $this->dateFormat);
		...
		if ($_POST && !$parent_location_id && !$parent_component_id && $component_id)
		{
			...
			$parent_location_id = $this->locations->get_id('property', ".entity.{$values['p_entity_id']}.{$values['p_cat_id']}");
		}
		...
		if ($_POST && (($parent_location_id && $parent_component_id) || $edit_parent))
		{
			...
			$parent_system_location = $this->locations->get_name($parent_location_id);
		...
		foreach ($saved_control_groups as $control_group)
		{
			...
			if (count($saved_control_items) > 0)
			{
				...
				if ($component_location_id && $type == 'location')
				{
					...
					//	$loc_arr = $this->locations->get_name($component_location_id);
	...
	function add_case()
	{
		...
		$data = array(
			...
			'img_add2' => $this->phpgwapi_common->image('phpgwapi', 'add2'),
			'img_undo' => $this->phpgwapi_common->image('phpgwapi', 'undo-4-512'),
			...
			'delete_img' => $this->phpgwapi_common->image('phpgwapi', 'recycle-bin-line')
...
JS;
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('controller') . '::' . $check_list->get_id(),
			'breadcrumb_selection' => 'controller::add_case' . '::' . $check_list->get_id()]);
		self::set_active_menu('controller::add_case');

		self::render_template_xsl(array(
			'check_list/fragments/check_list_menu', 'check_list/fragments/nav_control_plan',
	...
	function create_case_message()
	{
		...
		$categories = $catsObj->formatted_xslt_list(array(
	...
	function send_case_message()
	{
		...
		self::redirect(array(
			'menuaction' => 'controller.uicase.view_case_message', 'check_list_id' => $check_list_id,
	...
	function send_case_message_step_2($check_list_id, $location_code, $message_title, $message_cat_id, $case_ids)
	{
		...
		$location_id = $this->locations->get_id("controller", ".checklist");
		...
		$location_id_ticket = $this->locations->get_id('property', '.ticket');
		$user_id = $this->userSettings['id'];
	...
	function view_closed_cases()
	{
		...
		self::render_template_xsl(array(
			'check_list/fragments/check_list_menu', 'case/cases_tab_menu',
