...
class controller_uibulk_update extends phpgwapi_uicommon
{
	...
	function __construct()
	{
		...
		$this->acl_read = $this->acl->check('.checklist', ACL_READ, 'controller'); //1
		$this->acl_add = $this->acl->check('.checklist', ACL_ADD, 'controller'); //2
		$this->acl_edit = $this->acl->check('.checklist', ACL_EDIT, 'controller'); //4
		$this->acl_delete = $this->acl->check('.checklist', ACL_DELETE, 'controller'); //8
	...
	function assign()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('controller') . ' :: ' . lang('bulk update assigned')]);
	...
	public function get_future_checklist()
	{
		...
		if ($assigned_to)
		{
			...
			$assigned_to_name = $accounts_obj->get($assigned_to)->__toString();
		}
		$soentity = createObject('property.soentity');
		...
		foreach ($check_lists as $check_list)
		{
			...
			$_check_list[] = array(
				...
				'deadline' => $this->phpgwapi_common->show_date($check_list['deadline'], $this->userSettings['preferences']['common']['dateformat']),
				'planned_date' => $this->phpgwapi_common->show_date($check_list['planned_date'], $this->userSettings['preferences']['common']['dateformat']),
