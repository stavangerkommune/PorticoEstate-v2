...
class controller_uicomponent extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->manage = $this->acl->check('.control', 16, 'controller'); //16
		...
		{
			Settings::getInstance()->update('flags', ['app_header' => lang('controller') . '::' . lang('status locations')]);
		}
		else
		{
			Settings::getInstance()->update('flags', ['app_header' => lang('controller') . '::' . lang('status components')]);
		...
		if (empty($location_filter))
		{
			...
			if ($this->is_location($location_id) || $get_locations)
			{
				...
				foreach ($location_types as $location_type)
				{
					...
					{
						$location_filter[] = array(
							'id' => $this->locations->get_id('property', ".location.{$location_type['id']}"),
	...
	public function index()
	{
		...
		$default_value = array(
			...
			'selected' => !empty($this->flags['custom_frontend']) && $_my_negative_self == ($this->account * -1)
		);
		...
		$part_of_town_list = $property_bocommon->select_part_of_town(
			'filter',
		...
		$default_filter_month = !empty($this->flags['custom_frontend']) ? date('n') : 0;
		...
		$status_list = array(
			array(
				'id'	 => 'all',
				'name'	 => lang('All'),
				//				'selected' => empty($this->flags['custom_frontend']) ? 1 : false
			),
			array(
				'id'	 => 'in_queue',
				'name'	 => lang('in queue'),
				...
				'toolbar' => array(
					'item' => array(
						...
						array(
							...
							'list' => execMethod('property.bogeneric.get_list', array(
								'type' => 'entity_group',
						...
						array(
							...
							'text' => '',
							'value' => isset($this->flags['custom_frontend']) && $this->flags['custom_frontend'] ? 1 : 0
						)
					)
				)
			),
	...
	private function get_months($year = '', $filter_month = 0)
	{
		...
		else if ($year == date('Y') && $this->user_id < 0 && !empty($this->flags['custom_frontend']))
		{
	...
	public function query()
	{
		...
		else
		{
			...
			{
				$_items = execMethod(
					'property.soentity.read_entity_group',
					array(
		...
		if ($this->serverSettings['template_set'] == 'mobilefrontend')
		{
			//				$url_target = '_self';
			$url_target = '_blank';
		}
	...
	private function translate_calendar_info($param, $year, $month, $filter_status, &$found_at_least_one, $keep_only_assigned_to, $location_code, &$control_link_data, $url_target)
	{
		...
		//	$responsible = '---';
		$assigned_to = $param['info']['assigned_to'] > 0 ? $accounts_obj->id2name($param['info']['assigned_to']) : '&nbsp;<br/>';
