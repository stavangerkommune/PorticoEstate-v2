...
<xsl:template match="inspection_history"
	xmlns:php="http://php.net/xsl">
	<style>
		.table-hover-cells > tbody > tr > th:hover,
		.table-hover-cells > tbody > tr > td:hover {
	...
	<form method="post" id="form" action="{form_action}">
		...
		<div class="row">
			<div class="mt-5 container">
				<h2 class="text-center">Utførte rapporter siden <xsl:value-of select="limit_date"/>
				</h2>
				<div class="row">
...
<xsl:template match="monthly"
	...
	<div class="mt-5 container">
		...
		<div class="row">
			<div class="col">
				<p style="font-size: 14px">I <xsl:value-of select="current_month"/>
 er det satt opp befaring i <xsl:value-of select="part_of_town"/>
:
			</p>
		</div>
		<!--			<div class="col">
...
<xsl:template match="start_inspection"
xmlns:php="http://php.net/xsl">
<xsl:variable name="date_format">
	<xsl:value-of select="php:function('get_phpgw_info', 'user|preferences|common|dateformat')" />
</xsl:variable>
