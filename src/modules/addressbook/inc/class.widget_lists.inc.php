...
class addressbook_widget_lists
{
	...
	var $use_combos, $template, $use_filter, $left_combo_title, $left_combo,
		...
		$filter_option_list, $old_option_list, $left_combo_link_opt, $left_combo_name, $phpgwapi_common;
	...
	function get_widget()
	{
		$this->template->set_root($this->phpgwapi_common->get_tpl_dir('addressbook'));
		...
		$this->template->set_var('th_text', $theme['th_text']);
		$this->template->set_var('row_on',  $theme['row_on']);
		$this->template->set_var('row_off', $theme['row_off']);
		$this->template->set_var('row_text', $theme['row_text']);
