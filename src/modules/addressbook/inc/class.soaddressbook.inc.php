...
class addressbook_soaddressbook
{
	...
	function __construct($useacl = true)
	{
		...
		{
			$acl = Acl::getInstance();
			$acl->set_account_id($this->userSettings['account_id']);
			$this->grants = $acl->get_grants('addressbook', '.');
		}
	...
	function edit_person($fields)
	{
		...
		{
			$this->contacts->add_orgs_for_person(
				$orgs['insert'],
				$orgs['preferred_org'],
	...
	function add_person($fields)
	{
		...
		foreach ($fields['comm_data'] as $type_descr => $data)
		{
			...
			{
				$comms[] = array(
					'comm_descr' => $this->contacts->search_comm_descr($type_descr),
					'comm_data' 		  => $data,
	...
	function add_contact_with_email($name, $email)
	{
		...
		$comms[] = array(
			'comm_descr' 		=> $this->search_comm_descr('work email'),
			'comm_data' 		=> $email,
			'comm_preferred' 	=> 'Y'
		);
	...
	function upgrade_comms($add_comms = array(), $del_comms = array(), $edit_comms = array(), $data = array(), $comm_preferred = '', $contact_id = '')
	{
		...
		foreach ($data as $key => $value)
		{
			...
			if (array_key_exists($value['comm_description'], $edit_comms))
			{
				...
				$this->edit_comms(
					$value['key_comm_id'],
					array(
						'comm_data' => $edit_comms[$value['comm_description']],
		...
		foreach ($add_comms as $key => $value)
		{
			...
			$fields = array(
				'comm_descr' => $this->search_comm_descr($key),
				'comm_data' => $value,
				'comm_preferred' => $pref
			);
	...
	{
		$prefs = createObject('phpgwapi.preferences')->read();
