...
function cat_option($cat_id = '', $notall = False, $java = True, $multiple = False)
{
	...
	$cats_link .= $categories->formated_list('select', 'all', $cat_id, True);
	$cats_link .= '</select>' . "\n";
	return $cats_link;
}
...
function read_custom_fields()
{
	...
	@reset($userSettings['preferences']['addressbook']);
	//while (list($col,$descr) = @each($userSettings['preferences']['addressbook']))
...
{
	$userSettings = Settings::getInstance()->get('user');
	$preferences = createObject('phpgwapi.preferences', $userSettings['account_id']);

	if ($old)
	{
		$preferences->delete("addressbook", "extra_" . $old);
	}
	if ($new)
	{
		$preferences->add("addressbook", "extra_" . $new);
	}
	$preferences->save_repository(1);
}
