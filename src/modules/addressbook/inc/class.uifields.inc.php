...
class addressbook_uifields extends phpgwapi_uicommon
{
	...
	function index()
	{
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => "{$this->currentapp}.uifields.index",
				'phpgw_return_as' => 'json'
			))),
	...
	{
		if (
			isset($this->per_custom_fields) && (is_array($this->per_custom_fields) && isset($this->org_custom_fields) && is_array($this->org_custom_fields)) &&
	...
	function save_custom_field($old = '', $new = '', $apply_for = '')
	{
		...
		if ($edit_contacts)
		{
			$owner = isset($this->serverSettings['addressmaster']) ? $this->serverSettings['addressmaster'] : 0;
