...
class addressbook_uiaddressbook_persons extends phpgwapi_uicommon
{
	...
	function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('addressbook') . '::' . lang('Persons')]);
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		else
		{
			...
			$person_data['owner_name'] = $this->accounts_obj->id2name($person_data['owner']);
		}
		...
		$jqcal->add_listener('per_birthday', 'date', '', $config_calendar);
		...
		if ($mode == 'edit' && $id)
		{
			...
			$datatable_def[] = array(
				...
				'requestUrl' => json_encode(self::link(array(
					'menuaction' => "{$this->currentapp}.uiaddressbook_persons.get_others_data",
					'contact_id' => $id,
					'phpgw_return_as' => 'json'
				))),
