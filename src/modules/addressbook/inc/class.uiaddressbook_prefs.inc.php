...
class addressbook_uiaddressbook_prefs
{
	...
	*/
	...
	function __construct()
	{
		...
		{
			$this->map_tpl_to_real['lang_select_cols'] = lang('Fields to show in address list');
			$this->map_tpl_to_real['lang_abprefs'] = lang('addressbook preferences');
			$this->map_tpl_to_real['select_columns_submit_value'] = lang('select fields');
			$this->map_tpl_to_real['submit_save_value'] = lang('save');
			$this->map_tpl_to_real['submit_cancel_value'] = lang('cancel');
			$this->map_tpl_to_real['submit_remove_value'] = lang('remove');
		}
		else
		{
		...
		$this->map_tpl_to_real['org_preferences_link'] = phpgw::link(
			'/index.php',
			array(
		...
		$this->map_tpl_to_real['person_preferences_link'] = phpgw::link(
			'/index.php',
			array(
				'menuaction' => 'addressbook.uiaddressbook_prefs.index',
			...
			foreach ($this->contacts->stock_contact_fields as $falsefield => $english)
			{
				...
				if (!isset($this->selected_fields[$this->org_or_person][$falsefield]) || !$this->selected_fields[$this->org_or_person][$falsefield])
				{
					if ($this->org_or_person == 'Persons')
					{
						...
						{
							$this->fields_show_selectbox[$falsefield] = lang($this->contacts->stock_contact_fields[$falsefield]);
				...
				else
				{
					...
					$this->lang_fields[$falsefield] = lang($this->contacts->stock_contact_fields[$falsefield]);
				}
	...
	*/
	...
	function index()
	{
		...
		if ((is_array($this->selected_fields[$this->org_or_person])) || (is_array($this->cached_selected_fields[$this->org_or_person])) && !$this->cache_is_empty())
		{
	...
	function set_static_vars()
	{
		...
		$this->template->set_var('lang_remove_field', lang('remove'));
		$theme = Settings::getInstance()->get('theme');
		$this->template->set_var('th_bg', $theme['th_bg']);
		$this->template->set_var('font', $theme['font']);
		//Catergories
	...
	function show_selectbox($org_or_person = '')
	{
		...
		if (isset($this->fields_show_selectbox['comm_types']))
		{
			...
			{
				$this->template->set_var('lang_comtype_field', lang('Empty'));
		...
		{
			$this->template->set_var('lang_contact_field', lang('Empty'));
			$this->template->set_var('value', '');
			$this->template->parse('V_select_columns_form_options', 'B_select_columns_form_options', True);
		}
	}
	...
	function save_sessiondata($clear = false)
	{
		if (!$clear)
		{
			...
			Cache::session_set('addressbookpref', 'session_data', array(
		...
		else
		{
			...
			Cache::session_clear('addressbookpref', 'session_data');
		}
	}
	function read_sessiondata()
	{
		//		print '<br /><B>READING SESSIONDATA!</B><br />';
		$data =	Cache::session_get('addressbookpref', 'session_data');
	...
	function is_current_admin()
	{
		...
		//$acl->check('run', 1, 'admin') . "</B><br />";
		
		return $acl->check('run', 1, 'admin');
	}
	...
	function build_admin_tabs()
	{
		if ($this->is_current_admin())
		{
			...
			$this->map_tpl_to_real['final_admin_tabs'] = $this->phpgwapi_common->create_tabs($tabs, $selected);
		}
	}
}
