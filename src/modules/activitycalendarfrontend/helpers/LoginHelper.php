...
class LoginHelper
{
	public static function post_login()
	{
		...
		if (self::login())
		{
			...
			$redirect =	json_decode(Cache::session_get('activitycalendarfrontend', 'redirect'), true);
			...
			if (is_array($redirect) && count($redirect))
			{
				...
				Cache::session_clear('activitycalendarfrontend', 'redirect');
				\phpgw::redirect_link('/activitycalendarfrontend/', $redirect_data);
			}
			...
			\phpgw::redirect_link('/activitycalendarfrontend/', $after);
			exit;
		}
	}
	...
	private static function login()
	{
		...
		{
			$config = (new Config('activitycalendarfrontend'))->read();
