...
class hrm_bojob
{
	...
	var $serverSettings, $userSettings, $phpgwapi_common;
	...
	function save_sessiondata($data)
	{
		if ($this->use_session)
		{
			Cache::session_set('phpgw_hrm_job', 'session_data', $data);
		}
	}

	function read_sessiondata()
	{
		$data = Cache::session_get('phpgw_hrm_job', 'session_data');
	...
	{
		$qualification_list = $this->so->read_qualification(array(
			'job_id' => $job_id,
			'start' => $this->start,
			'query' => $this->query,
			'sort' => $this->sort,
	...
	{
		$qualification_list = $this->so->read_qualification_type(array(
			'start' => $this->start,
			'query' => $this->query,
			'sort' => $this->sort,
			'order' => $this->order,
	...
	{
		$task_list = $this->so->read_task(array(
			'job_id' => $job_id,
			'start' => $this->start,
			'query' => $this->query,
			'sort' => $this->sort,
