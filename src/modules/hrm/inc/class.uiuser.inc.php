...
class hrm_uiuser
{
	...
	var $nextmatchs, $account, $bo, $bocommon, $allrows, $bocategory, $phpgwapi_common, $userSettings;
	...
	function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['menu_selection' => 'hrm::user']);
	...
	function training()
	{
		...
		$receipt = Cache::session_get('hrm_training_receipt', 'session_data');
		Cache::session_clear('hrm_training_receipt', 'session_data');
	...
	function edit()
	{
		...
		$jqcal2 = CreateObject('phpgwapi.jqcal2');	// before phpgw_header() !!!
		$jqcal2->add_listener('values_start_date');
		$jqcal2->add_listener('values_end_date');
...
*/
		...
		$this->_print_pdf($document, 'CV_' . $accounts_obj->id2name($user_id));
