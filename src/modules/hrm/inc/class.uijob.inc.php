...
class hrm_uijob
{
	...
	var $nextmatchs, $account, $bo, $bocommon, $allrows, $bocategory, $acl, $userSettings, $phpgwapi_common;
	...
	public function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['menu_selection' => 'hrm::job']);
	}
	...
	function index()
	{
		...
		Settings::getInstance()->update('flags', ['menu_selection' => 'hrm::job::job_type']);
		...
		$receipt = Cache::session_get('hrm_job_receipt', 'session_data');
		Cache::session_clear('hrm_job_receipt', 'session_data');
		...
		$data = array(
			...
			'img_check'						=> $this->phpgwapi_common->get_image_path('hrm') . '/check.png',
			'lang_print'					=> lang('print'),
	...
	//	$pdf->ezStream(['compress' => 0]);
		...
		$this->_print_pdf($document, 'job');
	}

	function _print_pdf($document, $document_name = 'document')
	{
	...
	function qualification()
	{
		...
		$receipt = Cache::session_get('hrm_quali_receipt', 'session_data');
		Cache::session_clear('hrm_quali_receipt', 'session_data');
	...
	function task()
	{
		...
		$receipt = Cache::session_get('hrm_task_receipt', 'session_data');
		Cache::session_clear('hrm_task_receipt', 'session_data');
		...
		if (is_array($values))
		{
			...
			if ($values['save'] || $values['apply'])
			{
				...
				if (!$receipt['error'])
				{
					...
					if ($values['save'])
					{
						Cache::session_set('hrm_task_receipt', 'session_data', $receipt);
	...
	function lookup_qualification()
	{
		...
		$receipt = Cache::session_get('hrm_quali_tp_receipt', 'session_data');
		Cache::session_clear('hrm_quali_tp_receipt', 'session_data');
...
JS;
		...
		if (is_array($values))
		{
			if ($values['save'] || $values['apply'])
			{
				...
				if (!$receipt['error'])
				{
					...
					if ($values['save'])
					{
						Cache::session_set('hrm_quali_tp_receipt', 'session_data', $receipt);
		...
		if (is_array($values))
		{
			if ($values['save'] || $values['apply'])
			{
				...
				if (!isset($receipt['error']) || !$receipt['error'])
				{
					...
					if ($values['save'])
					{
						Cache::session_set('hrm_job_receipt', 'session_data', $receipt);
	...
	function edit_qualification()
	{
		...
		if (is_array($values))
		{
			...
			if ($values['save'] || $values['apply'])
			{
				...
				if (!$receipt['error'])
				{
					...
					if ($values['save'])
					{
						Cache::session_set('hrm_quali_receipt', 'session_data', $receipt);
		...
		$url = phpgw::link('/', $link_data_lookup);
		$code =  <<<JS
		self.name="first_Window";
		function qualifications_popup()
		{
			Window1=window.open("{$url}","Search","width=800,height=600,toolbar=no,scrollbars=yes,resizable=yes");
		}
JS;
	...
	function hierarchy()
	{
		Settings::getInstance()->update('flags', ['menu_selection' => 'hrm::job::organisation']);
