...
class manual_uidocuments extends phpgwapi_uicommon_jquery
{
	...
	public function add()
	{
		self::set_active_menu('manual::add');
		$this->edit();
	}
	...
	public function edit($cat_id = 0, $mode = 'edit')
	{
		...
		if ($mode == 'view')
		{
			self::set_active_menu('manual::view');
			if (!$this->acl_read)
			{
				phpgw::no_access();
			}
		...
		{
			$file_def[1] = array(
				'key' => 'delete_file', 'label' => lang('Delete file'),
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => 'manual.uidocuments.get_files',
		...
		Settings::getInstance()-> update('flags', ['app_header' => lang('manual') . '::' . lang('documents')]);
	...
	public function save()
	{
		...
		else
		{
			...
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'manual.uidocuments.edit',
				'cat_id' => $cat_id
			));
		}
	}
