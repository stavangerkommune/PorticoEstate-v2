...
class calendar_bocalendar
{
	...
	public function __construct($session = false)
	{
		...
		$this->grants = Acl::getInstance()->get_grants('calendar', '.');
		...
		$this->prefs['calendar']    = $this->userSettings['preferences']['calendar'];
		...
		{
			$this->owner = $this->contacts->is_contact($this->userSettings['account_id']);
		}

		$this->prefs['common'] = $this->userSettings['preferences']['common'];
	...
	function set_owner_to_group($owner)
	{
		...
		$this->g_owner = array();
		$group_owners = $this->accounts_obj->member($owner);
	...
	{
		$owner = ($owner == 0 ? $this->userSettings['account_id'] : $owner);
		$group_owners = $this->accounts_obj->membership();
	...
	function save_sessiondata($data = '')
	{
		if ($this->use_session)
		{
			...
			Cache::session_set('calendar', 'session_data', $data);
		}
	}
	...
	{
		if ($this->userSettings['apps']['admin'])
		{
	...
	function change_owner($params = '')
	{
		if ($GLOBALS['phpgw_info']['server']['calendar_type'] == 'sql')
		{
			if (is_array($params))
			{
				$this->so->change_owner(
					$this->contacts->is_contact($params['old_owner']),
	...
	function search_keywords($keywords)
	{
		if ($this->is_group)
		{
			$members = Acl::getInstance()->get_ids_for_location($this->owner, 1, 'phpgw_group');
		}
		else
		{
		...
		else
		{
			...
			if (is_array($l_participants) && count($l_participants))
			{
				...
				foreach ($l_participants as $participant)
				{
					...
					{
						$members = $this->accounts_obj->member(substr($participant, 2));
			...
			if (
				$l_cal['alarmdays'] > 0 || $l_cal['alarmhours'] > 0 ||
	...
	{
		$uim = False;
		$security_equals = $this->accounts_obj->membership($this->accounts_obj->search_person($contact));
		...
		{
			if ($this->accounts_obj->get_type($participant) == 'g')
			{
	...
	function validate_update($event)
	{
		...
		else
		{
			...
			foreach ($event['participants'] as $participant => $status)
			{
				if ($this->accounts_obj->search_person($participant))
				{
					$acct_part = true;
					break;
				}
		...
		if (
			!phpgwapi_datetime::date_valid($event['start']['year'], $event['start']['month'], $event['start']['mday']) || !phpgwapi_datetime::date_valid($event['end']['year'], $event['end']['month'], $event['end']['mday'])
	...
	function participants_not_rejected($participants, $event)
	{
		...
		foreach ($participants as $uid => $status)
		{
			...
			if (
				isset($event['participants'][$uid]) && $event['participants'][$uid] != 'R' &&
				$status != 'R'
			)
			{
	...
	function overlap($starttime, $endtime, $participants, $owner = 0, $id = 0, $restore_cache = False)
	{
		...
		//			$temp_start = intval($this->phpgwapi_common->show_date($starttime,'Ymd'));
		//			$temp_start_time = intval($this->phpgwapi_common->show_date($starttime,'Hi'));
		//			$temp_end = intval($this->phpgwapi_common->show_date($endtime,'Ymd'));
		//			$temp_end_time = intval($this->phpgwapi_common->show_date($endtime,'Hi'));
	...
	function check_perms($needed, $event = 0, $other = 0)
	{
		...
		if ($event === 0)
		{
			...
			$owner =  $this->accounts_obj->search_person(
				$other > 0
					? $other
					: $this->owner
			);
		...
		else
		{
			...
			$owner = $this->accounts_obj->search_person($event['owner']);
		...
		if (is_array($event) && is_array($event['participants']) && ($needed == ACL_READ))
		{
			...
			/*				this logic must be wrong - gets mapped to the wrong user - ask Dave				
				...
				{
					if (isset($this->grants[$this->accounts_obj->search_person($contact_id)])
						&& ($this->grants[$this->accounts_obj->search_person($contact_id)] & ACL_READ) 
						|| $this->accounts_obj->search_person($contact_id) == $user)
					{
						$grants |= ACL_READ;
						break;
					}
...
*/
	...
	function is_private($event, $owner)
	{
		...
		if ($event['public'] == 1 || ($this->check_perms(ACL_PRIVATE, $event) && $event['public'] == 0) || $event['owner'] == $this->userSettings['person_id'])
		{
			return False;
		}
		...
		{
			$is_private = True;
			$groups = $this->accounts_obj->membership($owner);
	...
	function store_to_cache($params)
	{
			...
			for ($i = 0; $i < $c_cached_ids; $i++)
			{
				...
				if ($startdate != $enddate)
				{
					...
					for ($j = $startdate, $k = 0; $j <= $enddate; $k++, $j = intval(date('Ymd', mktime(0, 0, 0, $start['month'], $start['mday'] + $k, $start['year']))))
					{
						...
						if (
							!isset($this->cached_events[$j][$c_evt_day])
		...
		if ($c_cached_ids_repeating)
		{
			...
			for ($date = mktime(0, 0, 0, $smonth, $sday, $syear); $date <= $edate; $date += phpgwapi_datetime::SECONDS_IN_DAY)
			{
				if ($this->debug)
				{
					//						$search_date = $this->phpgwapi_common->show_date($date,'Ymd');
	...
	function prepare_matrix($interval, $increment, $part, $fulldate)
	{
		...
		foreach ($this->cached_events[$fulldate] as $event)
		{
			...
			for ($h = $starttemp['hour']; $h <= $endtemp['hour']; $h++)
			{
				...
				$time_display = $this->phpgwapi_common->show_date($eventstart['raw'], $this->users_timeformat) . '-' . $this->phpgwapi_common->show_date($eventend['raw'], $this->users_timeformat);
	...
	function update_requested($userid, $part_prefs, $msg_type, $old_event, $new_event)
	{
		...
		switch ($ru = $part_prefs['calendar']['receive_updates'])
		{
			...
			case 'time_change':
				$diff = max(
					abs($this->maketime($old_event['start']) - $this->maketime($new_event['start'])),
			...
			case 'add_cancel':
				if (
					$old_event['owner'] == $userid && $msg_is_response ||
	...
	function send_update($msg_type, $to_notify, $old_event, $new_event = False, $user = False)
	{
		...
		foreach ($to_notify as $userid => $statusid)
		{
			...
			if ($userid != $this->userSettings['account_id'] ||  $msg_type == MSG_ALARM)
			{
				...
				$details['to-fullname'] = (string) $this->accounts_obj->get($userid);
				...
				$this->userSettings['preferences']['common']['tz_offset'] = $part_prefs['common']['tz_offset'];
				$this->userSettings['preferences']['common']['timeformat'] = $part_prefs['common']['timeformat'];
				$this->userSettings['preferences']['common']['dateformat'] = $part_prefs['common']['dateformat'];

				//FIXME i think this is dodgy
				$tz_offset = ((60 * 60) * intval($this->userSettings['preferences']['common']['tz_offset']));

				if ($old_starttime)
				{
					$details['olddate'] = $this->phpgwapi_common->show_date($old_starttime);
				}
...
*/
		...
		{
			$this->userSettings = $temp_user;
		}

		$this->userSettings['preferences']['common']['tz_offset'] = $temp_tz_offset;
		$this->userSettings['preferences']['common']['timeformat'] = $temp_timeformat;
		$this->userSettings['preferences']['common']['dateformat'] = $temp_dateformat;
	...
	function alarm_today($event, $today, $starttime)
	{
		$found = False;
		@reset($event['alarm']);
		$starttime_hi = $this->phpgwapi_common->show_date($starttime, 'Hi');
		$t_appt['month'] = $this->phpgwapi_common->show_date($today, 'm');
		$t_appt['mday'] = $this->phpgwapi_common->show_date($today, 'd');
		$t_appt['year'] = $this->phpgwapi_common->show_date($today, 'Y');
		$t_appt['hour'] = $this->phpgwapi_common->show_date($starttime, 'H');
		$t_appt['min']  = $this->phpgwapi_common->show_date($starttime, 'i');
		$t_appt['sec']  = 0;
		$t_time = $this->maketime($t_appt) - phpgwapi_datetime::user_timezone();
		$y_time = $t_time - 86400;
		$tt_time = $t_time + 86399;
		print_debug('T_TIME', $t_time . ' : ' . $this->phpgwapi_common->show_date($t_time));
		print_debug('Y_TIME', $y_time . ' : ' . $this->phpgwapi_common->show_date($y_time));
		print_debug('TT_TIME', $tt_time . ' : ' . $this->phpgwapi_common->show_date($tt_time));
		...
		if (is_array($event['alarm']))
		{
			foreach ($event['alarm'] as $key => $alarm)
			{
				if ($alarm['enabled'])
				{
					print_debug('TIME', $alarm['time'] . ' : ' . $this->phpgwapi_common->show_date($alarm['time']) . ' (' . $event['id'] . ')');
					if ($event['recur_type'] != MCAL_RECUR_NONE)   /* Recurring Event */
					{
						print_debug('Recurring Event');
						if ($alarm['time'] > $y_time && $this->phpgwapi_common->show_date($alarm['time'], 'Hi') < $starttime_hi && $alarm['time'] < $t_time)
						{
							$found = True;
						}
					}
	...
	function list_cals()
	{
		...
		/*
			if ($memberships = $this->accounts_obj->membership($this->userSettings['account_id']))
			{
				foreach($memberships as $group_info)
				{
					...
					if ($account_perms = Acl::getInstance()->get_ids_for_location($group_info['account_id'],ACL_READ,'calendar'))
					{
	...
	{
		if (($type = $this->accounts_obj->get_type($id)) == 'g')
		{
			$arr = &$groups;
			$value = "g_{$id}";
			$name = $this->phpgwapi_common->grab_owner_name($id);
	...
	function event2array($event)
	{
		...
		$this->cat->__construct($this->accounts_obj->search_person($this->owner), 'calendar');
		...
		$var['startdate'] = array(
			...
			'data'	=> $this->phpgwapi_common->show_date($this->maketime($event['start']) - $user_timezone),
		);

		$var['enddate'] = array(
			'field'	=> lang('End Date/Time'),
			'data'	=> $this->phpgwapi_common->show_date($this->maketime($event['end']) - $user_timezone)
		);
		...
		$var['updated'] = array(
			'field'	=> lang('Updated'),
			'data'	=> $this->phpgwapi_common->show_date($this->maketime($event['modtime']) - $user_timezone)
		);
		...
		if ($event['recur_type'] != MCAL_RECUR_NONE)
		{
			...
			if ($event['recur_enddate']['mday'] != 0 && $event['recur_enddate']['month'] != 0 && $event['recur_enddate']['year'] != 0)
			{
				...
				if ($recur_end != 0)
				{
					...
					$str_extra .= lang('ends') . ': ' . lang($this->phpgwapi_common->show_date($recur_end, 'l')) . ', ' . $this->long_date($recur_end) . ' ';
				}
			}
	...
	{
		if (($set = Cache::session_get('calendar', 'default_prefs_set')))
		{
			return;
		}
		...
		$defaults = array(
			...
			'interval'        => '30',
			'defaultlength'   => '60',
			'planner_start_with_group' => $this->accounts_obj->name2id('Default'),
		...
		if (
			isset($this->prefs['calendar']['send_updates']) && $this->prefs['calendar']['send_updates']
	...
	function get_org_contacts($lookup, $cat_id)
	{
		...
		$criteria[] = $this->contacts->criteria_for_index($this->userSettings['account_id']);
	...
	function get_per_contacts($lookup, $cat_id)
	{
		...
		//echo "lookup == $lookup and user_id == {$this->userSettings['account_id']}";
		...
		//
		//			$criteria[] = phpgwapi_sql_criteria::_append_or($criteria_search);
		//			$criteria[] = $this->contacts->criteria_for_index((int) $this->userSettings['account_id']);
		//
		//			if ( $cat_id )
		//			{
		//				$criteria[] = phpgwapi_sql_criteria::_equal('cat_id', Sanitizer::get_var('cat_id', 'int', 'bool') );
	...
	function get_groups($search)
	{
		if ($search == '%')
		{
			$groups = $this->accounts_obj->get_list('groups', -1, 'ASC', 'account_firstname');
		}
		else
		{
			$groups = $this->accounts_obj->get_list('groups', -1, 'account_firstname', 'ASC', $search);
		}
