...
class frontend_bofrontend
{
	...
	private static function get_location_obj()
	{
		static $location_obj;
	...
	private static function get_accounts_obj()
	{
		static $accounts_obj;
		if (!isset($accounts_obj))
		{
	...
	{
		$locations = self::get_location_obj()->get_locations();
	...
	public static function delegate_exist(string $username)
	{
		if (isset($username))
		{
			if (self::get_accounts_obj()->exists($username))
			{
				return self::get_accounts_obj()->name2id($username);
			}
		}
		return false;
	}
	...
	{
		$account = self::get_accounts_obj()->get($account_id);
	...
	public static function create_delegate_account(string $username, string $firstname, string $lastname, string $password, $group_lid = 'frontend_delegates')
	{
		...
		if (!self::get_accounts_obj()->exists($group_lid)) // No group account exist
		{
			$account = new phpgwapi_group();
			$account->lid = $group_lid;
			$account->firstname = 'Frontend';
			$account->lastname = 'Delegates';
			$frontend_delegates = self::get_accounts_obj()->create($account, array(), array(), $modules);
		...
		{
			$frontend_delegates = self::get_accounts_obj()->name2id($group_lid);
		}
		...
		if (isset($username) && isset($firstname) && isset($lastname) && isset($password))
		{
			if (!self::get_accounts_obj()->exists($username))
			{
				...
				$account->expires = -1;
				$result = self::get_accounts_obj()->create($account, array($frontend_delegates), array(), array(
					'frontend'
				));
				if ($result)
				{
	...
	public static function get_delegates($org_unit_id, $distinct = false)
	{
		...
		$owner_id = isset($owner_id) ? $owner_id : $userSettings['account_id'];
	...
	public static function add_delegate(int $account_id, int $owner_id, $org_unit_id, $org_name)
	{
		...
		if (empty($owner_id))
		{
			...
			$owner_id = $userSettings['account_id'];
		}
		...
		if (isset($account_id))
		{
			...
			if ($result && $db->affected_rows() > 0)
			{
				/* 					//Retrieve the usernames
					  $user_account = self::get_accounts_obj()->get($account_id);
					  $owner_account = self::get_accounts_obj()->get($owner_id);
