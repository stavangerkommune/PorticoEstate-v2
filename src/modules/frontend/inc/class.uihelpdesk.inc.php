...
class frontend_uihelpdesk extends frontend_uicommon
{
	...
	{
		parent::__construct();
		Cache::session_set('frontend', 'tab', $this->locations->get_id('frontend', '.ticket'));
	...
	{
		$this->apps['manual']['section'] = 'helpdesk.index';
		Settings::getInstance()->update('apps', ['manual' => ['section' => 'helpdesk.index']]);
	...
	public function view()
	{
		...
		if (isset($assignedto) && $assignedto != '')
		{
			...
			$assignedto_account = $accounts_obj->get($assignedto);
		...
		$data = array(
			...
			'section' => array(
				...
				'tabs_content' => $this->tabs_content,
				'tab_selected' => $this->tab_selected,
				'msgbox_data' => isset($msglog) ? $this->phpgwapi_common->msgbox($this->phpgwapi_common->msgbox_data($msglog)) : array(),
			)
		);
	...
	public function add_ticket()
	{
		...
		{
			$item = execMethod('property.boentity.read_single', (array(
				'id' => $p_num,
		...
		if (isset($values['save']))
		{
			...
			if (!$missingfields && !Sanitizer::get_var('added'))
			{
				...
				$ticket = array(
					'origin_id' => $this->locations->get_id('property', $origin),
