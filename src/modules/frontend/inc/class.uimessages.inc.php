...
class frontend_uimessages extends frontend_uicommon
{
	...
	public function __construct()
	{
		...
		$extra_tabs[0] = array(
			...
			'link' => phpgw::link('/', array(
				'menuaction' => "frontend.uimessages.index",
	...
	{
		$form_action = phpgw::link('/index.php', array('menuaction' => 'frontend.uimessages.index'));

		$message_id = Sanitizer::get_var('message_id', 'int', 'REQUEST');
		$bomessenger = CreateObject('messenger.bomessenger');

		if (isset($message_id))
		{
			//en enkelt melding
			$message_id = Sanitizer::get_var('message_id', 'int', 'REQUEST', 0);
