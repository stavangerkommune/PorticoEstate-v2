...
class frontend_borental
{
	/* 		//FIXME Sigurd 22. nov 2013: - not used?
		  ...
		  {
		  $contracts_per_location = Cache::session_get('frontend', $contract_state_identifier);
		  $exist = false;
		...
		if (isset($contract) && isset($contract_message) && $contract_message != '')
		{
			...
			if (isset($contract_message) && isset($to) && isset($from_address))
			{
				if (isset($serverSettings['smtp_server']) && $serverSettings['smtp_server'])
				{
					...
					$from = "{$userSettings['fullname']}<{$from_address}>";

					$receive_notification = false;
					$rcpt = $send->msg('email', $to, $title, stripslashes(nl2br($contract_message)), '', $from, '', $from, $userSettings['fullname'], 'html', '', array(), $receive_notification);
	...
	{
		$contracts_per_location = Cache::session_get('frontend', 'contracts_per_location');
	...
	{
		$contracts_in_per_location = Cache::session_get('frontend', 'contracts_in_per_location');
