...
class frontend_uientity extends frontend_uicommon
{
	...
	public function __construct()
	{
		...
		$location_info = $this->locations->get_name($this->location_id);
	...
	{
		$this->apps['manual']['section'] = 'entity.index';
		Settings::getInstance()->update('apps', ['manual' => ['section' => 'entity.index']]);
		...
		if ($this->entity_id && $this->cat_id)
		{
			...
			Settings::getInstance()->update('flags', ['app_header' => lang($this->type_app[$this->type]) . ' - ' . $appname . ': ' . $function_msg]);
		}
