...
$custom_fields->add($attrib, 'bb_office_user');
...
{
	$db->query('SET identity_insert bb_activity ON', __LINE__, __FILE__);
}

foreach ($bb_activity as $value_set)
{
	$values	= $db->validate_insert($value_set);
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_activity OFF', __LINE__, __FILE__);
}
else if ($serverSettings['db_type'] == 'postgres')
{
	$db->query("SELECT setval('seq_bb_activity', COALESCE((SELECT MAX(id)+1 FROM bb_activity), 1), false)", __LINE__, __FILE__);
}
else // mysql
{
	$db->query('SELECT MAX(id)+1 AS next_value FROM bb_activity', __LINE__, __FILE__);
	$db->next_record();
	$_next_value = (int) $db->f('next_value');

	$db->query("ALTER TABLE bb_activity AUTO_INCREMENT = {$_next_value}", __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_agegroup ON', __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_agegroup OFF', __LINE__, __FILE__);
}
else if ($serverSettings['db_type'] == 'postgres')
{
	$db->query("SELECT setval('seq_bb_agegroup', COALESCE((SELECT MAX(id)+1 FROM bb_agegroup), 1), false)", __LINE__, __FILE__);
}
else // mysql
{
	$db->query('SELECT MAX(id)+1 AS next_value FROM bb_agegroup', __LINE__, __FILE__);
	$db->next_record();
	$_next_value = (int) $db->f('next_value');

	$db->query("ALTER TABLE bb_agegroup AUTO_INCREMENT = {$_next_value}", __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_targetaudience ON', __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_targetaudience OFF', __LINE__, __FILE__);
}
else if ($serverSettings['db_type'] == 'postgres')
{
	$db->query("SELECT setval('seq_bb_targetaudience', COALESCE((SELECT MAX(id)+1 FROM bb_targetaudience), 1), false)", __LINE__, __FILE__);
}
else // mysql
{
	$db->query('SELECT MAX(id)+1 AS next_value FROM bb_targetaudience', __LINE__, __FILE__);
	$db->next_record();
	$_next_value = (int) $db->f('next_value');

	$db->query("ALTER TABLE bb_targetaudience AUTO_INCREMENT = {$_next_value}", __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_rescategory ON', __LINE__, __FILE__);
}
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_rescategory OFF', __LINE__, __FILE__);
}
else if ($serverSettings['db_type'] == 'postgres')
{
	$db->query("SELECT setval('seq_bb_rescategory', COALESCE((SELECT MAX(id)+1 FROM bb_rescategory), 1), false)", __LINE__, __FILE__);
}
else // mysql
{
	$db->query('SELECT MAX(id)+1 AS next_value FROM bb_rescategory', __LINE__, __FILE__);
	$db->next_record();
	$_next_value = (int) $db->f('next_value');

	$db->query("ALTER TABLE bb_rescategory AUTO_INCREMENT = {$_next_value}", __LINE__, __FILE__);
}
...
if (!$accounts_obj->exists('booking_group'))
{
	...
	$booking_group = $accounts_obj->create($account, array(), array(), $modules);
}
else
{
	$booking_group = $accounts_obj->name2id('booking_group');
}
...
if (!$accounts_obj->exists('booking_admin'))
{
	...
	$account->lastname = 'Group';
	$booking_admin = $accounts_obj->create($account, array(), array(), $modules);
}
else
{
	$booking_admin = $accounts_obj->name2id('booking_admin');
}
...
$db->query("INSERT INTO bb_article_category ( id, name)"
...
if (in_array($serverSettings['db_type'], array('mssql', 'mssqlnative')))
{
	$db->query('SET identity_insert bb_article_category OFF', __LINE__, __FILE__);
}
...
$db->query("INSERT INTO bb_payment_method (id, payment_gateway_name, payment_gateway_mode) VALUES(1, 'Vipps', 'live')");
$db->query("INSERT INTO bb_payment_method (id, payment_gateway_name, payment_gateway_mode, is_default) VALUES(2, 'Etterfakturering', 'live', 1)");
