...
class booking_uiarticle_mapping extends phpgwapi_uicommon
{
	...
	{
		parent::__construct();
		$this->flags['app_header']	 .= '::' . lang('article mapping');
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		$requestUrl	 = json_encode(self::link(
			array(
				...
				'location_id'		 => $this->locations->get_id('booking', '.article'),
				'location_item_id'	 => $id,
				'phpgw_return_as'	 => 'json'
			)
		));
...
JS;
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction'		 => "{$this->currentapp}.uiarticle_mapping._get_files",
				'id'				 => $id,
				'section'			 => 'documents',
				'phpgw_return_as'	 => 'json'
			))),
		...
		$jqcal2->add_listener('date_from', 'date');
		$data = array(
		...
		$this->flags['app_header']	 .= '::' . lang('edit');
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);

		if ($article->article_name)
		{
			$this->flags['app_header'] = lang('edit') . '::' . $article->article_name;
			Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		}
	...
	public function handle_multi_upload_file()
	{
		...
		$options['upload_dir']	 = $this->serverSettings['files_dir'] . '/booking/' . $options['base_dir'] . '/';
