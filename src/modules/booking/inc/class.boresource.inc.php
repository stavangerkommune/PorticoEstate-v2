...
class booking_boresource extends booking_bocommon_authorized
{
	...
	public function populate_grid_data($menuaction)
	{
		...
		foreach ($resources['results'] as &$resource)
		{
			...
			$resource['direct_booking_date'] = $resource['direct_booking'] ? $phpgwapi_common->show_date($resource['direct_booking'], $dateformat) : null;
	...
	function add_activity_facility_data(&$resources)
	{
		...
		foreach ($resources as &$resource)
		{
			...
			usort($resource['activities_list'], function ($a, $b)
			{
			...
			usort($resource['facilities_list'], function ($a, $b)
			{
				return strcmp(strtolower($a['name']), strtolower($b['name']));
			});
		}
	...
	public function get_schedule($id, $buildingmodule, $resourcemodule, $search = null)
	{
		...
		{
			$resource['days'][] = array(
				'label' => sprintf('%s<br/>%s %s', lang($date->format('l')), lang($date->format('M')), $date->format('d')),
				'key' => $date->format('D')
			);
			$date->modify('+1 day');
		}
