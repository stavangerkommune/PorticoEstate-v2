...
class booking_uibooking extends booking_uicommon
{
	...
	public function __construct()
	{
		...
		$this->fields = array(
			'allocation_id', 'activity_id', 'resources',
	...
	private function item_link(&$item, $key)
	{
		if (isset($item['type']) && in_array($item['type'], array('allocation', 'booking', 'event')))
		{
			$item['info_url'] = $this->link(array(
				'menuaction' => 'booking.ui' . $item['type'] . '.info',
				'id' => $item['id']
			));
		}
	}
	...
	public function building_schedule()
	{
		...
		foreach ($bookings['results'] as &$booking)
		{
			...
			$booking['link'] = $this->link(array(
				'menuaction' => 'booking.uibooking.show',
				'id' => $booking['id']
			));
	...
	public function add()
	{
		...
		else
		{
			...
			if ($application_id && empty($booking['organization_id']))
			{
				...
				{
					$organizations = createObject('booking.soorganization')->read(array('results' => -1, 'filters' => array(
		...
		$groups = $this->group_bo->so->read(array('results' => -1, 'filters' => array(
			'organization_id' => $allocation['organization_id'],
			'active' => 1
		)));
		$groups = $groups['results'];

		$resouces_full = $this->resource_bo->so->read(array(
			'results' => -1, 'filters' => array('id' => $booking['resources']),
			'sort' => 'name'
		));
		...
		$booking['validator'] = phpgwapi_jquery::formvalidator_generate(array(
			'location',
			'date', 'security', 'file'
		));
	...
	public function edit()
	{
		...
		$booking['application_link'] = self::link(array(
		...
		self::render_template_xsl('booking_edit', array(
			'booking' => $booking, 'activities' => $activities,
	...
	public function show()
	{
		...
		$this->send_sms_participants($id, 'booking');
		...
		$booking['delete_link'] = self::link(array(
