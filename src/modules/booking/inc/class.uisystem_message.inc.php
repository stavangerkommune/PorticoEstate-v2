...
class booking_uisystem_message extends booking_uicommon
{
	...
	public function index()
	{
		...
		Settings::getInstance()->update('apps', ['manual' => ['section' => 'booking_manual']]);
		...
		$data = array(
			...
			'datatable' => array(
				'source' => self::link(array(
					'menuaction' => $this->module . '.uisystem_message.index',
	...
	public function edit()
	{
		...
		{
			$system_message = array_merge($system_message, extract_values(
				$_POST,
				array(
			...
			if (!$errors)
			{
				...
				self::redirect(array(
					'menuaction' => 'booking.uisystem_message.edit', 'id' => $receipt['id'],
					'warnings' => $errors
				));
			}
		}
		...
		self::render_template_xsl('system_message_edit', array(
	...
	public function show()
	{
		...
		$system_message['system_message_link'] = self::link(array(
			'menuaction' => $this->module . '.uisystem_message.show',
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			if ($_POST['status'] == 'CLOSED')
			{
				...
				$receipt = $this->bo->update($system_message);
				self::redirect(array(
					'menuaction' => 'booking.uisystem_message.show', 'id' => $receipt['id'],
					'warnings' => $errors
				));
			}
		}
		...
		self::render_template_xsl('system_message', array(
			'system_message' => $system_message,
			'loggedin' => $loggedin
		));
	}
