...
class booking_uiactivity extends booking_uicommon
{
			...
			foreach ($children[$parent_id] as $activity)
			{
				...
				$node = array(
					...
					"href" => self::link(array(
						'menuaction' => 'booking.uiactivity.edit',
						'id' => $activity['id']
					)), 'target' => '_self',
					'label' => $activity['name'],
					'text' => $activity['name'],
	...
	public function index()
	{
		...
		$links = array(
			'show_inactive' => self::link(array(
				'menuaction' => 'booking.uiactivity.index',
				'show_all' => 'true'
			)),
			'hide_inactive' => self::link(array(
				'menuaction' => 'booking.uiactivity.index',
				'show_all' => ''
			))
		);
		...
		self::render_template_xsl('activities', array(
			'treedata' => $treedata, 'links' => $links,
	...
	public function add()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			if (!$errors)
			{
				...
				$hooks = new Hooks();
				$hooks->single(array(
					'id' => $receipt['id'], 'name' => $activity['name'],
					'location' => 'activity_add'
				), 'booking');
		...
		$activity['validator'] = phpgwapi_jquery::formvalidator_generate(array(
			'location',
			'date', 'security', 'file'
		));
	...
	public function edit()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			$activity = array_merge($activity, extract_values($_POST, array(
				'name', 'active',
			...
			if (!$errors)
			{
				...
				$hooks = new Hooks();
				$hooks->single(array(
					'id' => $id, 'name' => $activity['name'],
					'location' => 'activity_edit'
				), 'booking');
				self::redirect(array('menuaction' => 'booking.uiactivity.index'));
			}
		}
		...
		self::render_template_xsl('activity_edit', array(
			'activity' => $activity, 'parent' => $parent_activity,
