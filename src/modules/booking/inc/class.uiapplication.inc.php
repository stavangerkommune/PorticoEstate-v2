...
class booking_uiapplication extends booking_uicommon
{
	...
	public function custom_fields_example()
	{
		...
		$locations = $this->locations->get_locations($grant = false, 'booking', $allow_c_attrib = true, $allow_c_functions = false);
	...
	private function get_attributes($location)
	{
		...
		$attributes = $custom_fields->find($appname, $location, 0, '', 'ASC', 'attrib_sort', true, true);
	...
	{
		$custom_fields = createObject('phpgwapi.custom_fields');
		return $custom_fields->get_attribute_groups($appname, $location, $attributes);
	}
	...
	protected function assign_to_current_user(&$application)
	{
		...
		if (
			!empty($current_account_id) && is_array($application) &&
	...
	{
		if (
			!empty($account_id) && is_array($application) &&
	...
	protected function is_assigned_to(&$application)
	{
		...
		{
			$application['case_officer_full_name'] = $this->accounts_obj->get($application['case_officer_id'])->__toString();
		}
	}
	...
	private function _get_filters()
	{
		...
		//			if (!isset($this->userSettings['apps']['admin']) && !$this->bo->has_role(booking_sopermission::ROLE_MANAGER))
		{
			$filters[] = array(
				'type'		 => 'filter',
				'multiple'	 => true,
	...
	public function associated()
	{
		...
		foreach ($associations['results'] as &$association)
		{
			...
			$association['link'] = self::link(array(
				'menuaction' => 'booking.ui' . $association['type'] . '.edit',
				'id' => $association['id']
			));
	...
	public function payments()
	{
		...
		{
			$payment['created_value'] = $this->phpgwapi_common->show_date($payment['created']);
	...
	private function validate_limit_number($resource_id, $ssn, &$errors)
	{
		...
		if ($resource['booking_limit_number_horizont'] > 0 && $resource['booking_limit_number'] > 0)
		{
			...
			if ($limit_reached)
			{
				$errors['error_message'] = lang(
					'quantity limit (%1) exceeded for %2: maximum %3 times within a period of %4 days',
					$limit_reached,
					$resource['name'],
					$resource['booking_limit_number'],
					$resource['booking_limit_number_horizont']
	...
	public function add()
	{
		...
		if ($this->flags['currentapp'] == 'bookingfrontend' && $resource['simple_booking_start_date'] && $resource['simple_booking_start_date'] < time() && !$simple)
		{
			...
			if (!$errors)
			{
				...
				else
				{
					...
					Cache::message_set(lang("Your application has now been registered and a confirmation email has been sent to you.") . "<br />" .
						...
						lang(
							"Please check your Spam Filter if you are missing mail."
						));
		...
		else if ($this->flags['currentapp'] == 'bookingfrontend')
		{
			$application['cancel_link'] = self::link(array(
		...
		if ($this->flags['currentapp'] != 'bookingfrontend')
		{
			$jqcal2->add_listener('start_date', 'datetime', !empty($default_dates) ? strtotime($default_dates[0]['from_']) : 0);
			$jqcal2->add_listener('end_date', 'datetime', !empty($default_dates) ? strtotime($default_dates[0]['to_']) : 0);
		...
		if ($this->flags['currentapp'] == 'bookingfrontend' && $simple)
		{
			...
			$jqcal2->add_listener('start_date', 'date', $default_start_date, array(
				'min_date' => time(),
	...
	function add_contact()
	{
		...
		$location_id		 = $this->locations->get_id('booking', 'run');
		...
		if ($activate_prepayment && !empty($custom_config['payment']['method']) && !empty($custom_config['Vipps']['active']))
		{
			...
			switch ($this->userSettings['preferences']['common']['lang'])
			{
				case 'no':
				case 'nn':
					$vipps_logo .= "_NO";
			...
			$payment_methods[] = array(
				'method'	 => 'vipps',
				'logo'		 => $this->phpgwapi_common->image('bookingfrontend', $vipps_logo)
			);
		}
		...
		self::render_template_xsl(
			...
			array(
				...
				'add_img'				 => $this->phpgwapi_common->image('phpgwapi', 'add2'),
				'config'				 => CreateObject('phpgwapi.config', 'booking')->read(),
	...
	public function edit()
	{
		...
		$jqcal2 = createObject('phpgwapi.jqcal2');
		$jqcal2->add_listener('start_date', 'datetime', !empty($application['dates'][0]['from_']) ? strtotime($application['dates'][0]['from_']) : 0);
		$jqcal2->add_listener('end_date', 'datetime', !empty($application['dates'][0]['to_']) ? strtotime($application['dates'][0]['to_']) : 0);
		...
		$current_app = $this->flags['currentapp'] ? $this->flags['currentapp'] : 'booking';
	...
	private function _get_pdf_header($application, $config, $preview)
	{
		...
		//			if (!empty($this->serverSettings['logo_url']))
		{
			//cheating...
	...
	public function show()
	{
		...
		$application['edit_link'] = self::link(array(
			'menuaction' => 'booking.uiapplication.edit',
			'id' => $application['id']
		));
		$building_info = $this->bo->so->get_building_info($id);
		$application['building_id'] = $building_info['id'];
