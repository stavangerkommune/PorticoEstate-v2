...
class logistic_uiresource_type_requirement extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->acl_read = $this->acl->check('.project', ACL_READ, 'logistic'); //1
		$this->acl_add = $this->acl->check('.project', ACL_ADD, 'logistic'); //2
		$this->acl_edit = $this->acl->check('.project', ACL_EDIT, 'logistic'); //4
		$this->acl_delete = $this->acl->check('.project', ACL_DELETE, 'logistic'); //8
		$this->acl_manage = $this->acl->check('.project', 16, 'logistic'); //16

		self::set_active_menu("admin::logistic::resource_type_requirement");
		...
		$data = array(
			...
			'datatable' => array(
				'source' => self::link(array(
					'menuaction' => 'logistic.uiresource_type_requirement.index',
					'phpgw_return_as' => 'json',
					'type' => 'resource_type_requirement_list'
				)),
	...
	public function edit()
	{
		...
		if ($location_id)
		{
			$req_types = $this->so->get(0, 0, '', false, '', '', array(
		...
		if (isset($_POST['save']))
		{
			...
			$location_id = $this->locations->get_id('property', ".entity.{$entity_id}.{$category_id}");
			...
			$req_type_array = $this->so->get(0, 0, '', false, '', '', array(
				'location_id' => $location_id,
				'project_type_id' => $project_type_id
			));
			...
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'logistic.uiresource_type_requirement.view',
				'location_id' => $location_id,
				'project_type_id' => $req_type->get_project_type_id()
			));
		}
		...
		else
		{
			...
			Settings::getInstance()->update('flags', ['app_header' => lang('logistic') . '::' . lang('Project type')]);
	...
	public function view()
	{
		...
		{
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'logistic.uiresource_type_requirement.edit',
				'location_id' => $location_id,
				'project_type_id' => $project_type_id
			));
		}
