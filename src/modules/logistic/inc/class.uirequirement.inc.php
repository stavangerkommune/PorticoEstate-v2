...
class logistic_uirequirement extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		self::set_active_menu("logistic::project::requirement");

		Settings::getInstance()->update('flags', ['nonavbar' => true, 'noheader_xsl' => true, 'nofooter' => true]);

		$this->read = $this->acl->check('.activity', ACL_READ, 'logistic'); //1
		$this->add = $this->acl->check('.activity', ACL_ADD, 'logistic'); //2
		$this->edit = $this->acl->check('.activity', ACL_EDIT, 'logistic'); //4
		$this->delete = $this->acl->check('.activity', ACL_DELETE, 'logistic'); //8
		$this->manage = $this->acl->check('.activity', 16, 'logistic'); //16
	...
	public function view()
	{
		...
		else
		{
			...
			$location_info = $this->locations->get_name($requirement->get_location_id());

			$tabs = $this->make_tab_menu($requirement_id);

			$data = array(
				'tabs' => $this->phpgwapi_common->create_tabs($tabs, 'details'),
			...
			Settings::getInstance()->update('flags', ['app_header' => lang('logistic') . '::' . lang('Project') . '::' . lang('Requirement')]);
	...
	public function edit($requirement = null)
	{
		...
		$accounts = $this->acl->get_user_list_right(ACL_READ, 'run', 'logistic');
		...
		$jqcal2->add_listener('start_date', 'datetime');
		$jqcal2->add_listener('end_date', 'datetime');
	...
	public function save()
	{
		...
		{
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'logistic.uirequirement.view',
				'id' => $requirement_id,
				'nonavbar' => $this->nonavbar
			));
	...
	public function add_requirement_values()
	{
		...
		$data = array(
			'tabs' => $this->phpgwapi_common->create_tabs($tabs, 'constraints'),
	...
	public function view_requirement_values()
	{
		...
		{
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'logistic.uirequirement.add_requirement_values',
	...
	public function save_requirement_values()
	{
		...
		phpgw::redirect_link('/index.php', array(
			'menuaction' => 'logistic.uirequirement.view_requirement_values',
			'requirement_id' => $requirement_id,
			'nonavbar' => $this->nonavbar
		));
	}
	...
	public function assign_job()
	{
		...
		#FIXME timezone..
		//$this->phpgwapi_common->show_date($requirement->get_start_date())
		//$message .= 'Frist:' . $this->phpgwapi_common->show_date($requirement->get_start_date()) . "\n\n";

		$datetime_format = "{$this->userSettings['preferences']['common']['dateformat']} H:i";
		...
		foreach ($allocations as $allocation)
		{
			...
			if ($allocation->get_inventory_id())
			{
				...
				$system_location = $this->locations->get_name($inventory[0]['p_location_id']);
				$name = 'N∕A';
				...
				else if (preg_match('/.entity./i', $system_location['location']))
				{
					$name = execMethod('property.soentity.get_short_description', array(
						'location_id' => $inventory[0]['p_location_id'],
						'id' => $inventory[0]['p_id']
					));
				}
			}
	...
	function send_job_ticket()
	{
		...
		$location_id = $this->locations->get_id("logistic", ".activity");
		...
		if ($location_id_ticket = $this->locations->get_id('property', '.ticket'))
		{
