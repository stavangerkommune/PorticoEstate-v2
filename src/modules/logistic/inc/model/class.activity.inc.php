<?php
	...
	class logistic_activity extends logistic_model
	{
		...
		{
			$this->set_id(Sanitizer::get_var('id', 'POST', 'int'));
			$this->set_name(Sanitizer::get_var('name'));
			$this->set_responsible_user_id(Sanitizer::get_var('responsible_user_id', 'POST', 'int'));
			$this->set_description(Sanitizer::get_var('description'));

			if (isset($_POST['parent_activity_id']))
			{
				$activity_id = Sanitizer::get_var('parent_activity_id', 'POST', 'int');
				$this->set_parent_id($activity_id ? $activity_id : '' );
			}
			else
			{
				$this->set_parent_id(Sanitizer::get_var('parent_id', 'POST', 'int'));
			}
			...
			if (Sanitizer::get_var('start_date', 'POST', 'string') != '')
			{
				$start_date_ts = phpgwapi_datetime::date_to_timestamp(Sanitizer::get_var('start_date', 'POST', 'string'));
				$this->set_start_date($start_date_ts);
			}

			if (Sanitizer::get_var('end_date', 'POST', 'string') != '')
			{
				$end_date_ts = phpgwapi_datetime::date_to_timestamp(Sanitizer::get_var('end_date', 'POST', 'string'));
