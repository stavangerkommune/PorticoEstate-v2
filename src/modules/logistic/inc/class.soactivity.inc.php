...
class logistic_soactivity extends logistic_socommon
{
	...
	public function read_tree($sql, $filters, $num_of_objects = 0, $start = 0, $allrows = false)
	{
		...
		else
		{
			...
			$num_rows = isset($userSettings['preferences']['common']['maxmatchs']) && $userSettings['preferences']['common']['maxmatchs'] ? (int)$userSettings['preferences']['common']['maxmatchs'] : 15;
		}
	...
	public function get_responsible_user($user_id)
	{
		if ($user_id && is_numeric($user_id))
		{
			...
			$account = $accounts_obj->get($user_id);
			if (isset($account))
			{
