...
class logistic_menu
{
	...
	function get_menu()
	{
		...
		Settings::getInstance()->update('flags', ['currentapp' => 'logistic']);
		$acl					 =	Acl::getInstance();
		...
		if (isset($userSettings['preferences']['logistic']['menu_favorites']) && $userSettings['preferences']['logistic']['menu_favorites'])
		{
			$menu_favorites = $userSettings['preferences']['logistic']['menu_favorites'];
				...
				foreach ($targets as $target => $target_name)
				{
					$favorites_children["{$type}{$target}"] = array(
						...
						'url' => phpgw::link('/index.php', array(
							'menuaction' => "logistic.ui{$type}.index",
							'filter' => $target
						)),
		...
		if ($acl->check('run', Acl::READ, 'admin') || $acl->check('admin', Acl::ADD, 'logistic'))
		{
			$menus['admin'] = array(
				'index' => array(
