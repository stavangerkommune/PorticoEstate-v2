...
class logistic_uiactivity extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		self::set_active_menu("logistic::project::activity");

		$this->acl_read = $this->acl->check('.activity', ACL_READ, 'logistic'); //1
		$this->acl_add = $this->acl->check('.activity', ACL_ADD, 'logistic'); //2
		$this->acl_edit = $this->acl->check('.activity', ACL_EDIT, 'logistic'); //4
		$this->acl_delete = $this->acl->check('.activity', ACL_DELETE, 'logistic'); //8
		$this->acl_manage = $this->acl->check('.activity', 16, 'logistic'); //16
	...
	public function query()
	{
		...
		switch ($query_type)
		{
			...
			default: // ... all activities, filters (active and vacant)
				...
				$filters = array(
					'project' => Sanitizer::get_var('project'),
					'user' => Sanitizer::get_var('user'),
	...
	public function edit($activity = null)
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('logistic') . '::' . lang('Add activity')]);
	...
	public function save()
	{
		...
		if ($activity->validate())
		{
			...
			phpgw::redirect_link('/index.php', array(
				'menuaction' => 'logistic.uiactivity.view',
				'id' => $activity_id,
				'project_id' => $activity->get_project_id()
			));
		}
	...
	public function edit_favorite()
	{
		if ($activity_id = Sanitizer::get_var('id'))
		{
			...
			if (isset($this->userSettings['preferences']['logistic']['menu_favorites']) && $this->userSettings['preferences']['logistic']['menu_favorites'])
			{
				$menu_favorites = $this->userSettings['preferences']['logistic']['menu_favorites'];
			}
			else
			{
			...
			$preferences->add('logistic', 'menu_favorites', $menu_favorites, 'user');
	...
	public function view_resource_allocation()
	{
		...
		$ColumnDefs1 = array(
			...
			array(
				...
				'label' => lang('resource type descr'),
				'sortable' => true
			),
		...
		$data = array(
			'datatable_def' => $datatable_def,
			'tabs' => $this->phpgwapi_common->create_tabs($tabs, 'allocation'),
