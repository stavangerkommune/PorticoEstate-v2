...
class logistic_uirequirement_resource_allocation extends phpgwapi_uicommon_jquery
{
	...
	var  $part_of_town_id, $district_id, $status, $allrows, $lookup;
		...
		foreach ($result_objects as $result)
		{
			if (isset($result))
			{
				...
				$delete_href = self::link(
					array(
						'menuaction' => 'logistic.uirequirement_resource_allocation.delete',
						'id' => $allocation['id'],
						'phpgw_return_as' => 'json'
					)
				);
	...
	public function edit()
	{
		...
		foreach ($allocation_suggestions as &$allocation_suggestion)
		{
			if (isset($allocated['items'][$allocation_suggestion['id']]))
			{
				...
				{
					$allocated_date[] = $this->phpgwapi_common->show_date($calendar_entry['start_date'], $dateformat)
						. ' - ' . $this->phpgwapi_common->show_date($calendar_entry['end_date'], $dateformat);
			...
			if (isset($allocation_suggestion['inventory']))
			{
				foreach ($allocation_suggestion['inventory'] as &$inventory)
				{
					...
					{
						$inventory['allocated_date'] = $this->phpgwapi_common->show_date($allocated['allocations'][$inventory['inventory_id']]['start_date'], $dateformat)
							. ' - ' . $this->phpgwapi_common->show_date($allocated['allocations'][$inventory['inventory_id']]['end_date'], $dateformat);
					}
				}
			}
		}
	...
	public function view()
	{
		...
		else
		{
			...
			Settings::getInstance()->update('flags', ['app_header' => lang('logistic') . '::' . lang('Project') . '::' . lang('Activity') . '::' . lang('Allocation')]);
