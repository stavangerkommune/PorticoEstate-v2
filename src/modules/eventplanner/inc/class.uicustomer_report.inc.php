...
class eventplanner_uicustomer_report extends phpgwapi_uicommon
{
	...
	protected
		...
		$permissions,
		$custom_fields,
		$currentapp, $account, $acl_location, $cats;

	public function __construct()
	{
		parent::__construct();
		Settings::getInstance()->update('flags', ['app_header' => lang('eventplanner') . '::' . lang('customer report')]);
	...
	function columns()
	{
		...
		if (isset($values['save']) && $values['save'])
		{
			...
			$preferences->add('eventplanner', 'customer_report_columns', (array)$values['columns'], 'user');
	...
	function _get_fields()
	{
		...
		$selected = (array)$this->userSettings['preferences']['eventplanner']['customer_report_columns'];
...
*/
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('eventplanner') . '::' . lang('customer report') . '::' . lang('edit')]);
		...
		$booking_id = $customer_report->booking_id ? $customer_report->booking_id : Sanitizer::get_var('booking_id', 'int');
		...
		phpgwapi_jquery::load_widget('autocomplete');
		//	self::add_javascript($this->currentapp, 'base', 'customer_report.edit.js');
