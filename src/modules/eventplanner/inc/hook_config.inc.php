...
function default_vendor_category($config)
{
	...
	$status_entries = execMethod('eventplanner.bogeneric.get_list', array(
...
function vendor_receipt_text_editor($config)
{
	...
	if (
		empty($userSettings['preferences']['common']['rteditor'])
		|| $userSettings['preferences']['common']['rteditor'] != 'ckeditor'
	)
	{
		return;
	}
