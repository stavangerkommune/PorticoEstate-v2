...
class eventplanner_uiapplication extends phpgwapi_uicommon
{
	...
	{
		parent::__construct();
		Settings::getInstance()->update('flags', ['app_header' => lang('eventplanner'). '::' . lang('application')]);
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		$accounts = $this->acl->get_user_list_right(ACL_EDIT, '.application', 'eventplanner');
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => "{$this->currentapp}.uicalendar.query_relaxed",
		...
		$jqcal->add_listener('date_start', 'date', '', $config_calendar);
		$jqcal->add_listener('date_end', 'date', '', $config_calendar);
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction' => "{$this->currentapp}.uiapplication._get_files",
				'id' => $id,
				'section' => 'cv',
				'phpgw_return_as' => 'json'
			))),
	...
	public function handle_multi_upload_file()
	{
		...
		$options['upload_dir'] = $this->serverSettings['files_dir'] . '/eventplanner/' . $options['base_dir'] . '/';
