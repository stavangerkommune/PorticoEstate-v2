...
const BuildingResources = async (props: BuildingResourcesProps) => {
    ...
    return (
        <div className={'mx-3'}>
            ...
            <hr className={`my-2`}/>
            <ResourceContainer>
                {resources.map((res) => <Button asChild key={res.id} variant={'secondary'} color={'neutral'}
                                                className={'default'}>
                    <ClientPHPGWLink strURL={'bookingfrontend/'} oArgs={{menuaction: 'bookingfrontend.uiresource.show', building_id: props.building.id, id: res.id}} ><FontAwesomeIcon icon={faLayerGroup} />{res.name}</ClientPHPGWLink>
