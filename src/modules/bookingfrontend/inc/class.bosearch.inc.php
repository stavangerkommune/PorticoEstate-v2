	...
	function search($searchterm, $building_id, $filter_part_of_town, $filter_top_level, $activity_criteria = array(), $length = null)
	{
		...
		if (in_array('building', $types))
		{
			...
			foreach ($bui_result['results'] as &$bui)
			{
				...
				$bui['type'] = "building";
				$bui['link'] = phpgw::link('/bookingfrontend/', array(
					'menuaction' => 'bookingfrontend.uibuilding.show',
					'id' => $bui['id']
				));
				$bui['img_container'] = "building-" . $bui['id'];
				$bui['img_url'] = phpgw::link('/bookingfrontend/', array(
		...
		{
			$org_result = $this->soorganization->read(array(
				"query" => $searchterm, "sort" => "name",
			...
			foreach ($org_result['results'] as &$org)
			{
				...
				$org['link'] = phpgw::link('/bookingfrontend/', array(
		...
		if (in_array('resource', $types))
		{
			...
			$_res_result = $this->soresource->read(array(
				"query" => $searchterm, "sort" => "name",
			...
			foreach ($_res_result['results'] as &$res)
			{
				...
				$res['type'] = "resource";
				$res['link'] = phpgw::link('/bookingfrontend/', array(
					'menuaction' => 'bookingfrontend.uiresource.show',
					'id' => $res['id']
				));
				$res['img_container'] = "resource-" . $res['id'];
				$res['img_url'] = phpgw::link('/bookingfrontend/', array(
 ...
 */
		...
		if (in_array('event', $types))
		{
			...
			$event_result = $this->soevent->read(array(
				"query" => $searchterm, "sort" => "name",
			...
			foreach ($event_result['results'] as &$event)
			{
				...
				$event['link'] = phpgw::link('/bookingfrontend/', array(
	...
	function resquery($params = array())
	{
		...
		usort($all_activities_list, function ($a, $b)
		{
			return strcmp(strtolower($a['name']), strtolower($b['name']));
		});
		$all_facilities_list = array_values($all_facilities);
		usort($all_facilities_list, function ($a, $b)
		{
			return strcmp(strtolower($a['name']), strtolower($b['name']));
		});
		...
		usort($all_partoftown_list, function ($a, $b)
		{
			return strcmp(strtolower($a['name']), strtolower($b['name']));
		});
	...
	function search_available_resources($searchterm, $building_id, $filter_part_of_town, $filter_top_level, $activity_criteria = array(), $length = null, $params = array())
	{
		...
		usort($applicable_resources, function ($a, $b)
		{
			return strcmp(strtolower($a['name']), strtolower($b['name']));
		});
