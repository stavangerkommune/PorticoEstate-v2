...
class messenger_somessenger extends somessenger_
{
	...
	function read_message($message_id)
	{
		...
		$message = array(
			...
			'date' => $msg->udate,
			'subject' => $msg->Subject,
			'content' => $this->_get_text_part($message_id)
		);
		return $message;
	}
	...
	function send_message($message, $global_message = false)
	{
		...
		$mailer->From = $contacts->get_email($this->userSettings['person_id']);
		$mailer->FromName = $phpgwapi_common->display_fullname();
		$mailer->AddAddress($contacts->get_email($contacts->is_contact($message['to'])), $this->accounts_obj->id2name($message['to']));
