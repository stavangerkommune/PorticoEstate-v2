...
class notes_sonotes
{
	...
	function __construct()
	{
		...
		$this->grants	= $acl->get_grants2('notes', 'run');
	}
	...
	function read($data)
	{
		...
		if ($cat_id > 0)
		{
			$filtermethod .= " {$where} note_category='$cat_id' ";
			$where = 'AND';
		}
		...
		if ($query)
		{
			...
			$querymethod = " {$where} note_content LIKE '%$query%' ";
		}
		else
		{
			$querymethod = '';
		...
		while ($this->db->next_record())
		{
			...
			$notes[$id] = array(
				...
				'date'		=> $phpgwapi_common->show_date($this->db->f('note_date')),
