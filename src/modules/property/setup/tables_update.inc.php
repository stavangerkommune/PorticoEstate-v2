...
//			$db->query($sql,__LINE__,__FILE__);
	...
	*/
	...
	/**
	*
	* Update property version from 0.9.17.736 to 0.9.17.737
	*
	*/
	$test[] = '0.9.17.736';
	function property_upgrade0_9_17_736()
	{
		$GLOBALS['phpgw_setup']->oProc->m_odb->transaction_begin();

		$GLOBALS['phpgw_setup']->oProc->AddColumn('fm_request', 'representative', array
				(
					'type'		=> 'decimal',
					'precision' => '20',
					'scale'		=> '2',
		...
		if($GLOBALS['phpgw_setup']->oProc->m_odb->transaction_commit())
		{
			$GLOBALS['setup_info']['property']['currentver'] = '0.9.17.737';
			return $GLOBALS['setup_info']['property']['currentver'];
		}
	}
...
*/
	...
	/**
	*
	* Update property version from 0.9.17.738 to 0.9.17.739
	*
	*/
	$test[] = '0.9.17.738';
	function property_upgrade0_9_17_738()
	{
		$GLOBALS['phpgw_setup']->oProc->m_odb->transaction_begin();

		$GLOBALS['phpgw_setup']->oProc->AddColumn("fm_org_unit", 'persistent', array
				(
					'type'		=> 'int',
					'precision' => '2',
					'nullable'	=> True
		...
		if($GLOBALS['phpgw_setup']->oProc->m_odb->transaction_commit())
		{
			$GLOBALS['setup_info']['property']['currentver'] = '0.9.17.739';
			return $GLOBALS['setup_info']['property']['currentver'];
		}
	}
...
//		}
	 ...
	 * Notes
	 * Alter column
	 * Empty sql-cache to reflect datatypes on location values
	 */
	$test[] = '0.9.17.751';
	...
	*/
	$test[] = '0.9.17.753';
	function property_upgrade0_9_17_753()
	{
		$GLOBALS['phpgw_setup']->oProc->m_odb->transaction_begin();
		$db = & $GLOBALS['phpgw_setup']->oProc->m_odb;
		...
		$metadata = $GLOBALS['phpgw_setup']->db->metadata("fm_location1");

		if(!isset($metadata['area_gross']))
		{
			$cust = array
			(
				'datatype'		=> 'N',
			...
			for($i = 1; $i < $levels + 1; $i++)
			{
				...
				if(isset($metadata['bruttoareal']))
				{
					$db->query("UPDATE fm_location{$i} SET area_gross = bruttoareal", __LINE__, __FILE__);
					$db->query("UPDATE fm_location{$i}_history SET area_gross = bruttoareal", __LINE__, __FILE__);
				}
				if(isset($metadata['nettoareal']))
				{
					$db->query("UPDATE fm_location{$i} SET area_net = nettoareal", __LINE__, __FILE__);
					$db->query("UPDATE fm_location{$i}_history SET area_net = nettoareal", __LINE__, __FILE__);
				}
				if(isset($metadata['boareal']))
				{
					$db->query("UPDATE fm_location{$i} SET area_usable = boareal", __LINE__, __FILE__);
					$db->query("UPDATE fm_location{$i}_history SET area_usable = boareal", __LINE__, __FILE__);
				}
		...
		if($GLOBALS['phpgw_setup']->oProc->m_odb->transaction_commit())
		{
			$GLOBALS['setup_info']['property']['currentver'] = '0.9.17.754';
			return $GLOBALS['setup_info']['property']['currentver'];
		}
	}
	...
	*/
	$test[] = '0.9.17.757';
	function property_upgrade0_9_17_757()
	{
		$GLOBALS['phpgw_setup']->oProc->m_odb->transaction_begin();

		$GLOBALS['phpgw_setup']->oProc->AddColumn('fm_external_project', 'b_account_category', array(
			'type' => 'varchar',
			'precision' => '255',
			'nullable' => True
			));
		...
		if($GLOBALS['phpgw_setup']->oProc->m_odb->transaction_commit())
		{
			$GLOBALS['setup_info']['property']['currentver'] = '0.9.17.758';
			return $GLOBALS['setup_info']['property']['currentver'];
		}
	}
