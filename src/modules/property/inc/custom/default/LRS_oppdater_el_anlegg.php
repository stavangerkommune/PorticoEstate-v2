	...
	function check_history($values, $values_attribute, $entity_id, $cat_id, $receipt)
	{
		...
		if (($history_values[0]['datetime'] > $current_time))
		{
			if ((int)$history_values[0]['old_value'] != (int)$current_value)
			{
				...
				$coordinator_name	 = $userSettings['fullname'];
				$coordinator_email	 = "{$coordinator_name}<{$userSettings['preferences']['common']['email']}>";
				$bcc				 = '';
				...
				$rcpt = $send->msg('email', $_to, $subject, $body, '', $cc, $bcc, $coordinator_email, $coordinator_name, 'text', '', false, true);
			}
		}
	}
