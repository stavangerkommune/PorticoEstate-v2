...
class overfoer_stroem extends property_boentity
{
	...
	function update_data($values, $values_attribute = array(), $action)
	{
		...
		if (!$maaler_nr)
		{
			Cache::message_set("Målernummer er ikke registrert", 'error');
			$error = true;
		}

		if (!$maalerstand)
		{
			Cache::message_set("Målerstand er ikke registrert", 'error');
			$error = true;
		}
		...
		$location_id_meter_register = $this->location_obj->get_id('property', '.entity.1.11');
		...
		$html = <<<HTML
			...
			<html>
				...
				<body>
					...
					Epost: {$this->userSettings['preferences']['common']['email']}<br/>
					Ressursnr: {$ressursnr}
				</body>
			</html>
HTML;
