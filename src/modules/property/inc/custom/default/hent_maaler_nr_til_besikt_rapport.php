...
class entity_data_sync extends property_boentity
{
	...
	function update_data($values, $values_attribute = array(), $action)
	{
		...
		$location_id_maaler = $this->location_obj->get_id('property', '.entity.1.11');
		...
		$location_id_rapport = $this->location_obj->get_id('property', '.entity.2.17');
		$location_id_bod	 = $this->location_obj->get_id('property', '.entity.1.14');
			...
			foreach ($values_attribute as $entry)
			{
				switch ($entry['name'])
				{
					...
					case 'br_slokk_app_skiftet':
						if ($entry['value'])
						{
							$location_id_br_slokk_app	 = $this->location_obj->get_id('property', '.entity.1.10');
							$new_value					 = $entry['value'];
							...
							if ($update_interlink)
							{
								$interlink_data = array(
									'location1_id'		 => $this->location_obj->get_id('property', $this->acl_location),
									'location1_item_id'	 => $values['id'],
									'location2_id'		 => $this->location_obj->get_id('property', '.entity.1.10'),
					...
					case 'roykvarsler':
						...
						{
							$db_dateformat 		= $this->db::date_format();

							//2 = "Skiftet batteri"
							//3 = "Skiftet røykvarsler"

							$location_id_roykvarsler	 = $this->location_obj->get_id('property', '.entity.1.23');
							...
							if ($update_interlink)
							{
								$interlink_data = array(
									'location1_id'		 => $this->location_obj->get_id('property', $this->acl_location),
									'location1_item_id'	 => $values['id'],
									'location2_id'		 => $this->location_obj->get_id('property', '.entity.1.23'),
									'location2_item_id'	 => $id,
									'account_id'		 => $this->account
								);
	...
	{
		$location_id = $this->location_obj->get_id('property', '.entity.1.14');
	...
	{
		$location_id = $this->location_obj->get_id('property', '.entity.1.10');
		$location	 = explode('-', $location_code);
		$value_set	 = array();
	...
	{
		$location_id = $this->location_obj->get_id('property', '.entity.1.23');
		$location	 = explode('-', $location_code);
		$value_set	 = array();
