...
class property_boworkorder
{
	...
	function column_list($selected = array())
	{
		if (!$selected)
		{
			...
			$selected = isset($userSettings['preferences']['property']['workorder_columns']) ? $userSettings['preferences']['property']['workorder_columns'] : '';
		}
		...
		foreach ($history_array as $value)
		{
			...
			switch ($value['status'])
			{
				...
				case 'H':
					...
					break;
				case 'NP':
					$type					 = lang('moved to another project');
					break;
				case 'OA':
					$type					 = lang('order approved');
					break;
				case 'OB':
					$type					 = lang('order approval revoked');
					break;
				default:
			}
	...
	function notify_coordinator_on_consumption($order_id)
	{
		...
		if ($toarray)
		{
			...
			$body	 = '<a href ="' . phpgw::link('/index.php', array(
				...
				'id'		 => $order_id
			), false, true) . '">' . lang('workorder %1 has been edited', $order_id) . '</a>' . "\n";
			$body	 .= <<<HTML
				</br>
