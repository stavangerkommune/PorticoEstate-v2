...
class property_sotts
{
	...
	function read($data)
	{
		...
		if ($query)
		{
			...
			if (ctype_digit($query))
			{
				...
				$querymethod .= " OR fm_tts_tickets.external_ticket_id =" . (int)$query . ')';
			}
			else
			{
				$querymethod .= ')';
		...
		$_return_field_array = array(
			...
			'mail_recipients'		 => 'fm_tts_tickets.mail_recipients',
		);
	...
	function update_ticket(&$ticket, $id = 0, $receipt = array(), $values_attribute = array(), $simple = false)
	{
		...
		{
			$ticket['billable_hours']	 = (float)str_replace(array(' ', ','), array('', '.'), $ticket['billable_hours']);
		...
		if ($order_add || $order_edit)
		{
			if ((int)$ticket['budget'])
			{
				...
				$value_set_cost = array(
					'ticket_id'	 => $id,
					'amount'	 => str_replace(array(' ', ','), array('', '.'), $ticket['budget']),
	...
	function reset_views($id)
	{
		...
		$this->db->query("SELECT status FROM fm_tts_tickets WHERE id =" . (int)$id, __LINE__, __FILE__);
		$this->db->next_record();
		$status = $this->db->f('status');
		...
		$this->db->query("DELETE FROM fm_tts_views WHERE id=" . (int)$id, __LINE__, __FILE__);
