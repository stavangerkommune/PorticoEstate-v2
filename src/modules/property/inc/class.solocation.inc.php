...
class property_solocation
{
	...
	function read_entity_to_link($location_code, $exact = false)
	{
		...
		{
			if (!$this->acl->check('run', ACL_READ, $app))
			{
				continue;
			}
	...
	function read($data)
	{
		...
		$user_columns	 = isset($this->userSettings['preferences']['property']['location_columns_' . $type_id . !!$lookup]) ? $this->userSettings['preferences']['property']['location_columns_' . $type_id . !!$lookup] : array();
		$_user_columns	 = array();
