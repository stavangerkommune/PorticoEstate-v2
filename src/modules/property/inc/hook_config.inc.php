...
function fmtts_assign_group_candidates($config)
{
	...
	$groups			 = $accounts_obj->get_list('groups');
...
function project_status_on_last_order_closed($config)
{
	...
	$status_entries = execMethod('property.bogeneric.get_list', array(
		'type'		 => 'project_status',
...
function list_location_level_otions($config)
{
	...
	$level_assigned	 = isset($config['request_location_level']) ? $config['request_location_level'] : 1;
	$out			 = '';
	foreach ($location_types as $dummy => $level)
	{
		$selected = '';
...
function list_contact_at_location($config)
{
	...
	$entries = execMethod('property.bogeneric.get_list', array(
...
function filter_buildingpart($config)
{
	...
	$locations = array(
		'.project'			 => $translation->translate('project', array(), false, 'property'),
		'.b_account'		 => $translation->translate('accounting', array(), false, 'property'),
		'.project.request'	 => $translation->translate('request', array(), false, 'property'),
	);
