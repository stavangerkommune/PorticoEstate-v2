...
class import_entity_categories
{
	...
	public function add_entity_categories()
	{
		...
		$new_categories	 = Cache::session_get('property', 'new_entity_categories');
		...
		foreach ($new_categories as $building_part => $name)
		{
			...
			if ($category_id)
			{
				$categories['added'][$building_part] = array(
					'id'		 => $category_id, 'entity_id'	 => $this->entity_id_from_template,
					'name'		 => $name
				);
			}
			else
	...
	public function add_attributes_to_categories()
	{
		...
		$building_part_in_table	 = Cache::session_get('property', 'building_part_in_table');
	...
	private function _add_attrib_from_template($values)
	{
		...
		$attrib_group_list = $this->bo->read_attrib_group(array(
		...
		$attrib_list_template	 = $this->bo->read_attrib(array(
			'entity_id'	 => $template_entity_id,
			'cat_id'	 => $template_cat_id, 'allrows'	 => true
		));
	...
	public function prepare_attributes_for_template(&$columns, $attrib_names, $attrib_data_types, $attrib_precision)
	{
		...
		$template_attrib_list	 = $this->bo->read_attrib(array(
			'entity_id'	 => $entity_id,
	...
	public function add_attributes_to_template()
	{
		...
		$attributes	 = Cache::session_get('property', 'new_attribs_for_template');
