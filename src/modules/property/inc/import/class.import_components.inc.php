...
class import_components
{
	...
	{
		$location_id = (int)$this->location_obj->get_id($this->type_app[$this->type], ".{$this->type}.{$entity_id}.{$cat_id}");
	...
	private function _populate($values, $values_attribute)
	{
		...
		if (isset($values_insert['p_num']) && $values_insert['p_num'])
		{
			...
			$p_location_id	 = $this->location_obj->get_id($this->type_app[$this->type], ".{$this->type}.{$values_insert['p_entity_id']}.{$values_insert['p_cat_id']}");
		}
