...
class property_uirequest extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->flags['menu_selection']	 = 'property::project::request';
		Settings::getInstance()->update('flags', ['xslt_app' => true, 'menu_selection' => 'property::project::request']);
		...
		if ($this->nonavbar)
		{
			...
			Settings::getInstance()->update('flags', ['nonavbar' => true, 'noheader_xsl' => true, 'nofooter' => true, 'noframework' => true]);
		}
	}
	...
	public function query()
	{
		...
		$list_descr		 = Sanitizer::get_var('list_descr', 'bool');
	...
	public function handle_multi_upload_file()
	{
		...
		$options['script_url']	 = html_entity_decode(self::link(array(
			'menuaction' => 'property.uirequest.handle_multi_upload_file',
			'id'		 => $id
		)));
	...
	private function _get_filters()
	{
		...
		array_unshift($values_combo_box[$count], array(
	...
	function columns()
	{
		...
		{
			$preferences->add('property', 'request_columns', $values['columns'], 'user');
	...
	function index()
	{
		...
		$apps['manual']['section'] = "general.index.{$this->type}";
		...
		$make_relation	 = Sanitizer::get_var('make_relation', 'bool'); // lookup for maintenance planning
...
JS;
	...
	function priority_key()
	{
		...
		$this->flags['nofooter']		 = true;
		$this->flags['noframework']	 = true;
		Settings::getInstance()->update('flags', ['noheader' => true, 'nofooter' => true, 'noframework' => true]);
		...
		else
		{
			...
			if ($values['notify'])
			{
				...
				if (isset($this->serverSettings['smtp_server']) && $this->serverSettings['smtp_server'])
				{
					...
					$rcpt = $send->msg('email', $values['mail_address'], $subject, stripslashes($message), '', $cc, $bcc, $coordinator_email, $coordinator_name, 'plain');
				}
				else
				{
	...
	function edit($values = array(), $mode = 'edit')
	{
		...
		$jqcal->add_listener('values_consume_date');
		$jqcal->add_listener('values_planning_date');
		...
		{
			$link_survey = phpgw::link('/index.php', array(
				'menuaction' => 'property.uicondition_survey.edit',
				'id'		 => $values['condition_survey_id']
			));
		}
		...
		$data = array(
			...
			'generate_project_action'			 => phpgw::link('/index.php', array(
			...
			//		'location_type'						=> 'form2',
			'form_action'						 => phpgw::link('/index.php', $link_data),
			'done_action'						 => phpgw::link('/index.php', array('menuaction' => 'property.uirequest.index')),
			...
			'multi_upload_action' => phpgw::link('/index.php', array('menuaction' => 'property.uirequest.handle_multi_upload_file', 'id'	 => $id))
				...
				foreach ($_target_section['data'] as $_target_entry)
				{
					switch ($_target_section['location'])
					{
						case '.ticket':
							...
							$start_date	 = $this->phpgwapi_common->show_date($ticket['entry_date'], $this->userSettings['preferences']['common']['dateformat']);
							break;
						case '.project':
							$project	 = execMethod('property.soproject.read_single', (int)$_target_entry['id']);
							$budget		 = $project['budget'];
							$start_date	 = $this->phpgwapi_common->show_date($project['start_date'], $this->userSettings['preferences']['common']['dateformat']);
							$end_date	 = $this->phpgwapi_common->show_date($project['end_date'], $this->userSettings['preferences']['common']['dateformat']);
							break;
						case '.project.workorder':
							$workorder	 = execMethod('property.soworkorder.read_single', (int)$_target_entry['id']);
							$budget		 = $workorder['budget'];
							$start_date	 = $this->phpgwapi_common->show_date($workorder['start_date'], $this->userSettings['preferences']['common']['dateformat']);
							$end_date	 = $this->phpgwapi_common->show_date($workorder['end_date'], $this->userSettings['preferences']['common']['dateformat']);
							break;
						default:
							// nothing
					}
	...
	public function set_value()
	{
		...
		if ($id)
		{
			...
			try
			{
				...
				{
					$values['representative'] = Sanitizer::get_var('value', 'float');
