...
class property_uitenant_claim extends phpgwapi_uicommon_jquery
{
	...
	function __construct()
	{
		...
		$this->flags['menu_selection']	 = 'property::economy::claim';
		Settings::getInstance()->update('flags', ['xslt_app' => true, 'menu_selection' => 'property::economy::claim']);
	...
	function index($project_id = '')
	{
		...
		$jasper = array(); //execMethod('property.sojasper.read', array('location_id' => $this->locations->get_id('property', $this->acl_location)));
	...
	public function _add_links(&$value, $key, $data)
	{
		...
		if ($unset)
		{
			unset($this->serverSettings['webserver_url']);
		}
	}
	...
	function check()
	{
		...
		{
			Cache::message_set(lang('%1 claim is already registered for this project', $total_records), 'message');
			$this->bo->status	 = 'all';
			$this->status		 = 'all';
	...
	function edit($data = array(), $mode = 'edit')
	{
		...
		if ($values['project_id'])
		{
			...
			$project_values['workorder_budget']	 = $this->boproject->get_orders(array(
		...
		{
			$_vouchers	 = array();
			$vouchers			 = $soinvoice->read_invoice(
				array(
					'paid'			 => '1',
		...
		foreach ($location_exceptions as $location_exception)
		{
			...
			Cache::message_set("{$location_exception['severity']}: {$location_exception['location_descr']}", 'message');
		}
		...
		$data = array(
			...
			'status_list'						 => $this->bo->get_status_list(array(
				'format'	 => 'select',
		...
		$this->flags['app_header'] = lang('Tenant claim') . ': ' . $function;
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
