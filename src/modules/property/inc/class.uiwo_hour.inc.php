...
class property_uiwo_hour extends phpgwapi_uicommon_jquery
{
	...
	function __construct()
	{
		...
		Settings::getInstance()->update('flags', ['xslt_app' => true, 'menu_selection' => 'property::project::workorder']);
	...
	public function query_deviation()
	{
		...
		if (isset($list) and is_array($list))
		{
			...
			foreach ($list as $entry)
			{
				...
				$entry_date = (isset($entry['entry_date']) ? $this->phpgwapi_common->show_date($entry['entry_date'], $dateformat) : '');
	...
	function edit_deviation()
	{
		...
		$entry_date = (isset($values['entry_date']) ? $this->phpgwapi_common->show_date($values['entry_date'], $dateformat) : '');
	...
	function common_data($workorder_id, $view = '')
	{
		...
		$workorder_data = array(
			'link_workorder'		 => phpgw::link('/index.php', array(
			...
			'link_project'			 => phpgw::link('/index.php', array(
	...
	function view()
	{
		...
		$preferences = createObject('phpgwapi.preferences', $workorder['user_id']);

		$accounts_obj = new Accounts();
		$from_name	 = $accounts_obj->get($workorder['user_id'])->__toString();
		$from_email	 = "{$from_name}<{$preferences->data['common']['email']}>";
		...
		$ressursnr		 = isset($preferences->data['property']['ressursnr']) ? 'Brukes ikke for denne ordren' : '';
		if ($ressursnr && $workorder['vendor_category'] == 2)
		{
			$ressursnr	 = $preferences->data['property']['ressursnr'];
		}
		...
		$email_data = array(
			...
			'from_phone'				 => $preferences->data['common']['cellphone'],
...
HTML;
			...
			if ($preferences->data['property']['order_email_rcpt'] == 1)
			{
				$bcc = $from_email;
			}
			...
			if (isset($this->serverSettings['smtp_server']) && $this->serverSettings['smtp_server'])
			{
				if (isset($values['file_action']) && is_array($values['file_action']))
				{
					...
					foreach ($attachments as &$_attachment)
					{
						...
						{
							$resized = $this->serverSettings['temp_dir'] . "/resized_{$workorder_id}_" . basename($_attachment['file']);
				...
				try
				{
					$rcpt = $send->msg('email', $_to, $subject, $body, '', $cc, $bcc, $from_email, $from_name, 'html', '', $attachments, $email_receipt);
			...
			if ($rcpt)
			{
				...
				if ($_status)
				{
					try
					{
						execMethod('property.soworkorder.update_status', array(
				...
				if ($this->account != $workorder['user_id'])
				{
					try
					{
						...
						$_address = $this->userSettings['preferences']['common']['email'];

						$_to = $from_email; // reverse...

						$rcpt = $send->msg('email', $_to, $subject, $message, '', '', '', $_address, $this->userSettings['fullname'], 'html');
						if ($rcpt)
						{
		...
		$data = array(
			...
			'requst_email_receipt'				 => isset($preferences->data['request_order_email_rcpt']) && $preferences->data['property']['request_order_email_rcpt'] == 1 ? 1 : 0,
			'send_as_pdf'						 => !empty($this->userSettings['preferences']['property']['send_workorder_as_pdf']) && $this->userSettings['preferences']['property']['send_workorder_as_pdf'] == 1 ? true : false,
	...
	function pdf_order($workorder_id = '', $show_cost = false)
	{
		...
		if (!$workorder_id)
		{
			...
			Settings::getInstance()->update('flags', ['noheader' => $this->flags['noheader'], 'nofooter' => $this->flags['nofooter'], 'xslt_app' => $this->flags['xslt_app']]);
		}
		if (!$show_cost)
		{
			$show_cost = Sanitizer::get_var('show_cost', 'bool');
		...
		$preferences = createObject('phpgwapi.preferences', $common_data['workorder']['user_id']);

		$accounts_obj = new Accounts();
		$from_name = $accounts_obj->get($common_data['workorder']['user_id'])->__toString();

		$ecodimb = !empty($common_data['workorder']['ecodimb']) ? $common_data['workorder']['ecodimb'] : $project['ecodimb'];
		$from	 = lang('date') . ": {$date}\n";
		$from	 .= lang('dimb') . ": {$ecodimb}\n";
		$from	 .= lang('from') . ":\n {$from_name}";
		$from	 .= "\n Epost:  {$preferences->data['common']['email']}";
		$from	 .= "\n Tlf:  {$preferences->data['common']['cellphone']}";

		if (!empty($preferences->data['property']['ressursnr']) && $common_data['workorder']['vendor_category'] == 2)
		{
			$from	 .= "\n Ressursnr: {$preferences->data['property']['ressursnr']}";
		}
	...
	function tender()
	{
		...
		$dateformat	 = $this->userSettings['preferences']['common']['dateformat'];
		$date		 = $this->phpgwapi_common->show_date('', $dateformat);
	...
	{
		//		if (!$this->acl->check('.admin', ACL_ADD, 'property'))
		{
			phpgw::no_access();
		}
	...
	function send_order($workorder_id)
	{
		...
		$from_name	 = $accounts_obj->get($workorder['user_id'])->__toString();
		$from_email	 = "{$from_name}<{$preferences->data['common']['email']}>";
		$bcc		 = !empty($preferences->data['common']['email']) ? $from_email : '';
		...
		try
		{
			$send->msg('email', $_to, $subject, $body, '', $cc, $bcc, $from_email, $from_name, 'html', '', $attachments, $email_receipt);
			Cache::message_set(lang('Workorder %1 is sent by email to %2', $workorder_id, $_to), 'message');
