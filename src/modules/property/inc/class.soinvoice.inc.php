...
" . sprintf($sql2_base, 'fm_ecobilagoverf', $join_tables, $filtermethod, $querymethod);
	...
	public function get_vouchers($data)
	{
		...
		foreach ($values as $bilagsnr => $entry)
		{
			...
			$name		 = sprintf(
				"{$lang_payment_date}: % 10s | {$lang_voucer}:% 8s | {$lang_vendor}: % 50s | {$lang_currency}: % 3s | {$lang_parked}: % 1s | {$lang_type}: % 12s | {$lang_approved_amount}: % 19s | Status: % 1s",
				$payment_date,
				$voucher_id,
				...
				$entry['kreditnota'] ? 'X' : '',
				$entry['type'],
				number_format((float)$entry['godkjentbelop'], 2, ',', ' '),
				$status[0]
			);
