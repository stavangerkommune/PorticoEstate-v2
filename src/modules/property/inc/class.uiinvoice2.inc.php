...
class property_uiinvoice2 extends phpgwapi_uicommon_jquery
{
	...
	var $location_code;
	private $config;
	var $account_id, $bo, $bocommon, $status_id, $allrows, $fields;
	...
	public function reassign_order($line_id, $order_id, $voucher_id)
	{
		...
		if ($this->bo->reassign_order($line_id, $order_id))
		{
			...
			Cache::system_clear('property', "budget_order_{$voucher_info['voucher'][0]['order_id']}"); // target is cleared in the so-class
	...
	function split_voucher()
	{
		...
		$data = array(
			...
			'form_action'	 => phpgw::link('/index.php', array(
				'menuaction' => 'property.uiinvoice2.split_voucher',
				'voucher_id' => $voucher_id
			)),
			'voucher_id'	 => $voucher_id
		);
	...
	private function _split_voucher($voucher_id)
	{
		...
		if ($data)
		{
			...
			{
				Cache::message_set("Feil bilag", 'error');
				$error = true;
			}
			else
			{
	...
	function update_voucher()
	{
		...
		if ($values = Sanitizer::get_var('values'))
		{
			...
			{
				$receipt['error'][] = true;
				Cache::message_set(lang('not a valid category'), 'error');
			}
			if (!$this->bo->check_valid_b_account($values['b_account_id']))
			{
				$receipt['error'][] = true;
				Cache::message_set(lang('not a valid budget account'), 'error');
			}
	...
	function index()
	{
		...
		$userlist_default[]	 = array(
			'id'	 => '*' . $accounts_obj->get($this->account_id)->lid,
		...
		$user = $accounts_obj->get($this->userSettings['id']);
		...
		$version		 = isset($this->serverSettings['versions']['system']) ? $this->serverSettings['versions']['system'] : $this->serverSettings['versions']['phpgwapi'];

		if (isset($this->serverSettings['system_name']))
		{
			$powered_by = $this->serverSettings['system_name'] . ' ' . lang('version') . ' ' . $version;
		}
		else
		{
		...
		$data = array(
			...
			'site_title'			 => "{$this->serverSettings['site_title']}",
			'filter_form'			 => array(
		...
		$this->flags['app_header'] = lang('invoice');
	...
	public function get_vouchers()
	{
		...
		$vouchers = $this->bo->get_vouchers(array(
			'janitor_lid'			 => $janitor_lid, 'supervisor_lid'		 => $supervisor_lid,
	...
	public function get_single_line($line_id = 0)
	{
		...
		$custom_config	 = CreateObject('admin.soconfig', $this->locations->get_id('property', '.invoice'));
		...
		$sign_orig	 = '';
		$my_initials = $this->userSettings['account_lid'];
		...
		if (count($voucher))
		{
			...
			$selected_tax_code = !empty($voucher[0]['tax_code']) ? $voucher[0]['tax_code'] : 0;
			$default_tax_code = $selected_tax_code;
			...
			$approved_list[] = array(
				...
				'date'		 => $voucher[0]['oppsynsigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['oppsynsigndato'])) : '',
			...
			$approved_list[] = array(
				...
				'date'		 => $voucher[0]['saksigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['saksigndato'])) : '',
			...
			$approved_list[] = array(
				...
				'date'		 => $voucher[0]['budsjettsigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['budsjettsigndato'])) : '',
			...
			array_unshift($voucher_info['generic']['approve_list']['options'], array(
				'id'	 => '',
			...
			$voucher[0]['invoice_date']		 = $voucher[0]['invoice_date'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['invoice_date']), $dateformat) : '';
			$voucher[0]['payment_date']		 = $voucher[0]['payment_date'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['payment_date']), $dateformat) : '';
			$voucher[0]['oppsynsigndato']	 = $voucher[0]['oppsynsigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['oppsynsigndato']), $dateformat) : '';
			$voucher[0]['saksigndato']		 = $voucher[0]['saksigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['saksigndato']), $dateformat) : '';
			$voucher[0]['budsjettsigndato']	 = $voucher[0]['budsjettsigndato'] ? $this->phpgwapi_common->show_date(strtotime($voucher[0]['budsjettsigndato']), $dateformat) : '';
		...
		array_unshift($voucher_info['generic']['dimb_list']['options'], array(
			'id'	 => '',
			'name'	 => lang('select')
		));
		array_unshift($voucher_info['generic']['dime_list']['options'], array(
			'id'	 => '',
			'name'	 => lang('select')
		));
