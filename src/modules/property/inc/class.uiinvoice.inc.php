...
class property_uiinvoice extends phpgwapi_uicommon_jquery
{
	...
	private function _get_Filters()
	{
		...
		array_unshift($values_combo_box[1], array(
			'id'	 => $accounts_obj->get($this->account)->lid,
	...
	function index()
	{
		...
		if (!$start_date)
		{
			$start_date = $this->phpgwapi_common->show_date(mktime(0, 0, 0, '01', '01', date("Y")), $this->userSettings['preferences']['common']['dateformat']);
		}
		if (!$end_date)
		{
			//-- fecha actual
			$end_date = $this->phpgwapi_common->show_date(mktime(0, 0, 0, date("m"), date("d"), date("Y")), $this->userSettings['preferences']['common']['dateformat']);
		}
		//-- edicion de registro
...
JS;
	...
	public function query()
	{
		...
		$link_sub = phpgw::link('/index.php', array(
			'menuaction' => 'property.uiinvoice.list_sub',
			'user_lid'	 => $this->user_lid
		));
...
//		}
	...
	public function query_consume()
	{
		...
		$actual_date = $this->phpgwapi_common->show_date(mktime(0, 0, 0, date("m"), date("d"), date("Y")), $dateformat);
		...
		foreach ($content as &$entry)
		{
			...
			$entry['link_voucher']	 = urldecode(phpgw::link(
				'/index.php',
				array(
					'menuaction'		 => 'property.uiinvoice.index',
					'paid'				 => true,
	...
	public function edit()
	{
		...
		$approved_list[] = array(
			...
			'date'		 => $line['oppsynsigndato'] ? $this->phpgwapi_common->show_date(strtotime($line['oppsynsigndato'])) : ''
		);
		$approved_list[] = array(
			'role'		 => $role_check['is_supervisor'],
			'initials'	 => $line['supervisor'] ? $line['supervisor'] : '',
			'date'		 => $line['saksigndato'] ? $this->phpgwapi_common->show_date(strtotime($line['saksigndato'])) : ''
		);
		$approved_list[] = array(
			'role'		 => $role_check['is_budget_responsible'],
			'initials'	 => $line['budget_responsible'] ? $line['budget_responsible'] : '',
			'date'		 => $line['budsjettsigndato'] ? $this->phpgwapi_common->show_date(strtotime($line['budsjettsigndato'])) : ''
		);
	...
	function consume()
	{
		...
		{
			//-- actual date
			$start_date	 = $this->phpgwapi_common->show_date(mktime(0, 0, 0, date("m"), date("d"), date("Y")), $dateformat);
			$end_date	 = $start_date;
		}
...
JS;
	...
	function add()
	{
		...
		{
			$link_receipt = phpgw::link('/index.php', array(
				'menuaction' => 'property.uiinvoice.receipt',
				'voucher_id' => $receipt['voucher_id']
			));
		}

		Cache::session_clear('add_receipt', 'session_data');
		...
		{
			Cache::session_clear('add_values', 'session_data');
		}

		$values = Cache::session_get('add_values', 'session_data');

		if (!$values && Sanitizer::get_var('add_invoice', 'bool'))
		{
			$values = array();
			...
			Cache::session_set('add_values', 'session_data', $values);
		}
		else
		{
			Cache::session_clear('add_values', 'session_data');
		...
		if ($add_invoice && is_array($values))
		{
			...
			if (!is_array($receipt['error']))
			{
				...
				{
					unset($values);
					$receipt = Cache::session_set('add_receipt', 'session_data', $receipt);
	...
	function receipt()
	{
		...
		/*
			  ...
			  $pdf->ezText(lang('invoice date') . ' ' . $this->phpgwapi_common->show_date(strtotime($values[0]['invoice_date']),$dateformat) ,14);
			  $pdf->ezText(lang('Payment date') . ' ' . $this->phpgwapi_common->show_date(strtotime($values[0]['payment_date']),$dateformat) ,14);
		...
		$content_heading[]	 = array(
			...
			'value'	 => $this->phpgwapi_common->show_date(strtotime($values[0]['invoice_date']), $dateformat)
		);
		$content_heading[]	 = array(
			'text'	 => lang('Payment date'),
			'value'	 => $this->phpgwapi_common->show_date(strtotime($values[0]['payment_date']), $dateformat)
		);
		$content_heading[]	 = array(
			'text'	 => lang('Janitor'),
