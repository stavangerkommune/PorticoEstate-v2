...
class property_uis_agreement extends phpgwapi_uicommon_jquery
{
	...
	function import()
	{
		...
		$active_tab = Sanitizer::get_var('tab', 'string', 'POST', 'items');

		$import = CreateObject('property.import');

		$importfile	 = $import->importfile();
		$id			 = Sanitizer::get_var('id');
		if (isset($importfile) && is_file($importfile) && !Sanitizer::get_var('cancel'))
		{
			...
			else
			{
				$import_action	 = phpgw::link('/index.php', array(
					'menuaction' => 'property.uis_agreement.import',
					'id'		 => $id
				));
	...
	{
		$s_agreement_id = Sanitizer::get_var('s_agreement_id', 'int');
	...
	function deleteitem()
	{
		$item_id = Sanitizer::get_var('id', 'int');
		$id		 = Sanitizer::get_var('s_agreement_id', 'int');
			...
			if ($type_alarm == 'update' || $type_alarm == 'update_item')
			{
				...
				if ($type_alarm == 'update')
				{
					$requestUrl = json_encode(self::link(array(
						'menuaction'		 => 'property.uis_agreement.get_content',
	...
	function edit($values = array())
	{
		...
		//			$active_tab		= Sanitizer::get_var('tab', 'string', 'REQUEST', 'general');
		...
		$vendor_data = $this->bocommon->initiate_ui_vendorlookup(
			array(
		...
		$alarm_data = $this->bocommon->initiate_ui_alarm(
			array(
				'acl_location'	 => $this->acl_location,
				'alarm_type'	 => 's_agreement',
				'type'			 => 'form',
		...
		$myColumnDefs0 = array(
			array(
				'key'		 => 'time', 'label'		 => $alarm_data['header'][0]['lang_time'],
				'sortable'	 => true,
				'resizeable' => true, 'width'		 => 140
			),
			array(
				'key'		 => 'text', 'label'		 => $alarm_data['header'][0]['lang_text'],
				'sortable'	 => true,
				'resizeable' => true, 'width'		 => 340
			),
			array(
				'key'		 => 'user', 'label'		 => $alarm_data['header'][0]['lang_user'],
				'sortable'	 => true,
				'resizeable' => true, 'width'		 => 200
			),
			array(
				'key'		 => 'enabled', 'label'		 => $alarm_data['header'][0]['lang_enabled'],
				'sortable'	 => true, 'resizeable' => true, 'formatter'	 => 'JqueryPortico.FormatterCenter',
				'width'		 => 60
			),
			array(
				'key'		 => 'alarm_id', 'label'		 => "dummy", 'sortable'	 => true, 'resizeable' => true,
				'hidden'	 => true
			)
		);
		...
		$tabletools = array(
			array(
				...
				'action'	 => phpgw::link('/index.php', array(
					...
					's_agreement_id' => $id, 'from'			 => $view_only ? 'view' : 'edit'
				)), 'parameters' => json_encode($parameters)
			),
			array(
				'my_name'	 => 'edit', 'text'		 => lang('Edit'),
				'action'	 => phpgw::link('/index.php', array(
					'menuaction'	 => 'property.uis_agreement.edit_item',
					's_agreement_id' => $id
				)), 'parameters' => json_encode($parameters)
			...
			array(
				'my_name'	 => 'delete', 'text'		 => lang('Delete'),
				'action'	 => phpgw::link('/index.php', array(
					'menuaction'	 => 'property.uis_agreement.deleteitem',
					's_agreement_id' => $id
				)), 'parameters' => json_encode($parameters)
			),
		...
		$myColumnDefs3 = array(
			...
			array(
				'key'		 => 'budget_account', 'label'		 => lang('budget account'), 'sortable'	 => false,
				'resizeable' => true
			),
		...
		$data = array(
			...
			'td_count'							 => $td_count,
			'base_java_url'						 => json_encode(array(
				'menuaction' => "property.uis_agreement.edit",
				'id'		 => $id
			)),
			'link_import'						 => phpgw::link('/index.php', array(
				'menuaction' => 'property.uis_agreement.import',
				'tab'		 => 'items'
			)),
			...
			'cancel_url'						 => phpgw::link('/index.php', array('menuaction' => 'property.uis_agreement.index')),
			'lang_id'							 => lang('ID'),
	...
	{
		$s_agreement_id	 = Sanitizer::get_var('s_agreement_id', 'int');
		$id				 = Sanitizer::get_var('item_id', 'int');
	...
	function edit_item()
	{
		...
		$s_agreement_id	 = Sanitizer::get_var('s_agreement_id'); // in case of bigint
		$id				 = Sanitizer::get_var('id', 'int');
		$values			 = Sanitizer::get_var('values');
		$delete_last	 = Sanitizer::get_var('delete_last', 'bool', 'GET');
		...
		//			phpgwapi_xslttemplates::getInstance()->add_file(array('s_agreement','attributes_form'));
		...
		$s_agreement = $this->bo->read_single(array(
			's_agreement_id' => $s_agreement_id,
			'view'			 => true
		));
		$values		 = $this->bo->read_single_item(array(
			's_agreement_id' => $s_agreement_id,
			'id'			 => $id
		));
	...
	function view_item()
	{
		...
		$myColumnDefs0 = array(
			array(
				'key'		 => 'item_id', 'label'		 => $table_header[0]['header'], 'sortable'	 => true,
				'resizeable' => true, 'width'		 => 140
			),
			array(
				'key'		 => 'cost', 'label'		 => $table_header[2]['header'], 'sortable'	 => true,
				'resizeable' => true, 'width'		 => 340
			),
			array(
				'key'		 => 'this_index', 'label'		 => $table_header[3]['header'], 'sortable'	 => true,
				'resizeable' => true, 'width'		 => 200
			),
			array(
				'key'		 => 'index_count', 'label'		 => $table_header[4]['header'], 'sortable'	 => true,
				'resizeable' => true, 'formatter'	 => 'JqueryPortico.FormatterCenter', 'width'		 => 60
			),
			array(
				'key'		 => 'index_date', 'label'		 => $table_header[5]['header'], 'sortable'	 => true,
				'resizeable' => true, 'formatter'	 => 'JqueryPortico.FormatterCenter', 'width'		 => 60
			)
		);
	...
	function view()
	{
		...
		$tabletools = array(
			array(
				...
				'action'	 => phpgw::link('/index.php', array(
					'menuaction'	 => 'property.uis_agreement.view_item',
					's_agreement_id' => $s_agreement_id, 'from'			 => 'view'
				)), 'parameters' => json_encode($parameters)
			),
		);
