...
class property_uiqr_generator extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		self::set_active_menu('admin::property::qr_generator');
	}
	...
	private function _handle_import()
	{
		...
		if (!$step)
		{
			if ($cached_file = Cache::session_get('property', 'qr_generator_import_file'))
			{
				Cache::session_clear('property', 'qr_generator_import_file');
				unlink($cached_file);
				unset($cached_file);
			}
		}
		...
		if ($step > 1)
		{
			$cached_file = Cache::session_get('property', 'qr_generator_import_file');
		}
		...
		if ($step == 1 || isset($_FILES['import_file']['tmp_name']))
		{
			...
			Cache::session_set('property', 'qr_generator_import_file', $cached_file);
			$step		 = 1;
		}
		...
		switch ($step)
		{
			case 0:
				...
				$tabs['step_3']	 = array(
					'label'		 => lang('choose start line'), 'link'		 => null,
					'disable'	 => true
				);
				$tabs['step_4']	 = array(
					'label'		 => lang('choose columns'), 'link'		 => null,
					'disable'	 => true
				);
				$tabs['step_5']	 = array(
					'label'		 => lang('completed'), 'link'		 => null,
					'disable'	 => true
				);
				break;
			case 1:
		...
		$this->flags['app_header'] = lang('property') . '::' . lang('qr-generator');
