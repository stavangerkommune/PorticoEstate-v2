...
class property_uilookup extends phpgwapi_uicommon_jquery
{
	...
	function addressbook()
	{
		...
		$default_category = $this->userSettings['preferences']['addressbook']['default_category'];
...
JS;
		...
		$uicols = array(
			...
			'formatter'	 => array('', '', '', '', '', '', ''),
			'descr'		 => array(
				lang('ID'), lang('lid'), lang('lastname'), lang('firstname'),
				lang('email'), lang('mobile')
			)
		);
	...
	function organisation()
	{
		...
		$values_combo_box = $this->cats->formatted_xslt_list(array(
	...
	function vendor()
	{
		$default_category = $this->userSettings['preferences']['property']['default_vendor_category'];
...
JS;
	...
	function b_account()
	{
		...
		if ($role != 'group')
		{
			$values_combo_box	 = execMethod('property.bogeneric.get_list', array(
				'type'		 => 'b_account',
