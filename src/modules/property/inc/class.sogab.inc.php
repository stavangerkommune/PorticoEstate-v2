...
class property_sogab
{
	...
	function read($data)
	{
		...
		$location_id = $locations_obj->get_id('property', '.location.gab');
				...
				foreach ($custom_cols as $custom_col)
				{
					if ($custom_value = $this->db->f($custom_col['column_name'], true))
					{
						$custom_value = $this->custom->get_translated_value(array(
							'value'		 => $custom_value,
							'attrib_id'	 => $custom_col['id'], 'datatype'	 => $custom_col['datatype']
						), $location_id);
					}
	...
	function read_detail($data)
	{
		...
		$sql = $this->bocommon->generate_sql(array(
			'entity_table'	 => $entity_table, 'cols'			 => $cols,
