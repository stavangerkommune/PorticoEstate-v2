...
class property_uiimport_documents extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->acl_manage	 = $this->acl->check($this->acl_location, ACL_PRIVATE, 'property'); //16
		...
		$this->flags['menu_selection']	 = "property::documentation::import_documents";
		...
		else
		{
			...
			$this->path_upload_dir	 = $this->serverSettings['files_dir'] . $fakebase;
		}
	...
	public function download()
	{
		...
		$custom_frontend = !empty($this->flags['custom_frontend']) ? true : false;
	...
	public function step_1_import()
	{
		...
		$this->flags['app_header'] = lang('property') . '::' . lang('import documents');
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
	...
	function validate_info()
	{
		...
		foreach ($list_files as &$file_info)
		{
			...
			if (
				!$file_info['document_category_validate']
		...
		$maxmatchs = !empty($this->userSettings['preferences']['common']['maxmatchs'])  ? (int)$this->userSettings['preferences']['common']['maxmatchs'] : 15;
