...
class property_uijasper extends phpgwapi_uicommon_jquery
{
	...
	function __construct()
	{
		...
		$this->flags['menu_selection']	 = 'property::jasper';
		...
		if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
		{
			...
			if (!$receipt['error'])
			{
				try
				{
					...
					{
						Cache::session_set('jasper_receipt', 'session_data', $receipt);
						phpgw::redirect_link('/index.php', array(
							'menuaction' => 'property.uijasper.index',
							'app'		 => $this->app
						));
	...
	function edit()
	{
		...
		$locations			 = $this->locations->get_locations(false, $this->app);
		...
		$type_def = array(
			...
			array(
				'key'		 => 'is_id', 'label'		 => lang('is id'), 'sortable'	 => false, 'resizeable' => true,
		...
		{
			$type_def[] = array(
				'key'		 => 'delete_input', 'label'		 => lang('delete'), 'sortable'	 => false,
		...
		$this->flags['app_header'] = lang('property') . "::{$appname}::$function_msg::" . lang($this->app);
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
	...
	function view()
	{
		...
		else
		{
			foreach ($values['input'] as &$input)
			{
				...
				else
				{
					...
					{
						$input['value'] = date(Db::getInstance()->date_format(), strtotime($input['value']));
					}
		...
		if (!$user_input)
		{
			...
			$report_source	 = "{$this->serverSettings['files_dir']}/property/jasper/{$jasper_id}/{$values['file_name']}";
