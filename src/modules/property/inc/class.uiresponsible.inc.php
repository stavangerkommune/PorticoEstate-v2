...
class property_uiresponsible extends phpgwapi_uicommon_jquery
{
	...
	var $bolocation, $acl_delete, $bocommon, $cats, $location, $cat_id;
	...
	function __construct()
	{
		...
		self::set_active_menu("admin::{$this->appname}::responsible_matrix");
	}
	 ...
	 * @return array
	 */
	private function _get_Filter(): array
	{
		$values_combo_box	 = array();
		$combos				 = array();

		$locations = $this->locations->get_locations(false, $this->appname, false, false, true);
	...
	function index()
	{
		...
		if ($lookup)
		{
			...
			$this->flags['headonly']		 = true;
			Settings::getInstance()->update('flags', ['noframework' => true, 'headonly' => true]);
		}
		...
		$this->flags['app_header'] = lang('responsible matrix') . ":: {$function_msg}";
		...
		if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
		{
			...
			if (!$receipt['error'])
			{
				try
				{
					...
					{
						Cache::session_set('responsible_receipt', 'session_data', $receipt);
	...
	function edit()
	{
		...
		{
			$locations_info		 = $this->locations->get_name($values['location_id']);
		...
		{
			$_location_info		 = $this->locations->get_name($module['location_id']);
		...
		$datatable_def[] = array(
			...
			'ColumnDefs' => array(
				...
				array(
					'key'		 => 'location', 'label'		 => lang('location'), 'sortable'	 => true,
					'resizeable' => true
				),
				array(
					'key'		 => 'category', 'label'		 => lang('category'), 'sortable'	 => true,
					'resizeable' => true
				),
				array('key' => 'active', 'label' => lang('active'), 'sortable' => true, 'resizeable' => true),
				array(
					'key'		 => 'delete_module', 'label'		 => lang('delete'), 'sortable'	 => false,
		...
		$this->flags['app_header'] = lang('property') . "::{$appname}::$function_msg::" . lang($this->appname);
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		phpgwapi_xslttemplates::getInstance()->add_file(array('responsible', 'datatable_inline'));
	...
	function edit_role()
	{
		...
		$this->flags['app_header'] = lang('property') . "::{$appname}::$function_msg::" . lang($this->appname);
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		phpgwapi_xslttemplates::getInstance()->add_file(array('responsible'));
		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('edit_role' => $data));
	}
	...
	public function contact()
	{
		...
		phpgwapi_xslttemplates::getInstance()->add_file(array('responsible', 'nextmatchs', 'search_field'));
		...
		$table_add[] = array(
			...
			'add_action'			 => phpgw::link('/index.php', $link_add_action),
			'lang_cancel'			 => lang('cancel'),
			'lang_cancel_statustext' => lang('back to list type'),
			'cancel_action'			 => phpgw::link(
				'/index.php',
				array(
					'menuaction' => 'property.uiresponsible.index',
					'appname'	 => $this->appname
		...
		$receipt	 = Cache::session_get('responsible_contact_receipt', 'session_data');
		$msgbox_data = $this->phpgwapi_common->msgbox_data($receipt);
		Cache::session_clear('responsible_contact_receipt', 'session_data');
		...
		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('list_contact' => $data));
	...
	public function edit_contact()
	{
		...
		$contact_id			 = Sanitizer::get_var('contact', 'int');
		$contact_name		 = Sanitizer::get_var('contact_name', 'string');
		$responsibility_id	 = Sanitizer::get_var('responsibility_id', 'int');
		$responsibility_name = Sanitizer::get_var('responsibility_name', 'string');
		...
		if (isset($values) && is_array($values))
		{
			...
			if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
			{
				...
				if (!isset($receipt['error']) || !$receipt['error'])
				{
					...
					if (isset($values['save']) && $values['save'])
					{
						Cache::session_set('responsible_contact_receipt', 'session_data', $receipt);
						phpgw::redirect_link('/index.php', array(
							'menuaction' => 'property.uiresponsible.contact',
							'appname'	 => $this->appname,
							'location'	 => $this->location,
		...
		$this->flags['java_script'] .= $lookup_function;
		Settings::getInstance()->update('flags', ['java_script' => $this->flags['java_script']]);

		$jqcal = CreateObject('phpgwapi.jqcal');
		$jqcal->add_listener('values_active_from');
		$jqcal->add_listener('values_active_to');
		...
		$this->flags['app_header'] = lang('responsible matrix') . "::{$function_msg}";
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('edit_contact' => $data));
	}
