...
class send_lekeplasskontroller extends property_cron_parent
{
	...
	function send_report($check_list_id)
	{
		...
		{
			Cache::message_set("Missing recipient email address for {$component['navn']}, id {$component_id}", 'error');
			return false;
		}
...
HTML;
		...
		$from_email	 = isset($this->config['email_sender']) && $this->config['email_sender'] ? $this->config['email_sender'] : "noreply<noreply@{$this->serverSettings['hostname']}>";
