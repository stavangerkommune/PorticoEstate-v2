...
class Import_fra_agresso_X205 extends property_cron_parent
{
	...
	var $responsible, $bocommon, $dateformat, $datetimeformat, $config, $send, $accounts_obj;
	...
	function __construct()
	{
		...
		include(PHPGW_SERVER_ROOT . "/property/inc/export/{$this->userSettings['domain']}/Agresso_X114");
		$this->export = new export_conv;

		$now = time() + (int)$this->userSettings['preferences']['common']['tz_offset'] * 3600;
	...
	protected function remind()
	{
		...
		if ((int)$this->serverSettings['invoice_mail_reminder_time'] < (time() - (3600 * 24)))
		{
			$toarray = array();
			...
			{
				$prefs = $this->bocommon->create_preferences('common', $this->accounts_obj->name2id($lid));
			...
			else if (!$this->db->next_record())
			{
				...
				if ($to && !$this->skip_email)
				{
					...
					$body	 .= '<a href ="' . phpgw::link('/index.php', array(
						...
						'appname'	 => 'property', 'type'		 => 'vendor'
					), false, true) . '">Link til å legge inn ny leverandør</a>';
		...
		else
		{
			...
			if ($order_info['toarray'] && (!$this->skip_email || !$this->debug))
			{
				...
				if (isset($this->serverSettings['smtp_server']) && $this->serverSettings['smtp_server'])
				{
					...
					$body	 = '<a href ="' . phpgw::link('/index.php', array(
						'menuaction' => 'property.uiinvoice2.index',
						'voucher_id' => $bilagsnr, 'query'		 => $bilagsnr, 'user_lid'	 => 'all'
					), false, true) . '">Link til fakturabehandling</a>';
			...
			Db::getInstance()->Exception_On_Error = true;
			...
			Db::getInstance()->Exception_On_Error = false;
			return $bilagsnr;
		}
	}
	...
	function get_order_info($order_id = '')
	{
		...
		{
			$order_info['janitor']	 = $this->accounts_obj->get($janitor_user_id)->lid;
		}
		else
		{
			$order_info['janitor']	 = '';
		...
		{
			$order_info['supervisor'] = $this->accounts_obj->get($supervisor_user_id)->lid;
		}

		$budget_responsible_user_id = $this->invoice->get_default_dimb_role_user(3, $order_info['dimb']);
		if ($budget_responsible_user_id)
		{
			$order_info['budget_responsible'] = $this->accounts_obj->get($budget_responsible_user_id)->lid;
		}
