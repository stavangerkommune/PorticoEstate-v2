...
class deceased_tenants_BK_EBF extends property_cron_parent
{
	...
	function execute()
	{
		...
		// max. once each day
		if ((int)$this->serverSettings['last_executed_get_tenenants_time'] < (time() - (3600 * 24)))
		{
	...
	function handle_deceased($checklist)
	{
		...
		$cols = array(
			'navn', 'f_dato', 'd_dato', 'BOEI_adresse', 'objekt_id', 'leie_id',
			'flyttenr'
		);
...
class fiks
{
	...
	public function __construct()
	{
		$location_obj = CreateObject('phpgwapi.locations');
