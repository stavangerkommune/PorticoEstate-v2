...
class synkroniser_avdelinger_med_fellesdata extends property_cron_parent
{
	...
	function execute()
	{
		...
		/*
			 ...
			 * tjeneste
			 */
		if (isset($this->userSettings['apps']['rental']))
		{
...
class property_fellesdata
{
	...
	function __construct()
	{
		 ...
		 */
		$config				 = CreateObject('admin.soconfig', $this->location_obj->get_id('property', '.ticket'));
		...
		$this->portico_db = Db::getInstance();
	...
	private function initiate_config()
	{
		...
		$receipt = $this->config->add_attrib(
			array(
		...
		phpgw::redirect_link('/index.php', array(
			...
			'section_id'	 => $receipt_section['section_id'], 'location_id'	 => $this->location_obj->get_id('property', '.admin')
		));
	}
	...
	public function insert_values()
	{
		...
		foreach ($units as $unit)
		{
			$value_set = array(
				...
				'created_on'	 => time(),
				'created_by'	 => $this->userSettings['account_id'],
				'modified_by'	 => $this->userSettings['account_id'],
				'modified_on'	 => time()
			);
	...
	function update_customer_id()
	{
		...
		$this->portico_db->query($sql, __LINE__, __FILE__);

		$parties = array();
		while ($this->portico_db->next_record())
		{
			$parties[] = array(
				'id'		 => $this->portico_db->f('id'),
				'identifier' => $this->portico_db->f('identifier')
			);
		}
	...
	function update_agresso_prosjekt()
	{
		...
		if ($values)
		{
			$this->portico_db->query("UPDATE fm_external_project SET active = 0", __LINE__, __FILE__);
		}

		foreach ($values as $entry)
		{
			$active = $entry['status'] == 'C' ? 0 : 1;
			$this->portico_db->query("SELECT id FROM fm_external_project WHERE id ='{$entry['dimValue']}'", __LINE__, __FILE__);
			if ($this->portico_db->next_record())
			{
				$sql = "UPDATE fm_external_project SET name = '{$entry['dimValue']} {$entry['description']}', active = {$active} WHERE id = '{$entry['dimValue']}'";
			}
			else
			{
				$name	 = $this->portico_db->db_addslashes("{$entry['dimValue']} {$entry['description']}");
	...
	function update_tjeneste()
	{
		...
		if ($values)
		{
			$this->portico_db->query("UPDATE fm_eco_service SET active = 0", __LINE__, __FILE__);
		}

		foreach ($values as $entry)
		{
			$active = $entry['status'] == 'C' ? 0 : 1;
			$this->portico_db->query("SELECT id FROM fm_eco_service WHERE id =" . (int)$entry['dimValue'], __LINE__, __FILE__);
			if ($this->portico_db->next_record())
			{
	...
	function update_vendor()
	{
		...
		$metadata = $this->portico_db->metadata('fm_vendor_temp');
...
SQL;
			$this->portico_db->query($sql_table, __LINE__, __FILE__);
		}
		$this->portico_db->query('DELETE FROM fm_vendor_temp', __LINE__, __FILE__);
		...
		$this->portico_db->transaction_begin();
		...
		if ($valueset && !$error)
		{
			$this->portico_db->insert($sql, $valueset, __LINE__, __FILE__);
		}
		...
		while ($this->portico_db->next_record())
		{
			$vendors[] = array(
				1	 => array(
					'value'	 => (int)$this->portico_db->f('id'),
					'type'	 => PDO::PARAM_INT
				),
				2	 => array(
					'value'	 => $this->portico_db->f('navn'),
					'type'	 => PDO::PARAM_STR
				),
				3	 => array(
					'value'	 => 1,
				...
				5	 => array(
					'value'	 => (int)$this->portico_db->f('aktiv'),
					'type'	 => PDO::PARAM_INT
				),
				6	 => array(
					'value'	 => $this->portico_db->f('adresse'),
					'type'	 => PDO::PARAM_STR
				),
				7	 => array(
					'value'	 => $this->portico_db->f('postnummer'),
					'type'	 => PDO::PARAM_STR
				),
				8	 => array(
					'value'	 => $this->portico_db->f('sted'),
					'type'	 => PDO::PARAM_STR
				),
				9	 => array(
					'value'	 => $this->portico_db->f('organisasjonsnr'),
					'type'	 => PDO::PARAM_STR
				),
				10	 => array(
					'value'	 => $this->portico_db->f('bankkontonr'),
					'type'	 => PDO::PARAM_STR
				)
			);
		}
		...
		if ($vendors && !$error)
		{
			$this->portico_db->insert($sql, $vendors, __LINE__, __FILE__);

			$this->portico_db->query("UPDATE fm_vendor SET active = 0", __LINE__, __FILE__);

			$this->portico_db->query("UPDATE fm_vendor SET"
				. " active = 1,"
		...
		$this->portico_db->transaction_commit();
	}
	...
	function update_dimb()
	{
		...
		while ($db->next_record())
		{
			$values[] = array(
				'id'			 => (int)$db->f('ANSVAR'),
				'descr'			 => $this->portico_db->db_addslashes($db->f('BESKRIVELSE', true)),
		...
		foreach ($values as $entry)
		{
			$this->portico_db->query("SELECT id FROM fm_ecodimb WHERE id = {$entry['id']}", __LINE__, __FILE__);
			if ($this->portico_db->next_record())
			{
