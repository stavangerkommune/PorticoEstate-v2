...
class import_epostlister_lrs extends property_cron_parent
{
	...
	function update_email($values = array())
	{
		...
		$metadata			 = $this->db->metadata('phpgw_helpdesk_email_out_recipient_list_temp');
		$create_temp_table	 = false;

		if ($metadata && empty($metadata['alias']))
		{
			$this->db->query('DROP TABLE public.phpgw_helpdesk_email_out_recipient_list_temp', __LINE__, __FILE__);
...
SQL;
		...
		$this->db->query('DELETE FROM phpgw_helpdesk_email_out_recipient_list_temp', __LINE__, __FILE__);
		...
		{
			$set_id				 = (int)$this->db->f('set_id');
			$alias				 = $this->db->f('alias');
			$name				 = $this->db->f('name');
			$email				 = $this->db->f('email');
			$office				 = $this->db->f('office');
			$department			 = $this->db->f('department');
			$alias_supervisor	 = $this->db->f('alias_supervisor');
			$name_supervisor	 = $this->db->f('name_supervisor');
			$email_supervisor	 = $this->db->f('email_supervisor');
		...
		{
			$this->db->insert($sql, $valueset_diff, __LINE__, __FILE__);
		}

		if ($valueset && !$error)
		{
			$this->db->query("UPDATE phpgw_helpdesk_email_out_recipient_list SET active = 0", __LINE__, __FILE__);

			$ok = $this->db->query("UPDATE phpgw_helpdesk_email_out_recipient_list SET"
				. " active = 1,"
	...
	private function register_new_users()
	{
		...
		while ($this->db->next_record())
		{
			...
			$values[$alias_supervisor]	 = array(
				...
				'name'		 => $this->db->f('name_supervisor'),
				'email'		 => $this->db->f('email_supervisor'),
				'supervisor' => true
			);
		}
...
class helpdesk_account extends helpdesk_hook_helper
{
	...
	public function register_accounts($values)
	{
		...
		{
			if (!$accounts_obj->exists($account_lid))
			{
				...
				if ($autocreate_user)
				{
					...
					if ($fellesdata_user && $fellesdata_user['firstname'])
					{
						...
						$group_lid			 = $accounts_obj->id2lid($default_group_id);
