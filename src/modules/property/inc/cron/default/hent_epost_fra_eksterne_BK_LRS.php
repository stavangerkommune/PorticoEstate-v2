...
class hent_epost_fra_eksterne_BK_LRS extends property_cron_parent
{
	...
	public function __construct()
	{
		...
		$system_user_id	 = $accounts_obj->name2id($system_user_lid);
		if ($system_user_id)
		{
			Settings::getInstance()->update('user', ['account_id' => $system_user_id]);
			$preferences = createObject('phpgwapi.preferences', $system_user_id);
		...
		$this->config = CreateObject('admin.soconfig', $this->location_obj->get_id('helpdesk', '.admin'));
