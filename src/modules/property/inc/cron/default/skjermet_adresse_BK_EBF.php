...
class skjermet_adresse_BK_EBF extends property_cron_parent
{
	...
	function execute()
	{
		...
		$cols = array(
			'navn', 'f_dato', 'beste_adresse', 'objekt_id', 'leie_id',
			'flyttenr'
		);
...
HTML;
		...
		$toarray = array(
			'hc483@bergen.kommune.no', 'Bjørvik, Ole Christian <Ole.Bjorvik@bergen.kommune.no>',
...
class boei
{
	...
	public function get_db()
	{
		...
		try
		{
			$this->db = new Db2($boei_dsn, $external_db['boei']['db_user'], $external_db['boei']['db_pass'], $options);
			$this->db->set_config($external_db['boei']);
			$this->connected = true;
		}
		catch (Exception $e)
		{
