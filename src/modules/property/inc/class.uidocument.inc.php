...
class property_uidocument extends phpgwapi_uicommon_jquery
{
	...
	function __construct()
	{
		...
		if ($this->entity_id)
		{
			$this->flags['menu_selection'] .= "::entity_{$this->entity_id}";
		}
		else
		{
			$this->flags['menu_selection'] .= '::location';
		}
	...
	function index()
	{
		...
		if ($this->acl_add)
		{
			$data['datatable']['new_item'] = self::link(
				array(
	...
	public function query_at_location()
	{
		...
		foreach ($values as &$document_entry)
		{
			...
			else
			{
				if (!$link_to_files)
				{
					$link_view_file	 = phpgw::link('/index.php', array(
	...
	function list_doc()
	{
		...
		$data = array(
			...
			'datatable'		 => array(
				...
				'download'	 => self::link(
					array(
						'menuaction' => 'property.uidocument.download',
						'doc_type'	 => $this->doc_type,
						'entity_id'	 => $this->entity_id,
						'location_code'	 => $location_code,
						'export'	 => true,
	...
	function edit()
	{
		...
		else
		{
			...
			{
				$_values		 = execMethod('property.soentity.read_single', array(
					'entity_id'	 => $p_entity_id,
					'cat_id'	 => $p_cat_id, 'id'		 => Sanitizer::get_var('p_num')
				));
		...
		if ($values['save'] || $values['apply'])
		{
			...
			else if ($receipt['error'] && Sanitizer::get_var('phpgw_return_as') == 'json')
			{
				return array(
					'status'		 => 'error',
					'id'			 => null,
			...
			if (!$receipt['error'])
			{
				...
				Cache::session_set('document_receipt', 'session_data', $receipt);

				$redirect_link = phpgw::link('/index.php', array(
					'menuaction'	 => $_from ?  $from : 'property.uidocument.list_doc',
		...
		$jqcal->add_listener('values_document_date');
		...
		$data = array(
			...
			'done_action'					 => phpgw::link(
				...
				array(
					...
					'p_num'			 => $p_num,
					'preserve'		 => 1,
					'id'			 => $this->entity_id ? ($values['extra']['p_num'] ? $values['extra']['p_num'] : Sanitizer::get_var('p_num')) : ''
				)
			),
			'value_id'				 => $id,
			...
			'multi_upload_action'			 => phpgw::link('/index.php', array('menuaction' => 'property.uidocument.handle_multi_upload_file', 'id' => $id)),
		...
		phpgwapi_xslttemplates::getInstance()->add_file(array('document', 'multi_upload_file_inline', 'datatable_inline'));
	...
	public function handle_multi_upload_file()
	{
		...
		$options['script_url']	 = html_entity_decode(self::link(array(
			'menuaction' => 'property.uidocument.handle_multi_upload_file',
			'id'		 => $id
		)));
	...
	public function build_multi_upload_file()
	{
		...
		Settings::getInstance()->update('flags', ['nofooter' => $this->flags['nofooter']]);
	...
	function view()
	{
		...
		phpgwapi_xslttemplates::getInstance()->add_file(array('document', 'datatable_inline'));
