	...
	function view()
	{
		...
		if (isset($values['approval']) && $values['approval'] && $this->bo->config->config_data['workorder_approval'])
		{
			...
			$message = '<a href ="' . phpgw::link('/index.php', array(
				'menuaction' => 'property.uitts.view',
				'id'		 => $id
			), false, true) . '">' . lang('Workorder %1 needs approval', $ticket['order_id']) . '</a>';
...
JS;
		...
		self::render_template_xsl(
			array('tts', 'multi_upload_file_inline', 'attributes_form', 'datatable_inline', 'cat_sub_select'),
			$data,
			$xsl_rootdir = '',
			'view'
		);
	...
	private function _pdf_order($id = 0, $preview = false, $show_cost = false)
	{
		...
		if (!empty($this->bo->config->config_data['contact_at_location']))
		{
			...
			$_responsible = execMethod('property.boresponsible.get_responsible', array(
				'location'	 => explode('-', $ticket['location_code']),
	...
	private function _html_order($id = 0, $preview = false, $show_cost = false)
	{
		...
		if (!empty($this->bo->config->config_data['contact_at_location']))
		{
			...
			$_responsible = execMethod(
				'property.boresponsible.get_responsible',
				array(
					'location'	 => explode('-', $ticket['location_code']),
					'cat_id'	 => $ticket['cat_id'],
					'role_id'	 => $contact_at_location
