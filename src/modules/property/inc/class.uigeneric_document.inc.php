...
class property_uigeneric_document extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->flags['menu_selection'] = "property::documentation::generic";
		...
		$data = array(
			...
			'datatable'		 => array(
				'source'		 => self::link(array(
					'menuaction'		 => 'property.uigeneric_document.index',
					'phpgw_return_as'	 => 'json'
				)),
				'download'		 => self::link(array(
					'menuaction' => 'property.uigeneric_document.download',
	...
	public function query()
	{
		...
		{
			$item['name']	 = '<a href="' . self::link(array(
				'menuaction' => 'property.uigeneric_document.view_file',
				'file_id'	 => $item['id']
			)) . '">' . $item['name'] . '</a>';
			$item['link']	 = self::link(array(
				'menuaction' => 'property.uigeneric_document.view',
				'id'		 => $item['id']
			));
		}
	...
	public function get_locations_for_type()
	{
		...
		foreach ($locations as $item)
		{
			...
			$values[] = array(
				'location_code'	 => '<a href="' . self::link(array(
					'menuaction'	 => 'property.uilocation.view',
					'location_code'	 => $item['location_code']
				)) . '">' . $item['location_code'] . '</a>',
	...
	public function edit($values = array(), $mode = 'edit')
	{
		...
		if ($id)
		{
			...
			$entity_group	 = execMethod('property.bogeneric.get_list', array(
				'type'		 => 'entity_group',
				'add_empty'	 => true
			));
			...
			$datatable_def[] = array(
				...
				'requestUrl' => json_encode(self::link(array(
					'menuaction'		 => 'property.uigeneric_document.get_componentes',
			...
			$datatable_def[] = array(
				...
				'requestUrl' => json_encode(self::link(array(
					'menuaction'		 => 'property.uigeneric_document.get_locations_for_type',
		...
		$this->flags['app_header'] = lang('property') . '::' . lang('generic document');
	...
	public function get_location_filter()
	{
		...
		$location_filter = Cache::session_get('property', "location_filter_{$entity_group_id}");
		...
		if (!$location_filter)
		{
			...
			Cache::session_set('property', "location_filter_{$entity_group_id}", $location_filter);
		}
	...
	public function save()
	{
		...
		else
		{
			...
			if ($receipt['message'])
			{
				Cache::message_set($receipt['message'], 'message');
			}
			else
			{
				Cache::message_set($receipt['message'], 'error');
			}
	...
	public function get_componentes()
	{
		...
		if ($all_types)
		{
			...
			{
				$values[] = array(
					'id'		 => '<a href="' . self::link(array(
						'menuaction'	 => 'property.uientity.view',
						'location_id'	 => $item['location_id'], 'id'			 => $item['id']
					)) . '">' . $item['id'] . '</a>',
		...
		foreach ($_components as $item)
		{
			...
			$values[] = array(
				'id'		 => '<a href="' . self::link(array(
					'menuaction'	 => 'property.uientity.view',
					'location_id'	 => $location_id, 'id'			 => $item['id']
				)) . '">' . $item['id'] . '</a>',
				'name'		 => $item[$name_field],
