...
class property_uiexternal_communication extends phpgwapi_uicommon
{
	...
	public function __construct()
	{
		...
		$this->flags['app_header'] .= '::' . lang('external communication');
	...
	{
		$data			 = $this->botts->cats->formatted_xslt_list(array(
			'format'	 => 'filter',
	...
	public function index()
	{
		...
		$this->flags['app_header'] = lang($this->currentapp) . "::{$function_msg}";
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
	...
	public function add_deviation()
	{
		...
		$this->flags['app_header']	 .= '::' . lang('deviation');
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);

		phpgwapi_jquery::load_widget('core');
		phpgwapi_jquery::load_widget('autocomplete');
		phpgwapi_jquery::formvalidator_generate(array());
		self::add_javascript($this->currentapp, 'base', 'external_communication.add_deviation.js');
	...
	public function edit($values = array(), $mode = 'edit', $error = false)
	{
		...
		$message_note_def = array(
			...
			array(
				'key'		 => 'value_note', 'label'		 => lang('message'), 'sortable'	 => true,
				'resizeable' => true
			)
		);
...
JS;
		...
		$attach_file_def = array(
			array(
				'key'		 => 'created', 'label'		 => lang('date'), 'sortable'	 => true,
				'resizeable' => true
			),
			array(
		...
		$type_list	 = array();
		$type_list	 = execMethod("{$this->currentapp}.bogeneric.get_list", array(
		...
		$data											 = array(
			...
			'form_action'				 => self::link(array(
				'menuaction' => "{$this->currentapp}.uiexternal_communication.{$mode}",
				'id'		 => $id
			)),
			'edit_action'				 => self::link(array(
				'menuaction' => "{$this->currentapp}.uiexternal_communication.edit",
				'id'		 => $id
			)),
		...
		$this->flags['app_header']	 .= '::' . lang($mode);
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		...
		self::add_javascript($this->currentapp, 'base', 'external_communication.edit.js');
	...
	public function save_ticket()
	{
		...
		if (Db::getInstance()->transaction_commit())
		{
			Cache::message_set(lang('Saved'), 'message');
		}
	...
	private function _send($id)
	{
		...
		$config_admin = CreateObject('admin.soconfig', $this->locations->get_id($this->currentapp, '.admin'))->read();
		...
		try
		{
			$rcpt = $send->msg('email', $_to, $subject, stripslashes($html), '', $cc, $bcc, $coordinator_email, $coordinator_name, 'html', '', $attachments);
	...
	public function send_sms()
	{
		...
		phpgwapi_jquery::formvalidator_generate(array());
		self::add_javascript($this->currentapp, 'base', 'external_communication.send_sms.js');
