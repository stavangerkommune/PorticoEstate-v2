...
class property_uiimport_components extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		$this->flags['menu_selection']	 = 'admin::property::import_components';
		Settings::getInstance()->update('flags', ['menu_selection' => $this->flags['menu_selection']]);
	}

	public function download()
	{
		$components	 = Cache::session_get('property', 'preview_components');
...
HTML;
	...
	{
		$path_upload_dir = Cache::session_get('property', 'path_upload_dir');
	...
	{
		$cached_file = Cache::session_get('property', 'components_import_file');

		if ($_FILES['file']['tmp_name'])
		{
			if ($cached_file)
			{
				Cache::session_clear('property', 'components_import_file');
				unlink($cached_file);
				unset($cached_file);
			}
			...
			Cache::session_set('property', 'components_import_file', $cached_file);
		}
	...
	{
		$sheet_id	 = Cache::session_get('property', 'sheet_id');
		$cached_file = $this->_get_components_cached_file();
		if (!$cached_file)
		{
			$this->receipt['error'][] = array('msg' => lang('Cached file not exists'));
	...
	private function _build_columns()
	{
		...
		$sheet_id	 = Cache::session_get('property', 'sheet_id');
		$start_line	 = Cache::session_get('property', 'start_line');
		$template_id = Cache::session_get('property', 'template_id');
	...
	private function _prepare_profile()
	{
		...
		$template_id			 = Cache::session_get('property', 'template_id');
		$attrib_name_componentID = Cache::session_get('property', 'attrib_name_componentID');
		...
		$profile['attrib_name_componentID']	 = array(
			'id'	 => $attrib_name_componentID,
			'text'	 => $attrib_name_componentID_Text
		);
		Cache::session_set('property', 'profile', serialize($profile));
	...
	private function _prepare_values_to_preview()
	{
			...
			if ((int)$_result['building_part'] || $_result['building_part'] === '0')
			{
				...
				if (array_key_exists((string)$_result['building_part'], $list_entity_categories))
				{
					if (!empty($_result['component_id']))
					{
						...
						$building_part_in_table[$_result['building_part']] = array(
							'entity_id'	 => $entity_id,
							'cat_id'	 => $cat_id
						);
					}
		...
		if (count($new_attribs_for_template))
		{
			...
			Cache::session_set('property', 'new_attribs_for_template', serialize($new_attribs_for_template));
		...
		{
			Cache::session_set('property', 'new_attribs_for_template', '');
		...
		if (count($new_entity_categories))
		{
			...
			Cache::session_set('property', 'new_entity_categories', serialize($new_entity_categories));
			$result['new_entity_categories'] = array_values($new_entity_categories);
		}
		else
		{
			Cache::session_set('property', 'new_entity_categories', '');
		...
		Cache::session_set('property', 'building_part_in_table', serialize($building_part_in_table));
		Cache::session_set('property', 'preview_components', serialize($preview_components));
		Cache::session_set('property', 'new_components', serialize($import_data));
	...
	private function _save_values_import()
	{
		...
		$template_id			 = Cache::session_get('property', 'template_id');
		$attrib_name_componentID = Cache::session_get('property', 'attrib_name_componentID');
		$location_code			 = Cache::session_get('property', 'location_code');
		...
		$import_data = Cache::session_get('property', 'new_components');
		...
		if ($save_profile)
		{
			...
			if ($cod_profile)
			{
				$content							 = Cache::session_get('property', 'profile');
	...
	public function import_components()
	{
		...
		if ($step == 2)
		{
			...
			Cache::session_set('property', 'sheet_id', $sheet_id);
		...
		if ($step == 3)
		{
			...
			Cache::session_set('property', 'start_line', $start_line);
			Cache::session_set('property', 'template_id', $template_id);
		...
		if ($step == 4)
		{
			...
			Cache::session_set('property', 'attrib_name_componentID', $attrib_name_componentID);
		...
		if ($step == 5 && $save)
		{
			...
			Cache::session_set('property', 'location_code', $location_code);
			Cache::session_set('property', 'location_item_id', $location_item_id);
	...
	public function index()
	{
		...
		$tabs['components']	 = array(
			'label'		 => lang('Components'), 'link'		 => '#components',
			'disable'	 => 1
		);
		$tabs['files']		 = array('label' => lang('Files'), 'link' => '#files', 'disable' => 0);
		$tabs['relations']	 = array(
			'label'		 => lang('Relations'), 'link'		 => '#relations',
			'disable'	 => 1
		);
		...
		$related_def = array(
			array(
				'key'		 => 'location_code', 'label'		 => lang('location'), 'sortable'	 => true,
				'resizeable' => true
			),
		...
		$datatable_def[] = array(
			...
			'requestUrl' => json_encode(self::link(array(
				'menuaction'		 => 'property.uiimport_components.get_locations_for_type',
		...
		{
			Cache::session_set('property', 'path_upload_dir', $import_component_files->get_path_upload_dir());
		}
		...
		$this->flags['app_header'] = lang('property') . '::' . lang('import components');
	...
	public function get_attributes_from_template()
	{
		...
		$attrib_list = $this->bo->read_attrib(array(
		...
		foreach ($attrib_list as $attrib)
		{
			...
			$list[]		 = array(
				'id'		 => $attrib['column_name'], 'name'		 => $attrib['input_text'],
				'selected'	 => $selected
			);
		}
	...
	public function get_categories_for_type()
	{
		...
		$categories = $this->bocommon->select_category_list(
			array(
				'format'	 => 'filter',
				'selected'	 => '',
				'type'		 => 'location',
