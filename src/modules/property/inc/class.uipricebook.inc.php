...
class property_uipricebook extends phpgwapi_uicommon_jquery
{
	...
	function download()
	{
		...
		$name	 = array(
			'num', 'branch', 'vendor_id', 'm_cost', 'w_cost', 'total_cost',
	...
	function index()
	{
		...
		if ($values['submit_update'] && Sanitizer::get_var('phpgw_return_as') == 'json')
		{
		...
		$data = array(
			...
			'datatable'		 => array(
				...
				'field'		 => array(
					array(
						'key'		 => 'activity_id', 'label'		 => lang('Activity Id'), 'sortable'	 => false,
						'hidden'	 => TRUE, 'className'	 => 'center'
					),
					array(
						'key'		 => 'num', 'label'		 => lang('Activity num'), 'sortable'	 => true,
						'hidden'	 => FALSE, 'className'	 => 'center'
					),
					array(
						'key'		 => 'vendor_id', 'label'		 => lang('Vendor'), 'sortable'	 => false,
						'hidden'	 => FALSE, 'className'	 => 'center'
					),
					array(
						'key'		 => 'branch', 'label'		 => lang('branch'), 'sortable'	 => false,
						'hidden'	 => FALSE,
						'className'	 => 'center'
					),
					array(
						'key'		 => 'descr', 'label'		 => lang('Description'), 'sortable'	 => false,
						'hidden'	 => FALSE
					),
					array(
						'key'		 => 'unit', 'label'		 => lang('Unit'), 'sortable'	 => false, 'hidden'	 => FALSE,
						'className'	 => 'center'
					),
					array(
						'key'		 => 'w_cost', 'label'		 => lang('Labour cost'), 'sortable'	 => false,
						'hidden'	 => FALSE, 'className'	 => 'right'
					),
					array(
						'key'		 => 'm_cost', 'label'		 => lang('Material cost'), 'sortable'	 => false,
						'hidden'	 => FALSE, 'className'	 => 'right'
					),
					array(
						'key'		 => 'total_cost', 'label'		 => lang('Total Cost'), 'sortable'	 => true,
						'hidden'	 => FALSE, 'className'	 => 'right'
					),
					array(
					...
					array(
						'key'		 => 'agreement_id', 'label'		 => lang('Agreement id'), 'sortable'	 => false,
						'hidden'	 => TRUE, 'className'	 => 'center'
					),
					array(
	...
	function agreement_group()
	{
		...
		$this->flags['menu_selection'] = 'property::agreement::pricebook::group';
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);

		Cache::session_clear('property', 'referer');
	...
	function edit_agreement_group()
	{
		...
		$agreement_group_id	 = Sanitizer::get_var('agreement_group_id', 'int');
		$values				 = Sanitizer::get_var('values');
		$tabs				 = array();
		$tabs['general']	 = array('label' => lang('general'), 'link' => '#general');
		$active_tab			 = 'general';

		phpgwapi_xslttemplates::getInstance()->add_file(array('pricebook'));
		...
		$data = array(
			...
			'done_action'				 => phpgw::link('/index.php', array('menuaction' => 'property.uipricebook.agreement_group')),
		...
		$appname = lang('pricebook');
		self::set_active_menu('property::agreement::pricebook::group');

		$this->flags['app_header'] = lang('property') . ' - ' . $appname . ': ' . $function_msg;
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);

		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('edit_agreement_group' => $data));
	}
	...
	function prizing()
	{
		...
		$vendor_id		 = Sanitizer::get_var('vendor_id', 'int', 'GET');
		$agreement_id	 = Sanitizer::get_var('agreement_id', 'int', 'GET');
		$values			 = Sanitizer::get_var('values');

		$referer = Cache::session_get('property', 'referer');

		if (!$referer)
		{
			$referer = Sanitizer::get_var('HTTP_REFERER', 'string', 'SERVER', Sanitizer::clean_value($GLOBALS['HTTP_REFERER']));
			$referer .= '&cat_id=' . $cat_id;
			Cache::session_set('property', 'referer', $referer);
		}
		...
		if (isset($pricebook_list) and is_array($pricebook_list))
		{
			foreach ($pricebook_list as $pricebook)
			{
				...
				$content[] = array(
					...
					'total_cost'			 => $pricebook['total_cost'],
					'this_index'			 => $pricebook['this_index'],
					'date'					 => $this->phpgwapi_common->show_date($pricebook['date'], $dateformat),
		...
		$data = array(
			...
			'update_action'			 => phpgw::link('/index.php', array(
				'menuaction'	 => 'property.uipricebook.prizing',
		...
		$this->flags['app_header'] = lang('property') . ' - ' . $appname . ': ' . $function_msg;
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('prizing' => $data));
	...
	function activity()
	{
		...
		self::set_active_menu('property::agreement::pricebook::activities');
		...
		$uicols = array(
			'name'		 => array(
				'activity_id', 'num', 'descr', 'unit_name', 'ns3420', 'base_descr',
				'branch', 'dim_d'
			),
			'input_type' => array(
			...
			'descr'		 => array(
				'', lang('Activity Num'), lang('Description'), lang('Unit'),
...
JS;
	...
	function activity_vendor()
	{
		...
		phpgwapi_xslttemplates::getInstance()->add_file(array('pricebook', 'nextmatchs', 'search_field'));
	...
	function edit_activity()
	{
		...
		$activity_id		 = Sanitizer::get_var('activity_id', 'int');
		$agreement_group	 = Sanitizer::get_var('agreement_group', 'int', 'GET');
		$values				 = Sanitizer::get_var('values');
		...
		$this->flags['app_header'] = lang('property') . ' - ' . $appname . ': ' . $function_msg;
		Settings::getInstance()->update('flags', ['app_header' => $this->flags['app_header']]);
		phpgwapi_xslttemplates::getInstance()->set_var('phpgw', array('edit_activity' => $data));
	}
	...
	function delete()
	{
		...
		$activity_id		 = Sanitizer::get_var('activity_id', 'int');
		$agreement_id		 = Sanitizer::get_var('agreement_id', 'int', 'GET');
		$index_count		 = Sanitizer::get_var('index_count', 'int', 'GET');
		$agreement_group_id	 = Sanitizer::get_var('agreement_group_id', 'int');
