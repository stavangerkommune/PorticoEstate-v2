...
class activitycalendar_uiactivities extends activitycalendar_uicommon
{
	...
	public function add()
	{
		phpgw::redirect_link('/index.php', array('menuaction' => 'activitycalendar.uiactivities.edit'));
	}
		...
		$data = array(
			...
			'datatable' => array(
				...
				'download' => self::link(array(
					'menuaction' => 'activitycalendar.uiactivities.download',
					'type' => $type,
					'export' => true,
					'allrows' => true
				)),
				...
				'field' => array(
					...
					array(
						...
						'label' => lang('contact_person_1'),
						'sortable' => true,
						'hidden' => false
					),
					array(
						'key' => 'contact_person_2',
						'label' => lang('contact_person_2'),
						'sortable' => true,
						'hidden' => false
					),
					array(
						'key' => 'last_change_date',
						'label' => lang('last_change_date'),
						'sortable' => true,
						'hidden' => false
					)
				)
	...
	public function view()
	{
		...
		Settings::getInstance()->update('flags', ['app_header' =>lang('activitycalendar'). '::' . lang('view')]);
		// Get the contract part id
		$activity_id = (int)Sanitizer::get_var('id');
		...
		$data = array(
			...
			'cancel_url' => phpgw::link('/index.php', array('menuaction' => 'activitycalendar.uiactivities.index')),
	...
	{
		Settings::getInstance()->update('flags', ['app_header' => lang('activitycalendar') . '::' . lang('edit')]);
		...
		$g_id = Sanitizer::get_var('group_id');
		...
		if ($new_group)
		{
			$group_array = $this->so_group->get(0, 0, '', false, '', '', array(
				'group_id' => $activity->get_group_id(),
				'new_groups' => 'true'
			));
		...
		$category_options[] = array(
			'id' => '',
			'name' => lang('Ingen kategori valgt'),
			'selected' => 0
		);
		$current_category_id = $activity->get_category();
		foreach ($categories as $category)
		...
		$organization_options[] = array(
			'id' => '',
			'name' => lang('Ingen organisasjon valgt'),
			'selected' => 0
		);
		$current_organization_id = $activity->get_organization_id();
		...
		$data = array(
			'tabs' => phpgwapi_jquery::tabview_generate($tabs, $active_tab),
			'form_action' => phpgw::link('/index.php', array('menuaction' => 'activitycalendar.uiactivities.save')),
			'cancel_url' => phpgw::link('/index.php', array('menuaction' => 'activitycalendar.uiactivities.index')),
			'lang_save' => lang('save'),
			'lang_cancel' => lang('cancel'),
			...
			'organization_url' => phpgw::link('/index.php', array(
				'menuaction' => 'booking.uiorganization.show',
			...
			'group_url' => phpgw::link('/index.php', array(
				'menuaction' => 'booking.uigroup.show',
...
JS;
	...
	public function query()
	{
		...
		$email_param = Sanitizer::get_var('email');
		$email = false;
		...
		$uid = $this->userSettings['account_id'];
		...
		switch ($query_type)
		{
			case 'new_activities':
				$filters = array(
					'new_activities' => 'yes',
					'activity_state' => Sanitizer::get_var('activity_state'),
					'activity_category' => Sanitizer::get_var('activity_category'),
					'activity_district' => Sanitizer::get_var('activity_district'),
					'user_id' => $uid,
					'updated_date_hidden' => Sanitizer::get_var('date_change')
				);
	...
	public function save()
	{
		...
		if (isset($activity)) // If an activity object is created
		{
			...
			$activity->set_organization_id(Sanitizer::get_var('organization_id'));
			$activity->set_group_id(Sanitizer::get_var('group_id'));
			$internal_arena = Sanitizer::get_var('internal_arena_id');
			...
			{
				$activity->set_arena(Sanitizer::get_var('arena_id'));
			...
			$activity->set_contact_person_2_address(Sanitizer::get_var('contact_person_2_address'));
			$activity->set_contact_person_2_zip(Sanitizer::get_var('contact_person_2_zip'));
	...
	public function send_mail()
	{
		...
		$mailBaseURL = $c->config_data['mailBaseURL'];
		$activity_id = (int)Sanitizer::get_var('activity_id');
		$activity = activitycalendar_soactivity::get_instance()->get_single($activity_id);

		$message_type = Sanitizer::get_var('message_type');
		if ($message_type)
		{
	...
	{
		$activity_id = Sanitizer::get_var('act_id');
