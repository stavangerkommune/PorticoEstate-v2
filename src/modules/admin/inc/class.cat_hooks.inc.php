...
class admin_cat_hooks
{
	...
	function cat_add($data)
	{
		...
		if ($data['location_id'])
		{
			...
			$location_info = $location_obj->get_name($data['location_id']);
			$location = $location_info['location'];
			if ($location == 'vfs_filedata')
			{
				$location_obj->add("vfs_filedata.{$data['cat_id']}", $data['cat_name'], 'admin', false, false, false, true);
			}
		}
	}
	...
	function cat_delete($data)
	{
		...
		if ($data['location_id'])
		{
			...
			if ($location_info['location'] == 'vfs_filedata')
			{
				...
				$location_obj->delete('admin', $location, false);
			}
		}
	}
	...
	function cat_edit($data)
	{
		...
		if ($data['location_id'])
		{
			...
			{
				$location = "{$location_info['location']}.{$data['cat_id']}";
				$location_obj->update_description($location, $data['cat_name'], 'admin');
			}
		}
	}
}
