...
class admin_ui_custom extends phpgwapi_uicommon_jquery
{
	...
	public function __construct()
	{
		...
		{
			$this->acl_read		 = $this->acl->check($this->acl_location, ACL_READ, $this->appname);
			$this->acl_add		 = $this->acl->check($this->acl_location, ACL_ADD, $this->appname);
			$this->acl_edit		 = $this->acl->check($this->acl_location, ACL_EDIT, $this->appname);
			$this->acl_delete	 = $this->acl->check($this->acl_location, ACL_DELETE, $this->appname);
		}

		if ($this->flags['menu_selection'] = Sanitizer::get_var('menu_selection'))
		{
			$this->menu_selection = $this->flags['menu_selection'];
		}
		else
		{
			$this->flags['menu_selection'] = "admin::{$this->appname}";
			Settings::getInstance()->set('flags', $this->flags);
			$this->local_menu_selection = true;
		}

		if (Sanitizer::get_var('return_to_referer', 'bool'))
		{
			$referer = $_SERVER['HTTP_REFERER'];
			Cache::session_set('phpgwapi', 'ui_custom_referer', $referer);
		}
	}
	...
	function list_attribute()
	{
		...
		{
			//			$this->flags['menu_selection'] .= '::list_atrribs';
			//		Settings::getInstance()->set('flags', $this->flags);
		...
		if (is_array($attrib_list))
		{
			foreach ($attrib_list as $entry)
			{
				$content[] = array(
					...
					'search'				=> $entry['search'],
					'link_up'			 => phpgw::link('/index.php', array(
						'menuaction' => 'admin.ui_custom.list_attribute',
						'resort' => 'up', 'appname' => $appname, 'location' => $location, 'id' => $entry['id'],
						'allrows' => $this->allrows, 'menu_selection' => $this->flags['menu_selection']
					)),
					'link_down'			 => phpgw::link('/index.php', array(
						'menuaction' => 'admin.ui_custom.list_attribute',
					...
					'link_edit'			 => phpgw::link('/index.php', array(
						...
						'appname' => $appname, 'location' => $location, 'id' => $entry['id'], 'menu_selection' => $this->flags['menu_selection']
					)),
		...
		$referer =	Cache::session_get('phpgwapi', 'ui_custom_referer');
		...
		$table_add[] = array(
			...
			'add_action'			 => phpgw::link('/index.php', array(
				'menuaction' => 'admin.ui_custom.edit_attrib',
				'appname' => $appname, 'location' => $location, 'menu_selection' => $this->flags['menu_selection']
			)),
			'lang_done'				=> lang('done'),
	...
	function edit_attrib()
	{
		...
		if (isset($values['save']) && $values['save'])
		{
			...
			if (!isset($receipt['error']))
			{
				...
				//	phpgw::redirect_link('/index.php', array('menuaction' => 'admin.ui_custom.edit_attrib', 'appname' => $values['appname'], 'location' => $values['location'], 'id' => $id, 'menu_selection' => $this->flags['menu_selection']));
			}
			else
			{
	...
	function list_custom_function()
	{
		...
		if ($this->local_menu_selection)
		{
			$this->flags['menu_selection'] .= '::list_functions';
			Settings::getInstance()->set('flags', $this->flags);
		}
	...
	{
		$acl_delete	 = $this->acl->check($this->location, ACL_DELETE, $this->appname);
	...
	function edit_attrib_group()
	{
		...
		if (isset($values['save']) && $values['save'])
		{
			...
			if ($id)
			{
				if (!$this->acl->check($this->location, ACL_EDIT, $this->appname))
				{
					phpgw::no_access($this->appname);
				}
				$values['id']	 = $id;
			...
			{
				if (!$this->acl->check($this->location, ACL_ADD, $this->appname))
				{
					phpgw::no_access($this->appname);
				}
			}
		...
		$parent_list = $custom_fields->find_group($appname, $location, 0, '', '', '', true);
		...
		{
			$exclude	 = array($id);
			$children	 = $custom_fields->get_attribute_group_children($location_id, $id, 0, 0, true);
