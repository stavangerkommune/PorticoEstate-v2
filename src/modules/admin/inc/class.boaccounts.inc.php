...
class admin_boaccounts
{
	...
	public function check_rights($action, $access = 'group_access')
	{
		...
		$result = !$this->acl->check($access, $right, 'admin');
		return $result;
	}
	...
	{
		if ($this->acl->check('group_access', Acl::EDIT, 'admin'))
		{
		...
		if (
			$values['account_id'] == 0
			&& $this->acl->check('group_access', Acl::ADD, 'admin')
		)
		{
		...
		$old_group = $this->accounts->get($values['account_id']);
		...
		{
			if ($this->accounts->exists($values['account_name']))
			{
		...
		{
			$new_group = $this->accounts->get($values['account_id']);
		}
		...
		{
			$this->accounts->update_group($new_group, $values['account_apps']);
		}

		//Delete cached menu for members of group
		$members = $this->accounts->member($id);
	...
	function save_user(&$values)
	{
		...
		if (
			...
			&& $this->acl->check('account_access', Acl::ADD, 'admin')
		)
		{
		...
		if ($values['id'])
		{
			$user = $this->accounts->get($values['id'], $use_cache = false);
		}
		else
		{
		...
		{
			if ($this->accounts->exists($values['lid']))
			{
		...
		if (
			!count($values['account_permissions'])
		...
		if ($user->id)
		{
			Cache::user_clear('phpgwapi', 'menu', $user->id);
		}
		...
		if ($user->id)
		{
			if ($this->accounts->update_user($user, $groups, $acls, $apps))
			{
				return $user->id;
			}
		}
		...
		{
			return $this->accounts->create($user, $groups, $acls, $apps);
			{
				return $user->id;
			}
		}
	...
	{
		if ($this->acl->check('group_access', Acl::GROUP_MANAGERS, 'admin'))
		{
			phpgw::redirect_link(
				'index.php',
		...
		return $this->accounts->delete($group_id);
	}
	...
	{
		if ($this->acl->check('account_access', Acl::GROUP_MANAGERS, 'admin'))
		{
			phpgw::redirect_link(
				'index.php',
	...
	{
		$temp_user = $this->accounts->member($account_id);
		if (!$temp_user)
		{
			return array();
		}
	...
	function load_apps($account_id, $inherited = false)
	{
		...
		if ($account_id)
		{
			$apps = (new App\modules\phpgwapi\controllers\Applications($account_id))->read_account_specific($inherited);
	...
	function refresh_session_data($id)
	{
		...
		Cache::session_clear('phpgwapi', 'phpgw_info');
	}
