...
class registration_bopending
{
	...
	var $use_session;
	var $total_records = 0;
	var $so, $status_id, $allrows, $acl_location,
	 $location_code, $status, $uicols, $custom,
	  $bocommon, $config, $phpgwapi_common, $serverSettings, $userSettings;
	...
	{
		$data = Cache::session_get('pending_user', 'session_data');
	...
	function save_sessiondata($data)
	{
		if ($this->use_session)
		{
			$data = Cache::session_set('pending_user', 'session_data', $data);
		}
	}
	...
	function read($data = array())
	{
		...
		{
			$user['reg_dla'] = $this->phpgwapi_common->show_date($user['reg_dla']);
		}
	...
	function approve_users($values)
	{
		...
		foreach ($custom_functions as $entry)
		{
			...
			$file = PHPGW_SERVER_ROOT . "/registration/inc/custom/{$this->userSettings['domain']}/{$entry['file_name']}";
