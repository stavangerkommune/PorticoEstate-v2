...
class registration_uipending extends phpgwapi_uicommon_jquery
{
	...
	public function edit()
	{
		...
		if (isset($_POST['save']) && $id)
		{
			...
			$insert_record_entity = Cache::session_get('property', 'insert_record_entity');
			...
			{
				$message = lang('messages_form_error');
		...
		if (!$this->acl->check('run', Acl::READ, 'admin'))
		{
			$valid_apps = $this->acl->get_app_list_for_id('admin', Acl::ADD, $this->userSettings['account_id']);
		}
		else
		{
		...
		Settings::getInstance()->update('flags', ['app_header' => lang('registration') . '::' . lang('edit user')]);
