...
class registration_boreg
{
	...
	function step1()
	{
		...
		else
		{
			...
			{
				Cache::session_set('registration', 'loginid', $r_reg['email']);
				$r_reg['loginid'] = $r_reg['email'];
			}
			else
			{
				Cache::session_set('registration', 'loginid', $r_reg['loginid']);
			}
			$ui->step2();
		}
	}
	...
	function step2()
	{
		...
		if ($this->config['username_is'] == 'email')
		{
			...
			else
			{
				...
				$loginid		 = Cache::session_get('registration', 'loginid');
		...
		if (!$errors)
		{
			...
			if ($ssn)
			{
				...
				$fields['ssn_hash']	 = Db::getInstance()->db_addslashes($ssn_hash); // just to be safe :)
			}
		...
		{
			Cache::session_clear('registration', 'loginid');
	...
	function lostpw1()
	{
		...
		if (!$errors && !$accounts_obj->exists($loginid))
		{
	...
	function lostpw3()
	{
		...
		$lid = Cache::session_get('registration', 'loginid');
		if (!$lid)
		{
