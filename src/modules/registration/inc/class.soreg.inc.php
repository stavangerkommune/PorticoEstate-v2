...
class registration_soreg
{
	...
	var $reg_id;
	var $db;
	var $config, $boaddressbook, $accounts_obj, $phpgwapi_common, $template, $serverSettings;
	...
	function account_exists($account_lid)
	{
		...
		if ($this->accounts_obj->exists($account_lid) || $this->db->f('cnt'))
		{
		...
		else
		{
			...
			$this->db->transaction_commit();
			Cache::session_set('registration', 'loginid', $account_lid);
			return false;
		}
	}
	...
	{
		$account_lid = Cache::session_get('registration', 'loginid');
		$this->reg_id = md5(time() . $account_lid . $this->phpgwapi_common->randomstring(32));
...
HTML;
			...
			$noreply = 'No reply <noreply@' . $this->serverSettings['hostname'] . '>';
	...
	This is a confirmation email for your new account on {$this->serverSettings['system_name']}::{$this->serverSettings['site_title']}.
...
HTML;
		...
		else
		{
			...
			if ($fields['n_given'])
			{
				$this->template->set_var('firstname', $fields['n_given'] . ' ');
			}

			if ($fields['n_family'])
			{
				$this->template->set_var('lastname', $fields['n_family'] . ' ');
			}


			$url = phpgw::link('/registration/', array(
				'menuaction' => 'registration.boreg.step4',
				'reg_id' => $this->reg_id, 'logindomain' => $_REQUEST['logindomain']
			), false, true);
			$this->template->set_var('activate_url', "</br><a href='$url'>Link.</a></br>");

			if ($this->config['support_email'])
			{
				$this->template->set_var('support_email_text', lang('Report all problems and abuse to'));
				$this->template->set_var('support_email', $this->config['support_email']);
			}

			$body = $this->template->fp('out', 'message');
		}

		$subject = $this->config['subject_confirm'] ? lang($this->config['subject_confirm']) : lang('Account registration');
		$noreply = $this->config['mail_nobody'] ? ('No reply <' . $this->config['mail_nobody'] . '>') : ('No reply <noreply@' . $this->serverSettings['hostname'] . '>');
	...
	function lostpw1($account_lid)
	{
		...
		//
		$reg_id = md5(time() . $account_lid . $this->phpgwapi_common->randomstring(32));
		...
		$user_id = $this->accounts_obj->name2id($account_lid);

		$account_info = $this->accounts_obj->get($user_id);
		...
		if (!$info['email'])
		{
			$preferences = createObject('phpgwapi.preferences', $user_id);
			$info['email'] = isset($preferences->data['common']['email']) && $preferences->data['common']['email'] ? $preferences->data['common']['email'] : '';
		}
		...
		if ($info['email'])
		{
			...
			$url = phpgw::link('/registration/', array(
				'menuaction' => 'registration.boreg.lostpw2',
				'reg_id' => $reg_id, 'logindomain' => $_REQUEST['logindomain']
			), false, true);
			$this->template->set_var('firstname', $info['firstname']);
			$this->template->set_var('lastname', $info['lastname']);
			$this->template->set_var('activate_url', "</br><a href='$url'>Link.</a></br>");
			...
			try
			{
				$smtp->msg('email', $info['email'], $subject, $this->template->fp('out', 'message'), '', '', '', $noreply, '', 'html');
			}
			catch (Exception $e)
			{
				$error = $e->getMessage();
	...
	function lostpw2($account_lid)
	{
		...
		Cache::session_set('registration', 'loginid', $account_lid);
		Cache::session_set('registration', 'id', $account_id);
	}
	...
	function lostpw3($account_lid, $passwd)
	{
		...
		$account_id = Cache::session_get('registration', 'id');
		$auth->change_password('supposed to be old password', $passwd, $account_id);
	...
	function create_account($account_lid, $_reg_info)
	{
		...
		$group_id = $default_group_id ? $default_group_id : $this->accounts_obj->name2id('default');
		...
		if (!$this->accounts_obj->exists($account_lid))
		{
			...
			$account_id = $this->accounts_obj->create($account, $groups, $acls, $apps, $contact_data);
			if ($account_id)
			{
				$log = new Log();
				$log->write(array(
		...
		if (isset($this->config['messenger_welcome_message']) && $this->config['messenger_welcome_message'])
		{
			...
			$hooks->single($args, 'registration');
		}
		...
		if (isset($fields['location_code']) && $fields['location_code'])
		{
			$args = array(
				...
				'contact_id' => $this->accounts_obj->get($account_id)->person_id,
				'account_lid' => $account_lid,
				'account_id' => $account_id,
				'email' => $fields['email']
			);

			$hooks->single($args, 'property');
		}
		$this->db->transaction_commit();
