...
class Login
{
	...
	{
		$create_account = new \App\Modules\Api\Security\Sso\CreateAccount();
		$create_account->display_create();
	}

	public function create_mapping()
	{
		$CreateAccount = new \App\Modules\Api\Security\Sso\CreateMapping();
	...
	public function login()
	{
		...
		if (isset($_POST['login']) && $this->serverSetting['auth_type'] == 'sql') {
			...
			if ($receipt) {
				\App\Modules\Api\Services\Cache::message_set($receipt, 'message');
			}
			return $this->_sessionid;
		}
	}
