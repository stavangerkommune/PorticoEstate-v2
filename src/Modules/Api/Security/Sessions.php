...
//		\App\Helpers\DebugArray::debug($this->db->get_transaction());
	...
	public function create($login, $passwd = '', $skip_auth = false)
	{
		...
		if (
			\App\Modules\Api\Security\GloballyDenied::user($this->_account_lid)
...
//		\App\Helpers\DebugArray::debug($user_info);
	...
	public function read_repositories( $use_cache = false)
	{
		...
		$apps = (new \App\Modules\Api\Controllers\Applications($this->_account_id))->read();

		Settings::getInstance()->set('apps', $apps);
		
		$accounts = new \App\Modules\Api\Controllers\Accounts\Accounts($this->_account_id);
  ...
  //      die();
	...
	protected function _login_blocked($login, $ip)
	{
		...
		if (
			...
			$send = new \App\Modules\Api\Services\Send();
			...
			$config = new \App\Modules\Api\Services\Config('phpgwapi');
