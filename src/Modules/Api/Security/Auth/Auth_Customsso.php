...
/*
	...
	namespace App\Modules\Api\Security\Auth;
	use PDO;
	...
	*/
	class Auth extends Auth_
	{
		...
		*/
		...
		// Alternative config
			$locations = new \App\Modules\Api\Controllers\Locations();
			...
			try {
				if ($config['fellesdata']['host']) {
					if (!$this->ping($config['fellesdata']['host'])) {
						...
						\App\Modules\Api\Services\Cache::message_set($message, 'error');
					}
					...
					$config = (new \App\Modules\Api\Services\Config('rental'))->read();
		...
		*/
		...
		{
			$userSettings = \App\Modules\Api\Services\Settings::getInstance()->get('user');
