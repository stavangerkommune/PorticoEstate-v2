...
namespace App\Modules\Api\Security\Sso;
use App\Modules\Api\Security\Sso\Mapping;
...
class CreateAccount
{
	...
	public function display_create()
	{
		...
		if (isset($submit) && $submit) {
			...
			$account = new \App\Modules\Api\Controllers\Accounts\phpgwapi_user();
			try {
			...
			if (!is_array($error) || count($error) == 0) {
				...
				if ($account_id) {
					...
					$preferences = new \App\Modules\Api\Services\Preferences($account_id);
					...
					$Log = new \App\Modules\Api\Services\Log();
					$Log->write(array(
