...
//			$this->attributes[] = '#NAMEHERE#';
			...
			if(!$this->dav_user)
			{
				$this->dav_user = isset($this->userSettings['userid']) ? (string) $this->userSettings['userid'] : '';
			}
			//FIXME pwd has to be clear text.
			if(!$this->dav_pwd)
			{
				$this->dav_pwd = isset($this->userSettings['passwd']) ? (string) $this->userSettings['passwd'] : '';
			}
			// For testing purpose:
...
*/
		...
		*/
		function acl_check ($data)
		{
			...
			{
				if ($this->serverSettings['acl_default'] == 'grant')
				{
					return True;
				}
				else
		...
		*/
		function lock_token ($data)
		{
			...
			$data['operation'] = ACL_READ | ACL_EDIT | ACL_ADD;
	...
	$this->debug('Read:returning contents.  Status:'.$status);
		...
		*/
		...
		{	
			...
			if ( isset($this->serverSettings['include_pwd']) && $this->serverSettings['include_pwd'] )
			{
...
$this->debug('cp : from forbidden by ACL !');
			...
			{
				$remote_operation=ACL_EDIT;
			}
			else
			{
				$remote_operation=ACL_ADD;
...
//								'deleteable' => 'Y',
				...
				{ 
					if ($this->serverSettings['acl_default'] != 'grant')
					{
						$htaccess = 'require user '.$this->userSettings['account_lid'];
						if ( ! $this->write(array(
	...
	*/
		...
		* See vfs_shared
		*/
		function set_attributes ($data,$operation=ACL_EDIT,$is_dir=False)
		{
