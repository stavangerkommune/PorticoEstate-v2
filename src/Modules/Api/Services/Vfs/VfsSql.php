<?php
	...
	*/
	...
	use App\Modules\Api\Services\Vfs\VfsFileoperationFilesystem;
	...
	class Vfs extends VfsShared
	{
		...
		*
		...
		function add_journal($data)
		{
			...
			if(!$this->acl_check(array('string' => $p->fake_full_path, 'relatives' => array($p->mask), 'operation' => ACL_ADD)) &&
		...
		*
		...
		function acl_check($data)
		{
			...
			return $GLOBALS['phpgw']->acl->check('run', ACL_READ, $currentapp);
		}
