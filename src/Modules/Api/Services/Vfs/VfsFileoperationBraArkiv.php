<?php
	...
	*/
	...
	use App\Modules\Api\Controllers\Locations;
	Use Exception;
	...
	class VfsFileoperationBraArkiv
	{
		...
		public function __construct()
		{
			...
			$location_id		= $this->locations->get_id('admin', 'vfs_filedata');
			$config = (new \App\Modules\Api\Services\ConfigLocation($location_id))->read();


			$section = 'braArkiv';
			$location_url = $config[$section]['location_url'];//'http://braarkiv.adm.bgo/service/services.asmx';
			$braarkiv_user =  $config[$section]['braarkiv_user'];
			$braarkiv_pass =  $config[$section]['braarkiv_pass'];

			if(!isset($config) || !$config)
			{
				$this->config = (new \App\Modules\Api\Services\ConfigLocation($location_id));
				$this->init_config();
			}
