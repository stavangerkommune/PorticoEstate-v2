...
namespace App\Modules\Api\Controllers\Accounts;
...
use App\Modules\Api\Controllers\Accounts\phpgwapi_group;
use App\Modules\Api\Controllers\Accounts\phpgwapi_user;
...
include_once SRC_ROOT_PATH . "/Modules/Api/Controllers/Accounts/AccountTypes.php";
switch ($acct_type) {
	case 'sqlldap':
		include_once SRC_ROOT_PATH . "/Modules/Api/Controllers/Accounts/AccountsSql.php";
		//fall through

	case 'ldap':
		include_once SRC_ROOT_PATH . "/Modules/Api/Controllers/Accounts/AccountsLdap.php";
		break;

	default:
		$acct_type = 'Sql';
}
include_once SRC_ROOT_PATH . "/Modules/Api/Controllers/Accounts/Accounts{$acct_type}.php";
...
abstract class Accounts_
{
	...
	public function update_group($group, $modules = null)
	{
		...
		// module permissions
		if (is_array($modules)) {
			$apps = new \App\Modules\Api\Controllers\Applications($group->id);
		...
		(new \App\Modules\Api\Services\Hooks())->process('editgroup');
	...
	public function update_user($user, $groups, $acls = array(), $modules = null)
	{
		...
		$installed_apps = \App\Modules\Api\Services\Settings::getInstance()->get('apps');
		...
		// application permissions
		if (is_array($modules)) {
			$apps = new \App\Modules\Api\Controllers\Applications($user->id);
		...
		(new \App\Modules\Api\Services\Hooks())->process('editaccount');
	...
	{
		\App\Modules\Api\Services\Cache::system_set('phpgwapi', "account_{$account->id}", $account);
	}
	...
	protected function _create_group($group, $members)
	{
		...
		(new \App\Modules\Api\Services\Hooks())->process('addgroup');
	...
	protected function _create_user($user, $groups, $contact_data = array())
	{
		...
		$Preferences = \App\Modules\Api\Services\Preferences::getInstance($user->id);
		$Hooks = new \App\Modules\Api\Services\Hooks();
