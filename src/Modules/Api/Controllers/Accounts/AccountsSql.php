...
use App\Modules\Api\Controllers\Accounts\Accounts_;
...
class Accounts extends Accounts_
{
	...
	public function delete($account_id)
	{
		...
		if ($deleted) {
			...
			\App\Modules\Api\Services\Cache::system_clear('phpgwapi', "account_{$account_id}");
	...
	public function get($id, $use_cache = true)
	{
		...
		if ($use_cache) {
			$account = \App\Modules\Api\Services\Cache::system_get('phpgwapi', "account_{$id}");
		...
		if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			\App\Modules\Api\Services\Cache::system_set('phpgwapi', "account_{$id}", $account);
		} else {
			$account = new phpgwapi_user();
		}
		$cache[$id] = $account;
