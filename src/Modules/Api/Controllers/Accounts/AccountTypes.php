 ...
 */
	...
	class phpgwapi_group extends phpgwapi_account
	{
		...
		private function _validate_groupname($group, $lookup = true)
		{
			...
			if (\App\Modules\Api\Security\GloballyDenied::user($group) )
			{
	...
	//		unset($array['passwd'], $array['passwd_hash']);
		...
		public function __set($name, $value)
		{
			switch($name)
			{
				...
				case 'passwd':
					...
					$Auth = new \App\Modules\Api\Security\Auth\Auth();
		...
		private function _validate_quota($quota)
		{
			...
			$config = (new \App\Modules\Api\Services\Config('filemanager'))->read_repository();
		...
		private function _validate_username($username, $lookup = true)
		{
			...
			if (\App\Modules\Api\Security\GloballyDenied::user($username))
			{
